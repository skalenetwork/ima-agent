...
function discoverBlsThreshold(
  ...
  if (!joSChainNetworkInfo) return -1;
...
function discoverBlsParticipants(
  joSChainNetworkInfo: discoveryTools.TSChainNetworkInfo,
): number {
  const imaState: state.TIMAState = state.get();
  joSChainNetworkInfo = joSChainNetworkInfo || imaState.joSChainNetworkInfo;
...
function checkBlsThresholdAndBlsParticipants(
  ...
  if (nThreshold <= 0) {
    details.fatal(
      "Operation {} will fail because discovered BLS threshold {}" +
  ...
  if (nParticipants <= 0) {
    details.fatal(
      "Operation {} will fail because discovered BLS number of participants {}" +
        " is invalid number or bad value",
      strOperation,
      nParticipants,
    );
  ...
  if (nThreshold > nParticipants) {
    details.fatal(
      "Operation {} will fail because discovered BLS threshold {} is greater " +
        "than BLS number of participants {}",
      strOperation,
      nThreshold,
      nParticipants,
    );
...
function discoverPublicKeyByIndex(
  ...
  details.fatal(
    "BLS 1/{} public key discovery failed for node #{}, node data is: {}",
    cntNodes,
    nNodeIndex,
    joNode,
  );
...
function discoverCommonPublicKey(
  ...
  joSChainNetworkInfo: discoveryTools.TSChainNetworkInfo,
  isThrowException: boolean,
): discoveryTools.TBLSCommonPublicKey | null {
  const imaState: state.TIMAState = state.get();
  joSChainNetworkInfo = joSChainNetworkInfo || imaState.joSChainNetworkInfo;
  const jarrNodes = joSChainNetworkInfo.network;
  for (let i = 0; i < jarrNodes.length; ++i) {
...
function hexPrepare(
  strHex: string,
  isInvertBefore: boolean,
  isInvertAfter: boolean,
): Uint8Array {
  if (isInvertBefore == undefined) isInvertBefore = true;
  if (isInvertAfter == undefined) isInvertAfter = true;
  let arrBytes = imaUtils.hexToBytes(strHex);
  if (isInvertBefore) arrBytes = arrBytes.reverse();
  arrBytes = imaUtils.bytesAlignLeftWithZeroes(arrBytes, 32);
  if (isInvertAfter) arrBytes = arrBytes.reverse();
  return arrBytes;
}
...
function keccak256Message(
  ...
  for (let i = 0; i < cnt; ++i) {
    ...
    let bytesDestinationContract = imaUtils.hexToBytes(
      joMessage.destinationContract,
    );
    bytesDestinationContract = imaUtils.bytesAlignLeftWithZeroes(
...
export function keccak256U256(u256: any, isHash: boolean): string {
  ...
  } else strMessageHash = "0x" + imaUtils.bytesToHex(arrBytes);
...
function splitSignatureShare(signatureShare: any): any {
  ...
  if (jarr.length < 2)
    throw new Error(
      `Failed to split signatureShare=${signatureShare.toString()}`,
    );
...
function performBlsGlue(
  ...
  details.debug(
    "{p}Discovered number of BLS participants is {}.",
    strLogPrefix,
    nParticipants,
  );
  if (
    !checkBlsThresholdAndBlsParticipants(
      nThreshold,
      nParticipants,
      "BLS glue",
      details,
  ...
  details.trace(
    "{p}{sunny} will work in {} director with {} sign results...",
    strLogPrefix,
    "performBlsGlue",
    strActionDir,
    arrSignResults.length,
  );
  ...
  try {
    ...
    for (let i = 0; i < cnt; ++i) {
      ...
      if (!jo)
        throw new Error(
          `Failed to save BLS part ${i} because it's not JSON object`,
        );
      const strPath = strActionDir + "/sign-result" + jo.index + ".json";
      details.trace(
        "{p}Saving {} file containing {}",
        strLogPrefix,
        strPath,
        jo,
      );
    ...
    const strGlueCommand =
      ...
      "/glue-result.json";
    details.trace(
      "{p}Will execute BLS glue command: {}",
      strLogPrefix,
      strGlueCommand,
    );
    let strOutput = childProcessModule.execSync(strGlueCommand, {
      cwd: strActionDir,
    });
    details.trace(
      "{p}BLS glue output is:\n{raw}",
      strLogPrefix,
      strOutput || "<<EMPTY>>",
    );
    joGlueResult = imaUtils.jsonFileLoad(
      path.join(strActionDir, "glue-result.json"),
    );
    details.trace("{p}BLS glue result is: {}", strLogPrefix, joGlueResult);
    if (
      joGlueResult &&
      "X" in joGlueResult.signature &&
      "Y" in joGlueResult.signature
    ) {
      ...
      const strHasG1Command =
        imaState.strPathHashG1 + " --t " + nThreshold + " --n " + nParticipants;
      details.trace(
        "{p}Will execute HashG1 command {}",
        strLogPrefix,
        strHasG1Command,
      );
      ...
      const joResultHashG1 = imaUtils.jsonFileLoad(
        path.join(strActionDir, "g1.json"),
      );
      details.trace("{p}HashG1 result is: {}", strLogPrefix, joResultHashG1);
      if (
        "g1" in joResultHashG1 &&
        ...
        joGlueResult = null;
        throw new Error(
          `malformed HashG1 result: ${JSON.stringify(joResultHashG1)}`,
        );
      }
    } else {
      ...
      joGlueResult = null;
      throw new Error(
        `malformed BLS glue result: ${JSON.stringify(joSavedGlueResult)}`,
      );
    }
    fnShellRestore();
  } catch (err) {
    details.critical(
      "{p}BLS glue error description is: {err}, stack is: \n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
function performBlsGlueU256(
  ...
  try {
    ...
    strOutput = childProcessModule
      .execSync(strGlueCommand, { cwd: strActionDir })
      .toString("utf8");
    details.trace(
      "{p}BLS glue output is:\n{raw}",
      strLogPrefix,
    ...
    if (
      ...
      strOutput = childProcessModule
        .execSync(strHasG1Command, { cwd: strActionDir })
        .toString("utf8");
      details.trace(
        "{p}HashG1 output is:\n{raw}",
        strLogPrefix,
    ...
    details.critical(
      "BLS glue error description is: {err}, stack is: \n{stack}",
      err,
      err,
    );
...
function performBlsVerifyI(
  ...
  if (!joResultFromNode) return true;
  const imaState: state.TIMAState = state.get();
  if (!imaState.joSChainNetworkInfo)
    throw new Error("No own S-Chain network information");
  ...
  try {
    details.trace(
      "{p}BLS node #{} - first message nonce is {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      nIdxCurrentMsgBlockStart,
    );
    details.trace(
      "{p}BLS node #{} - first source chain name is {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      strFromChainName,
    );
    details.trace(
      "{p}BLS node #{} - messages array {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      jarrMessages,
    );
    ...
    details.trace(
      "{p}BLS node #{} - hashed verify message is {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      strMessageHash,
    );
    ...
    imaUtils.jsonFileSave(
      strActionDir + "/BLS_keys" + nZeroBasedNodeIndex + ".json",
      joPublicKey,
    );
    const strVerifyCommand =
      ...
      nZeroBasedNodeIndex +
      " --input " +
      strSignResultFileName;
    details.trace(
      "{p}Will execute node #{} BLS verify command: {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      strVerifyCommand,
    );
    strOutput = childProcessModule
      .execSync(strVerifyCommand, { cwd: strActionDir })
      .toString("utf8");
    details.trace(
      "{p}BLS node #{} verify output is:\n{raw}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      strOutput || "<<EMPTY>>",
    );
    details.success(
      "{p}BLS node #{} verify success",
      strLogPrefix,
      nZeroBasedNodeIndex,
    );
    fnShellRestore();
    ...
    details.critical(
      "{p}BLS node #{} verify error:, error description is: {err}, " +
        "stack is: \n{stack}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      err,
...
function performBlsVerifyIU256(
  ...
  try {
    ...
    details.debug(
      "{p}BLS u256 node #{} verify message {} composed from {} using glue {} " +
        "and public key {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      joMsg,
    ...
    const strVerifyCommand =
      ...
      strSignResultFileName;
    details.trace(
      "{p}Will execute node #{} BLS u256 verify command: {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      strVerifyCommand,
    );
    strOutput = childProcessModule
      ...
      .toString("utf8");
    details.trace(
      "{p}BLS u256 node #{} verify output is:\n{raw}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      strOutput || "<<EMPTY>>",
    );
    details.success(
      "{p}BLS u256 node #{} verify success",
      strLogPrefix,
      nZeroBasedNodeIndex,
    );
    fnShellRestore();
    ...
    details.error(
      "{p}BLS u256 node #{} verify error, error description is: {err}, " +
        "stack is: \n{stack}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      err,
...
function performBlsVerify(
  ...
  try {
    details.trace(
      "{p}BLS/summary verify message - first message nonce is {}",
      strLogPrefix,
      nIdxCurrentMsgBlockStart,
    );
    details.trace(
      "{p}BLS/summary verify message - first source chain name is {}",
      strLogPrefix,
      strFromChainName,
    );
    details.trace(
      "{p}BLS/summary verify message - messages array {}",
      strLogPrefix,
      jarrMessages,
    );
    const strMessageHash = owaspUtils.removeStarting0x(
    ...
    details.trace(
      "{p}BLS/summary verify message - hashed verify message is {}",
      strLogPrefix,
      strMessageHash,
    );
    const joMsg = { message: strMessageHash };
    ...
    imaUtils.jsonFileSave(
      strActionDir + "/common_public_key.json",
      joCommonPublicKeyToSave,
    );
    details.trace(
      "{p}BLS common public key for verification is:\n{}",
      strLogPrefix,
      joCommonPublicKey,
    );
    const strVerifyCommand =
      ...
      "./glue-result.json";
    details.trace(
      "{p}Will execute BLS/summary verify command: {}",
      strLogPrefix,
      strVerifyCommand,
    );
    strOutput = childProcessModule
      ...
      .toString("utf8");
    details.trace(
      "{p}BLS/summary verify output is:\n{raw}",
      strLogPrefix,
      strOutput || "<<EMPTY>>",
    );
    ...
    details.error(
      "{p}BLS/summary verify error description is: {err}, stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
    );
    details.error(
      "BLS/summary verify output is:\n{raw}",
      strOutput || "<<EMPTY>>",
    );
    fnShellRestore();
  }
...
function performBlsVerifyU256(
  ...
  try {
    ...
    details.trace(
      "{p}BLS u256 common public key for verification is:\n{}",
      strLogPrefix,
      joCommonPublicKey,
    );
    const strVerifyCommand =
      ...
      "./glue-result.json";
    details.trace(
      "{p}Will execute BLS u256/summary verify command: {}",
      strLogPrefix,
      strVerifyCommand,
    );
    strOutput = childProcessModule
      ...
      .toString("utf8");
    details.trace(
      "{p}BLS u256/summary verify output is:\n{raw}",
      strLogPrefix,
      strOutput || "<<EMPTY>>",
    );
    ...
    details.error(
      "{p}BLS u256/summary  error description is: {err}, stack is: \n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
async function checkCorrectnessOfMessagesToSign(
  ...
  if (strDirection == "M2S") {
    ...
    if (!ethersProvider) {
      throw new Error(
        "CRITICAL ERROR: No provider specified in extra signing options " +
    ...
    throw new Error(
      "CRITICAL ERROR: Failed checkCorrectnessOfMessagesToSign() with " +
  ...
  details.debug(
    ...
    strLogPrefix,
    strDirection,
    "verifyOutgoingMessageData",
    joMessageProxy ? joMessageProxy.address : "<NullContract>",
    strCallerAccountAddress,
    jarrMessages.length,
    jarrMessages,
    nIdxCurrentMsgBlockStart,
    joChainName,
  );
  let cntBadMessages = 0;
  let i = 0;
  const cnt = jarrMessages.length;
  if (strDirection == "S2M" || strDirection == "S2S") {
    for (i = 0; i < cnt; ++i) {
      ...
      try {
        ...
        if (!joMessageProxy) throw new Error("No message proxy available");
        const isValidMessage =
          await joMessageProxy.callStatic.verifyOutgoingMessageData(
            outgoingMessageData,
  ...
  if (cntBadMessages > 0) {
    details.critical(
      "{p}Correctness validation failed for {} of {} message(s)",
      strLogPrefix,
      cntBadMessages,
      cnt,
    );
    ...
    details.success(
      "{p}Correctness validation passed for {} message(s)",
      strLogPrefix,
      cnt,
    );
}
...
async function prepareSignMessagesImpl(
  optsSignOperation: TSignOperationOptions,
): Promise<boolean> {
  ...
  optsSignOperation.sequenceId = owaspUtils.removeStarting0x(
    owaspUtils.ethersMod.ethers.utils.id(
      log.generateTimestampString(null, false),
    ),
  );
  ...
  if (
    ...
    optsSignOperation.details.debug(
      ...
      optsSignOperation.jarrMessages.length,
  ...
  optsSignOperation.details.trace(
    "{p}Will sign {} message(s), sequence ID is {}...",
    optsSignOperation.strLogPrefix,
    optsSignOperation.jarrMessages.length,
    optsSignOperation.sequenceId,
  );
  optsSignOperation.details.trace(
    "{p}Will query to sign {} skaled node(s)...",
    optsSignOperation.strLogPrefix,
    optsSignOperation.jarrNodes.length,
  );
  optsSignOperation.nThreshold = discoverBlsThreshold(
    optsSignOperation.imaState.joSChainNetworkInfo,
  );
  optsSignOperation.nParticipants = discoverBlsParticipants(
    optsSignOperation.imaState.joSChainNetworkInfo,
  );
  optsSignOperation.details.trace(
    "{p}Discovered BLS threshold is {}.",
    optsSignOperation.strLogPrefix,
  ...
  optsSignOperation.details.trace(
    "{p}Will BLS-collect {} from {} nodes, sequence ID is {}",
    optsSignOperation.strLogPrefix,
...
async function gatherSigningCheckFinish(
  ...
  if (cntSuccess < optsSignOperation.nCountOfBlsPartsToCollect) return false;
  optsSignOperation.strLogPrefixB =
    `${optsSignOperation.strDirection} /# ` +
    `${optsSignOperation.nTransferLoopCounter}/BLS/Summary: `;
  let strError: string | null = null;
  let strSuccessfulResultDescription: string | null = null;
  ...
  optsSignOperation.details.trace(
    "Will call signed-hash answer-sending callback {}, messages is(are) {}, " +
      "glue result is {}",
    strError ? log.fmtError(" with error {}", strError) : "",
    optsSignOperation.jarrMessages,
    joGlueResult,
  );
  optsSignOperation
    .fn(strError, optsSignOperation.jarrMessages, joGlueResult)
    .catch(function (err: Error | string): void {
      optsSignOperation.details.critical(
        "Problem(2) in BLS sign result handler: {err}",
        err,
      );
      optsSignOperation.errGathering =
        "Problem(2) in BLS sign " +
    ...
    .catch(function (err: Error | string): void {
      ...
      optsSignOperation.details.error(
        "Problem(3) in BLS sign result handler, not enough successful BLS signature " +
          "parts({}) when all attempts done, error details: {err}",
        cntSuccess,
        err,
      );
      optsSignOperation.errGathering =
        "Problem(3) in BLS sign result handler, not enough " +
...
async function gatherSigningStartImpl(
  ...
  optsSignOperation.details.debug(
    "{p}Waiting for BLS glue result...",
    optsSignOperation.strLogPrefix,
  );
  optsSignOperation.errGathering = null;
  for (
    let idxStep = 0;
    idxStep < optsSignOperation.joGatheringTracker.nWaitIntervalMaxSteps;
    ++idxStep
  ) {
    await threadInfo.sleep(
      optsSignOperation.joGatheringTracker.nWaitIntervalStepMilliseconds,
    );
    if (await gatherSigningCheckFinish(optsSignOperation)) return;
    if (await gatherSigningCheckOverflow(optsSignOperation)) return;
  }
  // timeout
  optsSignOperation
    .fn(
    ...
    .catch(function (err: Error | string): void {
      ...
      optsSignOperation.details.critical(
        "Problem(4) in BLS sign result handler, not enough successful BLS signature " +
          "parts({}) and timeout reached, error details: {err}",
        cntSuccess,
        err,
      );
      optsSignOperation.errGathering =
        "Problem(4) in BLS sign result handler, not enough " +
...
async function gatherSigningFinishImpl(
  ...
  if (!optsSignOperation.bHaveResultReportCalled) {
    optsSignOperation.details.error(
      "Failed BLS sign result awaiting(2): {err}",
...
async function doSignConfigureChainAccessParams(
  ...
  if (optsSignOperation.strDirection == "M2S") {
    optsSignOperation.targetChainName = optsSignOperation.imaState
      .chainProperties.sc.strChainName
      ? optsSignOperation.imaState.chainProperties.sc.strChainName
      : "";
    optsSignOperation.fromChainName = optsSignOperation.imaState.chainProperties
      .mn.strChainName
      ? optsSignOperation.imaState.chainProperties.mn.strChainName
      : "";
    optsSignOperation.targetChainID =
      optsSignOperation.imaState.chainProperties.sc.chainId;
    optsSignOperation.fromChainID =
      optsSignOperation.imaState.chainProperties.mn.chainId;
  } else if (optsSignOperation.strDirection == "S2M") {
    optsSignOperation.targetChainName = optsSignOperation.imaState
      .chainProperties.mn.strChainName
      ? optsSignOperation.imaState.chainProperties.mn.strChainName
      : "";
    optsSignOperation.fromChainName = optsSignOperation.imaState.chainProperties
      .sc.strChainName
    ...
    optsSignOperation.fromChainName =
      optsSignOperation.joExtraSignOpts.chainNameSrc;
    optsSignOperation.targetChainID =
      optsSignOperation.joExtraSignOpts.chainIdDst;
    optsSignOperation.fromChainID =
      optsSignOperation.joExtraSignOpts.chainIdSrc;
  } else {
    throw new Error(
      "CRITICAL ERROR: Failed doSignMessagesImpl() with " +
...
async function doSignProcessHandleCall(
  ...
  const strNodeDescColorized = log.fmtDebug(
    "{url} ({}/{}, ID {}), sequence ID is {}",
    strNodeURL,
    i,
    optsSignOperation.jarrNodes.length,
    joNode.nodeID,
  ...
  optsSignOperation.details.debug(
    ...
    optsSignOperation.strLogPrefix,
    joNode.nodeID,
    joOut.result ? joOut.result : null,
  );
  try {
    if (
      joOut.result.signResult.signatureShare.length > 0 &&
      ...
      optsSignOperation.strLogPrefixA = `${optsSignOperation.strDirection}/BLS/#${nZeroBasedNodeIndex}: `;
      try {
        const cntSuccess = optsSignOperation.arrSignResults.length;
        ...
        optsSignOperation.details.trace(
          "{p}Will verify sign result for node {}",
          optsSignOperation.strLogPrefixA,
          nZeroBasedNodeIndex,
        );
        if (!optsSignOperation.imaState.joSChainNetworkInfo)
...
async function doSignProcessOneImpl(
  ...
  const joCall = await rpcCall.create(strNodeURL, rpcCallOpts).catch(function (
    err: Error | string,
  ): void {
    ++optsSignOperation.joGatheringTracker.nCountReceived;
    ++optsSignOperation.joGatheringTracker.nCountErrors;
  ...
  optsSignOperation.details.trace(
    "{p}{} Will invoke {bright} to node #{} via {url} for transfer from chain {} " +
      "to chain {} with params {}, sequence ID is {}",
    optsSignOperation.strLogPrefix,
    log.generateTimestampString(null, true),
    "skale_imaVerifyAndSign",
    i,
...
async function doSignMessagesImpl(
  ...
  try {
    ...
    for (let i = 0; i < optsSignOperation.jarrNodes.length; ++i) {
      ...
      doSignProcessOneImpl(i, optsSignOperation)
        ...
        .catch(function (err): void {
          log.error(
            "Failed single BLS sign processing, reported error is: {err}",
            err,
          );
        });
    }
    ...
    if (optsSignOperation.details) {
      optsSignOperation.details.critical(
        "Failed BLS sign due to generic " +
...
async function prepareSignU256(
  optsSignU256: TSignU256Options,
): Promise<boolean> {
  ...
  if (!optsSignU256.imaState.joSChainNetworkInfo)
    throw new Error("No own S-Chain network information");
  optsSignU256.nThreshold = discoverBlsThreshold(
    optsSignU256.imaState.joSChainNetworkInfo,
  );
  optsSignU256.nParticipants = discoverBlsParticipants(
    optsSignU256.imaState.joSChainNetworkInfo,
  );
  optsSignU256.details.trace(
    "{p}Discovered BLS threshold is {}.",
  ...
  optsSignU256.details.trace(
    "{p}Will(optsSignU256.u256) collect {} from {} nodes",
    optsSignU256.strLogPrefix,
...
async function doSignU256OneImplHandleCallResult(
  ...
  optsSignU256.details.trace(
    "{p}Did invoked {} for to sign value {}, answer is: {}",
    optsSignU256.strLogPrefix,
    "skale_imaBSU256",
    optsSignU256.u256.toString(),
    joOut,
  );
  if (
    !joOut ||
    ...
    !joOut.result ||
    typeof joOut.result !== "object" ||
    !("signature" in joOut.result) ||
    joOut.result.signature != "object"
  ) {
    ++optsSignU256.joGatheringTracker.nCountErrors;
    const strErrorMessage = owaspUtils.extractErrorMessage(
      joOut,
      "unknown wallet error(2)",
    );
    optsSignU256.details.error(
      "{p}S-Chain node {} reported wallet error: {err}",
      optsSignU256.strLogPrefix,
      strNodeDescColorized,
      strErrorMessage,
    );
    ...
    if (
      ...
      try {
        ...
        if (cntSuccess > optsSignU256.nCountOfBlsPartsToCollect) {
          ...
          optsSignU256.details.notice(
            "{p}Will ignore sign result for node {} because {}/{} threshold " +
              "number of BLS signature parts already gathered",
            strLogPrefixA,
            nZeroBasedNodeIndex,
            optsSignU256.nThreshold,
            optsSignU256.nCountOfBlsPartsToCollect,
        ...
        if (
          ...
          optsSignU256.details.error(
            "{p} BLS u256 one node verify failed",
            strLogPrefixA,
          );
        }
      } catch (err) {
      ...
      } else ++optsSignU256.joGatheringTracker.nCountErrors;
    }
  } catch (err) {
    ++optsSignU256.joGatheringTracker.nCountErrors;
    optsSignU256.details.critical(
...
async function doSignU256OneImpl(
  ...
  try {
    ...
    optsSignU256.details.trace(
      "{p}Will invoke skale_imaBSU256 for to sign value {}",
      optsSignU256.strLogPrefix,
      optsSignU256.u256.toString(),
    );
    const joIn: any = {
    ...
    const joOut = await joCall.call(joIn);
    await doSignU256OneImplHandleCallResult(
      i,
      optsSignU256,
      joCall,
      joIn,
    ...
    if (joCall) await joCall.disconnect();
    return true;
  }
}
...
async function gatherSigningCheckFinish256(
  ...
  if (cntSuccess < optsSignU256.nCountOfBlsPartsToCollect) return false;
  const strLogPrefixB = "BLS u256/Summary: ";
  const strError = null;
  const joGlueResult = performBlsGlueU256(
    optsSignU256.details,
    optsSignU256.u256,
    optsSignU256.arrSignResults,
  );
  if (joGlueResult) {
    optsSignU256.details.success(
  ...
  optsSignU256.details.trace(
    "Will call signed-256 answer-sending callback {}, u256 is {}, " +
      "glue result is {}",
    strError ? " with error " + log.fmtError("{err}", strError) : "",
    optsSignU256.u256,
    joGlueResult,
  );
  optsSignU256.fn(strError, optsSignU256.u256, joGlueResult).catch(function (
    err: Error | string,
  ): void {
    optsSignU256.details.critical(
      "Problem(2) in BLS u256 sign result handler: {err}",
      err,
    );
    optsSignU256.errGathering =
      "Problem(2) in BLS u256 sign result " +
...
async function gatherSigningCheckOverflow256(
  ...
  optsSignU256
    ...
    .catch(function (err: Error | string): void {
      ...
      optsSignU256.errGathering =
        "Problem(3) in BLS u256 sign result handler, not " +
...
async function doSignU256Gathering(
  optsSignU256: TSignU256Options,
): Promise<void> {
  optsSignU256.details.debug(
    "{p}Waiting for BLS glue result ",
  ...
  for (
    let idxStep = 0;
    idxStep < optsSignU256.joGatheringTracker.nWaitIntervalMaxSteps;
    ++idxStep
  ) {
    await threadInfo.sleep(
      optsSignU256.joGatheringTracker.nWaitIntervalStepMilliseconds,
    );
    if (await gatherSigningCheckFinish256(optsSignU256)) return;
    if (await gatherSigningCheckOverflow256(optsSignU256)) return;
  }
  // timeout
  optsSignU256
    .fn(
    ...
    .catch(function (err: Error | string): void {
      ...
      optsSignU256.errGathering =
        "Problem(4) in BLS u256 sign result handler, not " +
...
export async function doSignU256(
  ...
  optsSignU256.details.trace(
    "{p}Invoking signing u256 procedure...",
    optsSignU256.strLogPrefix,
  );
  ...
  if (
    ...
    optsSignU256.details.warning(
      "{p}BLS u256 signing is unavailable",
      optsSignU256.strLogPrefix,
    );
    await optsSignU256.fn(
  ...
  if (!(await prepareSignU256(optsSignU256))) return;
  ...
  if (optsSignU256.errGathering) {
    optsSignU256.details.error(
      "Failed BLS u256 sign result awaiting: {err}",
      optsSignU256.errGathering.toString(),
    );
    return;
  }
  optsSignU256.details.information(
    "{p}Completed signing u256 procedure",
    optsSignU256.strLogPrefix,
  );
}
...
export async function doVerifyReadyHash(
  ...
  try {
    ...
    const joMsg = { message: strMessageHash };
    details.debug(
      "{p}BLS node #{} - composed {} using hash {} and glue {} and public key {}",
      strLogPrefix,
      nZeroBasedNodeIndex,
      joMsg,
      strMessageHash,
...
export async function doSignReadyHash(
  ...
  try {
    ...
    } else details.warning("Will sign via SGX without SSL options");
    joCall = await rpcCall.create(joAccount.strSgxURL, rpcCallOpts);
    if (!joCall)
      throw new Error(
        `Failed to create JSON RPC call object to ${joAccount.strSgxURL}`,
      );
    const joIn: any = {
      jsonrpc: "2.0",
    ...
    details.error(
      "{p}JSON RPC call to SGX failed, error is: {err}, stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
async function prepareHandlingOfSkaleImaVerifyAndSign(
  ...
  optsHandleVerifyAndSign.details.debug(
    "{p}Will verify and sign {}",
    optsHandleVerifyAndSign.strLogPrefix,
    optsHandleVerifyAndSign.joCallData,
  );
  ...
  if (!optsHandleVerifyAndSign.imaState.joSChainNetworkInfo)
    throw new Error("No own S-Chain network information");
  optsHandleVerifyAndSign.nThreshold = discoverBlsThreshold(
    optsHandleVerifyAndSign.imaState.joSChainNetworkInfo,
  );
  optsHandleVerifyAndSign.nParticipants = discoverBlsParticipants(
    optsHandleVerifyAndSign.imaState.joSChainNetworkInfo,
  );
  optsHandleVerifyAndSign.details.debug(
  ...
  if (
    ...
    return false;
  optsHandleVerifyAndSign.strMessageHash = owaspUtils.removeStarting0x(
    keccak256Message(
      optsHandleVerifyAndSign.jarrMessages,
      optsHandleVerifyAndSign.nIdxCurrentMsgBlockStart,
      optsHandleVerifyAndSign.strFromChainName,
...
async function prepareS2sOfSkaleImaVerifyAndSign(
  ...
  const strSChainNameSrc =
    optsHandleVerifyAndSign.joCallData.params.srcChainName;
  const strSChainNameDst =
    optsHandleVerifyAndSign.joCallData.params.dstChainName;
  optsHandleVerifyAndSign.details.trace(
  ...
  if (
    joSChainSrc == null ||
    strUrlSrcSChain == null ||
    strUrlSrcSChain.length == 0
  ) {
    throw new Error(
      `Could not handle ${optsHandleVerifyAndSign.strDirection} ` +
...
async function handleBlsSignMessageHashResult(
  ...
  optsHandleVerifyAndSign.details.trace(
    "{p}{bright} Call to SGX done, " + "answer is: {}",
    optsHandleVerifyAndSign.strLogPrefix,
    optsHandleVerifyAndSign.strDirection,
    joOut,
  );
  ...
  if ("qa" in optsHandleVerifyAndSign.joCallData.params)
    optsHandleVerifyAndSign.joRetVal.qa =
...
export async function handleSkaleImaVerifyAndSign(
  ...
  try {
    ...
    optsHandleVerifyAndSign.details.debug(
      "{p}Will BLS-sign verified messages.",
      optsHandleVerifyAndSign.strLogPrefix,
    );
    let joAccount =
      optsHandleVerifyAndSign.imaState.chainProperties.sc.joAccount;
    if (!joAccount.strSgxURL) {
      joAccount = optsHandleVerifyAndSign.imaState.chainProperties.mn.joAccount;
      if (!joAccount.strSgxURL)
        throw new Error(
          "SGX URL is unknown, cannot sign(handle) IMA message(s)",
        );
      if (!joAccount.strBlsKeyName)
    ...
    if (
      ...
      optsHandleVerifyAndSign.details.warning(
        "Will sign via SGX without SSL options",
      );
    joCall = await rpcCall.create(joAccount.strSgxURL, rpcCallOpts);
    if (!joCall)
      throw new Error(
  ...
  optsHandleVerifyAndSign.details.exposeDetailsTo(
    log.globalStream(),
    "IMA messages verifier/signer",
    optsHandleVerifyAndSign.isSuccess,
  );
  optsHandleVerifyAndSign.details.close();
  return optsHandleVerifyAndSign.joRetVal;
}

async function handleSkaleImaBSU256Prepare(
  optsBSU256: TBSU256Options,
): Promise<boolean> {
  optsBSU256.details.debug(
    "{p}Will U256-BLS-sign {}",
    optsBSU256.strLogPrefix,
    optsBSU256.joCallData,
  );
  if (!optsBSU256.imaState.joSChainNetworkInfo)
    throw new Error("No own S-Chain network information");
  optsBSU256.nThreshold = discoverBlsThreshold(
    optsBSU256.imaState.joSChainNetworkInfo,
  );
  optsBSU256.nParticipants = discoverBlsParticipants(
    optsBSU256.imaState.joSChainNetworkInfo,
  );
  optsBSU256.details.trace(
    "{p}Discovered BLS threshold is {}.",
    optsBSU256.strLogPrefix,
  ...
  optsBSU256.details.trace(
    "{p}U256 original value is {}",
    optsBSU256.strLogPrefix,
    optsBSU256.u256,
  );
  optsBSU256.strMessageHash = keccak256U256(optsBSU256.u256, true);
  optsBSU256.details.trace(
    "{p}hash of U256 value to sign is {}",
    optsBSU256.strLogPrefix,
    optsBSU256.strMessageHash,
  );
...
async function handleBlsSignMessageHash256Result(
  ...
  if (
    ...
    const strError =
      "BLS-sign finished with " + `error: ${joSignResult.errorMessage}`;
    optsBSU256.joRetVal.error = strError;
    optsBSU256.details.error(
      "{p}U256/BLS-sign(2) finished with error: {err}",
      optsBSU256.strLogPrefix,
      joSignResult.errorMessage,
    );
    await joCall.disconnect();
...
export async function handleSkaleImaBSU256(
  ...
  let joCall: rpcCall.TRPCCall | null = null;
  try {
    if (!(await handleSkaleImaBSU256Prepare(optsBSU256))) return null;
    if (!optsBSU256.joAccount)
      throw new Error("No account to perform blsSignMessageHash for U256");
    let rpcCallOpts: rpcCall.TRPCCallOpts | null = null;
    if (
      "strPathSslKey" in optsBSU256.joAccount &&
    ...
    } else optsBSU256.details.warning("Will sign via SGX without SSL options");
    joCall = await rpcCall.create(optsBSU256.joAccount.strSgxURL, rpcCallOpts);
    if (!joCall) {
      throw new Error(
        "Failed to create JSON RPC call object " +
    ...
    optsBSU256.details.trace(
      "{p}Will invoke SGX with call data {}",
      optsBSU256.strLogPrefix,
      joIn,
    );
    const joOut = await joCall.call(joIn);
