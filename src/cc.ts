...
export function isStringAlreadyColorized(s?: any): boolean {
  if (s && typeof s === "string" && s.length > 0 && s[0] == "\x1b") return true;
  return false;
}
...
export function yn(flag?: any): string {
  if (!gFlagIsEnabled) return flag ? "true" : "false";
...
export function rainbowPart(s: string, i: number): string {
  if (!gFlagIsEnabled) return s ? s.toString() : JSON.stringify(s);
...
export function rainbow(s?: any): string {
  ...
  let res = "";
  const cnt = s.length;
  for (let i = 0; i < cnt; ++i) res = res + rainbowPart(s[i], i);
  return res;
}
...
function urlObjColorized(objURL?: any): string {
  let strURL = "";
  if (!objURL) return strURL;
  if (
    objURL.protocol &&
    objURL.protocol !== null &&
    objURL.protocol !== undefined
  )
    strURL += yellow(objURL.protocol) + normal("//");
  if (
    objURL.username &&
    objURL.username !== null &&
    objURL.username !== undefined
  ) {
    strURL += magenta(objURL.username);
    if (
      objURL.password &&
      objURL.password !== null &&
      objURL.password !== undefined
    )
      strURL += normal(":") + yellow(objURL.password);
    strURL += normal("@");
  }
  if (objURL.hostname) strURL += magenta(logArgToStringAsIpv4(objURL.hostname));
  if (objURL.port && objURL.port !== null && objURL.port !== undefined)
    strURL += normal(":") + logArgToString(objURL.port);
  if (
    objURL.pathname &&
    objURL.pathname !== null &&
    objURL.pathname !== undefined &&
    objURL.pathname !== "/"
  )
...
export function urlColorized(x?: any): string {
  if (typeof x === "string" || x instanceof String) return urlStrColorized(x);
  ...
  for (i = 0; i < cnt; ++i) {
    ...
    if (typeof arg === "string" || arg instanceof String) {
      ...
      if (objURL != null && objURL != undefined) {
        ...
        if (objURL.protocol) strURL += yellow(objURL.protocol) + normal("//");

        if (objURL.username) {
          strURL += magenta(objURL.username);
          if (objURL.password) strURL += normal(":") + yellow(objURL.password);
        ...
        if (objURL.port) strURL += normal(":") + logArgToString(objURL.port);

        if (objURL.pathname)
          strURL += yellow(replaceAll(objURL.pathname, "/", normal("/")));

        if (objURL.search) strURL += magenta(objURL.search);
...
export const jsonColorizer: any = {
  ...
  censor: (censor: any): any => {
    ...
    return (key: any, value: any) => {
      ...
      if (i >= jsonColorizer.cntCensoredMax) return "[Unknown]";
  ...
  replacerConsole: (
    ...
    let r = pIndent || "";
    if (pKey) r = r + logArgToString(pKey.replace(/[": ]/g, "")) + ": ";

    if (pVal) r = r + logArgToString(pVal);
  ...
  prettyPrintConsole: (obj?: any): any => {
    ...
    obj = JSON.parse(
      JSON.stringify(obj, getCircularReplacerForJsonStringify()),
    );
...
export function syntaxHighlightJSON(
  ...
  return jo.replace(
    ...
    function (match: any): string {
      if (!gFlagIsEnabled) return match;
      let cls = "number";
      if (/^"/.test(match)) {
      ...
      else if (/NaN/.test(match)) cls = "nan";
      else if (/undefined/.test(match)) cls = "undefined";
      else if (
        (typeof match === "string" || match instanceof String) &&
        match.length >= 2 &&
...
function trimLeftUnneededTimestampZeros(s?: any): string {
  while (s.length >= 2) {
    if (s[0] == "0" && s[1] >= "0" && s[1] <= "9") s = s.substring(1);
    else break;
  }
  return s;
}
...
export function getDurationString(tsFrom: number, tsTo: number): string {
  ...
  n = Math.floor(n / 60);
  s = n2s(secs, 2) + s;
  if (n == 0) return trimLeftUnneededTimestampZeros(s);
  s = ":" + s;
  ...
  try {
    ...
    for (i = 0; i < cnt; ++i) {
      ...
      if (s.indexOf("at ") == 0) {
        ...
        if (n >= 0) {
          s =
            error(s.substring(0, n)) +
            normal(":") +
            warning(s.substring(n + 1));
        } else s = error(s);
      }
      arr[i] = s;
    }
