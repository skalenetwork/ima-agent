...
function parseContractArgs(imaState: state.TIMAState, joArg: any): boolean {
    if (joArg.name == "manager-contracts") {
        owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
        imaState.managerContracts = imaUtils.normalizePath(joArg.value);
        return true;
    }
    if (joArg.name == "ima-contracts") {
        owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
        imaState.chainProperties.mn.aliasOrAddress = imaUtils.normalizePath(joArg.value);
        return true;
    }
    if (joArg.name == "ima-schain-contracts") {
        owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
        imaState.chainProperties.sc.aliasOrAddress = imaUtils.normalizePath(joArg.value);
        return true;
    }
    return false;
}
...
export function parse(joExternalHandlers: any, argv?: any[]): number {
    ...
    for (let idxArg = 2; idxArg < cntArgs; ++idxArg) {
        ...
        if (parseCredentialsArgs(imaState, joArg))
            continue;
        if (parseContractArgs(imaState, joArg))
            continue;
...
function commonInitCheckGeneralArgs(): void {
    ...
    ensureHaveValue(
        ...
        imaState.managerContracts, false, isPrintGathered, null, (x: any) => {
            return log.fmtWarning(x);
        });
    ensureHaveValue(
        "Main-net IMA alias or address",
        imaState.chainProperties.mn.aliasOrAddress, false, isPrintGathered, null, (x: any) => {
            return log.fmtWarning(x);
        });
    ensureHaveValue(
        "S-Chain IMA alias or address",
        imaState.chainProperties.sc.aliasOrAddress, false, isPrintGathered, null, (x: any) => {
            return log.fmtWarning(x);
        });
    ensureHaveValue(
        "S<->S Target S-Chain alias or address",
        imaState.chainProperties.tc.aliasOrAddress, false, isPrintGathered, null, (x: any) => {
            return log.fmtWarning(x);
        });
...
function commonInitTransferringArgs(): void {
    ...
    if (imaState.managerContracts !== undefined) {
        ensureHaveValue(
