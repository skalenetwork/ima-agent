...
const gStrVersion = imaUtils
  .fileLoad(path.join(__dirname, "../VERSION"), "N/A")
  .toString()
  .trim();

function att(...args: any[]): string {
  return log.fmtAttention(...args);
}

export function printAbout(isLog?: boolean): boolean {
  isLog = isLog ?? false;
  const strMsg = log.fmtTrace(
    att(gStrAppName),
    " version ",
    log.fmtNotice(gStrVersion),
  );
  if (isLog) log.information(strMsg);
  else console.log(strMsg);
  return true;
}
...
export function ensureHaveValue(
  name: string,
  value: any,
  isExitIfEmpty: boolean,
  isPrintValue: boolean,
  fnNameColorizer?: any,
  fnValueColorizer?: any,
): boolean {
  ...
  if (value.length === 0) {
    ...
    if (!isPrintValue)
      console.log(
        log.fmtError("WARNING:, missing value for ", fnNameColorizer(name)),
      );
    if (isExitIfEmpty) process.exit(126);
  }
  let strDots = "...";
...
export function ensureHaveCredentials(
  ...
  if (!(typeof joAccount === "object")) {
    log.error(
      "ARGUMENTS VALIDATION WARNING: bad account specified for {} chain",
  ...
  if (
    "strTransactionManagerURL" in joAccount &&
  ...
  if (cntAccountVariantsSpecified == 0) {
    log.error(
      "ARGUMENTS VALIDATION WARNING: bad credentials information " +
...
export function findNodeIndex(joSChainNodeConfiguration: any): number {
  try {
    ...
    for (let i = 0; i < cnt; ++i) {
      const joNodeDescription =
        joSChainNodeConfiguration.skaleConfig.sChain.nodes[i];
      if (joNodeDescription.nodeID == searchID) return i;
    }
  } catch (err) {}
  return 0;
}

function parseHelp(imaState: state.TIMAState, joArg: any): boolean {
  // exits process on "--help"
  if (joArg.name != "help") return false;
  printAbout();
  const strAboutText = imaUtils
    .fileLoad(path.join(__dirname, "../about.txt"), "N/A")
    .toString();
...
function parseVersion(imaState: state.TIMAState, joArg: any): boolean {
  ...
  if (joArg.name != "version") return false;
  printAbout();
  process.exit(0);
}
...
function parseTransactionManagerArgs(
  ...
  if (joArg.name == "tm-priority-main-net") {
    owaspUtils.verifyArgumentIsInteger(joArg);
    imaState.chainProperties.mn.joAccount.nTmPriority = owaspUtils.toInteger(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "tm-priority-s-chain") {
    owaspUtils.verifyArgumentIsInteger(joArg);
    imaState.chainProperties.sc.joAccount.nTmPriority = owaspUtils.toInteger(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "tm-priority-t-chain") {
    owaspUtils.verifyArgumentIsInteger(joArg);
    imaState.chainProperties.tc.joAccount.nTmPriority = owaspUtils.toInteger(
      joArg.value,
    );
    return true;
  }
...
function parseAbiArgs(imaState: state.TIMAState, joArg: any): boolean {
  ...
  if (joArg.name == "abi-main-net") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.mn.strPathAbiJson = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "abi-s-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.sc.strPathAbiJson = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "abi-t-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.tc.strPathAbiJson = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
...
function parseErcArgs(imaState: state.TIMAState, joArg: any): boolean {
  if (joArg.name == "erc20-main-net") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.mn.strPathJsonErc20 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "erc20-s-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.sc.strPathJsonErc20 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  ...
  if (joArg.name == "erc20-t-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.tc.strPathJsonErc20 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  ...
  if (joArg.name == "erc721-main-net") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.mn.strPathJsonErc721 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "erc721-s-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.sc.strPathJsonErc721 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  ...
  if (joArg.name == "erc721-t-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.tc.strPathJsonErc721 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  ...
  if (joArg.name == "erc1155-main-net") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.mn.strPathJsonErc1155 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  if (joArg.name == "erc1155-s-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.sc.strPathJsonErc1155 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
  ...
  if (joArg.name == "erc1155-t-chain") {
    owaspUtils.verifyArgumentIsPathToExistingFile(joArg);
    imaState.chainProperties.tc.strPathJsonErc1155 = imaUtils.normalizePath(
      joArg.value,
    );
    return true;
  }
...
function parseTransactionArgs(imaState: state.TIMAState, joArg: any): boolean {
  ...
  if (joArg.name == "gas-price-multiplier-mn") {
    ...
    if (gasPriceMultiplier < 0.0) gasPriceMultiplier = 0.0;
  ...
  if (joArg.name == "gas-multiplier-mn") {
    ...
    if (gasMultiplier < 0.0) gasMultiplier = 0.0;
...
function parsePaymentAmountArgs(
  imaState: state.TIMAState,
  joArg: any,
): boolean {
  if (joArg.name == "value") {
    owaspUtils.verifyArgumentWithNonEmptyValue(joArg);
    imaState.nAmountOfWei = owaspUtils.parseMoneySpecToWei(
      joArg.value.toString(),
      true,
    );
    return true;
  }
  ...
  if (joArg.name == "amounts") {
    imaState.arrAmountsOfTokens =
      owaspUtils.verifyArgumentIsArrayOfIntegers(joArg);
    return true;
  }
  if (joArg.name == "tids") {
...
function parsePendingWorkAnalysisArgs(
  imaState: state.TIMAState,
  joArg: any,
): boolean {
  if (joArg.name == "pwa") {
...
function parseReimbursementArgs(
  imaState: state.TIMAState,
  joArg: any,
): boolean {
  ...
  if (joArg.name == "reimbursement-recharge") {
    owaspUtils.verifyArgumentWithNonEmptyValue(joArg);
    imaState.nReimbursementRecharge = owaspUtils.parseMoneySpecToWei(
      joArg.value.toString(),
      true,
    );
    return true;
  ...
  if (joArg.name == "reimbursement-withdraw") {
    owaspUtils.verifyArgumentWithNonEmptyValue(joArg);
    imaState.nReimbursementWithdraw = owaspUtils.parseMoneySpecToWei(
      joArg.value.toString(),
      true,
    );
    return true;
...
function parseNetworkDiscoveryArgs(
  imaState: state.TIMAState,
  joArg: any,
): boolean {
...
function parseBlockScannerArgs(imaState: state.TIMAState, joArg: any): boolean {
  ...
  if (joArg.name == "bs-max-all-range") {
    owaspUtils.verifyArgumentIsInteger(joArg);
    imaHelperAPIs.setMaxIterationsInAllRangeEventsScan(
      owaspUtils.toInteger(joArg.value),
    );
    return true;
  }
...
function parseJsonRpcServerArgs(
  imaState: state.TIMAState,
  joArg: any,
): boolean {
...
function parseCrossImaCommunicationArgs(
  imaState: state.TIMAState,
  joArg: any,
): boolean {
...
export function parse(joExternalHandlers: any, argv?: any[]): number {
  ...
  for (let idxArg = 2; idxArg < cntArgs; ++idxArg) {
    ...
    if (parseBasicArgs(imaState, joArg)) continue;
    if (parseChainAccessArgs(imaState, joArg)) continue;
    if (parseTransactionManagerArgs(imaState, joArg)) continue;
    if (parseSgxArgs(imaState, joArg)) continue;
    if (parseCredentialsArgs(imaState, joArg)) continue;
    if (parseAbiArgs(imaState, joArg)) continue;
    if (parseErcArgs(imaState, joArg)) continue;
    if (parseTransactionArgs(imaState, joArg)) continue;
    if (parsePaymentAmountArgs(imaState, joArg)) continue;
    if (parseTransferArgs(imaState, joArg)) continue;
    if (parseMulticallArgs(imaState, joArg)) continue;
    if (parsePendingWorkAnalysisArgs(imaState, joArg)) continue;
    if (parseLoggingArgs(imaState, joArg)) continue;
    if (parseBlsArgs(imaState, joArg)) continue;
    if (parseMonitoringArgs(imaState, joArg)) continue;
    if (parseBlockScannerArgs(imaState, joArg)) continue;
    if (parseReimbursementArgs(imaState, joArg)) continue;
    if (parseOracleArgs(imaState, joArg)) continue;
    if (parseNetworkDiscoveryArgs(imaState, joArg)) continue;
    if (parseBlockScannerArgs(imaState, joArg)) continue;
    if (parseJsonRpcServerArgs(imaState, joArg)) continue;
    if (parseCrossImaCommunicationArgs(imaState, joArg)) continue;
    if (parseShowConfigArgs(imaState, joArg)) continue;
    if (parseOtherArgs(imaState, joArg)) continue;
    if (
      joArg.name == "register" ||
    ...
    console.log(
      log.fmtFatal(
        "COMMAND LINE PARSER ERROR: unknown command line argument {}",
        joArg.name,
      ),
    );
    return 666;
...
async function asyncCheckUrlAtStartup(
  ...
  try {
    details.debug(
      "Will check URL {url} connectivity for {} at start-up...",
      u,
      name,
    );
    const isLog = false;
    ...
    if (isOnLine) {
      details.success(
        "Done, start-up checking URL {url} connectivity for {}, " +
          "URL is on-line.",
        u,
        name,
      );
    ...
    details.error(
      "Failed to check URL {url} connectivity for {} at start-up, " +
...
function commonInitCheckAbiPaths(): void {
  ...
  if (
    ...
    imaState.joAbiSkaleManager = imaUtils.jsonFileLoad(
      imaState.strPathAbiJsonSkaleManager,
      null,
    );
    ...
    log.warning(
      "WARNING: No Skale Manager ABI file path is provided in command line " +
  ...
  if (
    ...
    imaState.chainProperties.mn.joAbiIMA = imaUtils.jsonFileLoad(
      imaState.chainProperties.mn.strPathAbiJson,
      null,
    );
    ...
    log.warning(
      "WARNING: No Main-net IMA ABI file path is provided in command line " +
        "arguments(needed for particular operations only)",
    );
  }
  ...
  if (
    ...
    imaState.chainProperties.sc.joAbiIMA = imaUtils.jsonFileLoad(
      imaState.chainProperties.sc.strPathAbiJson,
      null,
    );
    ...
    log.warning(
      "WARNING: No S-Chain IMA ABI file path is provided in command line arguments" +
  ...
  if (
    ...
    imaState.chainProperties.tc.joAbiIMA = imaUtils.jsonFileLoad(
      imaState.chainProperties.tc.strPathAbiJson,
      null,
    );
    ...
    log.warning(
      "WARNING: No S<->S Target S-Chain IMA ABI file path is provided " +
...
function commonInitCheckContractPresences(): void {
  ...
  if (imaState.bHaveSkaleManagerABI) {
    ...
    log.warning(
      "WARNING: Missing Skale Manager ABI path for S-Chain to S-Chain transfers",
    );
...
function commonInitPrintFoundContracts(): void {
  ...
  const oct = function (
    joContract?: owaspUtils.ethersMod.ethers.Contract | null,
  ): string {
...
function commonInitCheckErc20(): void {
  ...
  if (imaState.chainProperties.mn.strPathJsonErc20.length > 0) {
    if (isPrintGathered) {
      log.information(
        "Loading Main-net ERC20 ABI from {}",
        imaState.chainProperties.mn.strPathJsonErc20,
      );
    }
    imaState.chainProperties.mn.joErc20 = imaUtils.jsonFileLoad(
      imaState.chainProperties.mn.strPathJsonErc20,
      null,
    );
    ...
    if (imaState.chainProperties.sc.strPathJsonErc20.length > 0) {
      if (isPrintGathered) {
        log.information(
          "Loading S-Chain ERC20 ABI from {}",
          imaState.chainProperties.sc.strPathJsonErc20,
        );
      }
      imaState.chainProperties.sc.joErc20 = imaUtils.jsonFileLoad(
        imaState.chainProperties.sc.strPathJsonErc20,
        null,
      );
    ...
    if (n1 > 0) {
      ...
      if (n2 > 0) n2 = imaState.chainProperties.sc.strCoinNameErc20.length;
      if (n1 > 0) {
      ...
      if (n1 === 0) log.error("Main-net ERC20 JSON is invalid");
      if (n2 === 0 && imaState.chainProperties.sc.strPathJsonErc20.length > 0)
    ...
    if (imaState.chainProperties.sc.strPathJsonErc20.length > 0) {
      ...
      if (n2 > 0) {
        ...
        if (n2 > 0) {
          ...
          if (
            n2 === 0 &&
            imaState.chainProperties.sc.strPathJsonErc20.length > 0
          )
            log.error(
              "S-Chain ERC20 token name is not discovered(malformed JSON)",
            );
  ...
  if (n1 !== 0 && n2 === 0) {
    if (imaState.strAddrErc20Explicit.length === 0) {
      log.warning(
        "IMPORTANT NOTICE: Both S-Chain ERC20 JSON and explicit " +
          "ERC20 address are not specified",
      );
    } else {
      if (isPrintGathered)
        log.attention(
          "IMPORTANT NOTICE: S-Chain ERC20 ABI will be auto-generated",
        );
      imaState.chainProperties.sc.strCoinNameErc20 =
        imaState.chainProperties.tc.strCoinNameErc20 ?? ""; // assume same
      imaState.chainProperties.sc.joErc20 = JSON.parse(
        JSON.stringify(imaState.chainProperties.mn.joErc20),
      ); // clone
  ...
  if (imaState.chainProperties.tc.strPathJsonErc20.length > 0) {
    if (isPrintGathered) {
      log.information(
        "Loading S<->S Target S-Chain ERC20 ABI from {}",
        imaState.chainProperties.tc.strPathJsonErc20,
      );
    }
    imaState.chainProperties.tc.joErc20 = imaUtils.jsonFileLoad(
      imaState.chainProperties.tc.strPathJsonErc20,
      null,
    );
    ...
    if (n2 > 0) {
      ...
      if (n2 > 0) {
        ...
        if (
          n2 === 0 &&
          imaState.chainProperties.tc.strPathJsonErc20.length > 0
        ) {
          log.fatal(
  ...
  if (
    ...
    log.warning(
      "IMPORTANT NOTICE: Both S<->S Target S-Chain ERC20 JSON and explicit " +
        "ERC20 address are not specified",
    );
  }
}
...
function commonInitCheckErc721(): void {
  ...
  if (imaState.chainProperties.mn.strPathJsonErc721.length > 0) {
    if (isPrintGathered) {
      log.information(
        "Loading Main-net ERC721 ABI from {}",
        imaState.chainProperties.mn.strPathJsonErc721,
      );
    }
    imaState.chainProperties.mn.joErc721 = imaUtils.jsonFileLoad(
      imaState.chainProperties.mn.strPathJsonErc721,
      null,
    );
    ...
    if (imaState.chainProperties.sc.strPathJsonErc721.length > 0) {
      if (isPrintGathered) {
        log.information(
          "Loading S-Chain ERC721 ABI from {}",
          imaState.chainProperties.sc.strPathJsonErc721,
        );
      }
      imaState.chainProperties.sc.joErc721 = imaUtils.jsonFileLoad(
        imaState.chainProperties.sc.strPathJsonErc721,
        null,
      );
    ...
    if (n1 > 0) {
      ...
      if (n2 > 0) n2 = imaState.chainProperties.sc.strCoinNameErc721.length;
      if (n1 > 0) {
        ...
        if (
          n2 === 0 &&
          imaState.chainProperties.sc.strPathJsonErc721.length > 0
        )
          log.fatal(
      ...
      if (n1 === 0) log.fatal("Main-net ERC721 JSON is invalid");
        ...
        if (n2 > 0) {
          if (isPrintGathered) {
            ...
            if (
              ...
              log.fatal(
                "S-Chain ERC721 token name is not " +
  ...
  if (n1 !== 0 && n2 === 0) {
    if (imaState.strAddrErc721Explicit.length === 0) {
      if (isPrintGathered) {
        log.warning(
          "IMPORTANT NOTICE: Both S-Chain ERC721 JSON and explicit " +
      ...
      if (isPrintGathered)
        log.attention(
          "IMPORTANT NOTICE: S-Chain ERC721 ABI will be auto-generated",
        );
      imaState.chainProperties.sc.strCoinNameErc721 =
        imaState.chainProperties.mn.strCoinNameErc721 ?? ""; // assume same
      imaState.chainProperties.sc.joErc721 = JSON.parse(
        JSON.stringify(imaState.chainProperties.mn.joErc721),
      ); // clone
  ...
  if (
    ...
    log.information(
      "Loading S<->S Target S-Chain ERC721 ABI from {}",
      imaState.chainProperties.tc.strPathJsonErc721,
    );
    imaState.chainProperties.tc.joErc721 = imaUtils.jsonFileLoad(
      imaState.chainProperties.tc.strPathJsonErc721,
      null,
    );
...
function commonInitCheckErc1155(): void {
  ...
  if (imaState.chainProperties.mn.strPathJsonErc1155.length > 0) {
    if (isPrintGathered) {
      log.information(
        "Loading Main-net ERC1155 ABI from {}",
        imaState.chainProperties.mn.strPathJsonErc1155,
      );
    }
    imaState.chainProperties.mn.joErc1155 = imaUtils.jsonFileLoad(
      imaState.chainProperties.mn.strPathJsonErc1155,
      null,
    );
    ...
    if (imaState.chainProperties.sc.strPathJsonErc1155.length > 0) {
      if (isPrintGathered) {
        log.information(
          "Loading S-Chain ERC1155 ABI from {}",
          imaState.chainProperties.sc.strPathJsonErc1155,
        );
      }
      imaState.chainProperties.sc.joErc1155 = imaUtils.jsonFileLoad(
        imaState.chainProperties.sc.strPathJsonErc1155,
        null,
      );
    ...
    if (n1 > 0) {
      ...
      if (n2 > 0) {
        imaState.chainProperties.sc.strCoinNameErc1155 =
          imaUtils.discoverCoinNameInJSON(
            imaState.chainProperties.sc.joErc1155,
          );
      }
      n1 = imaState.chainProperties.mn.strCoinNameErc1155.length;
      if (n2 > 0) n2 = imaState.chainProperties.sc.strCoinNameErc1155.length;
      if (n1 > 0) {
        if (!imaState.bShowConfigMode) {
          if (isPrintGathered) {
            log.information(
              "Loaded Main-net ERC1155 ABI {}",
              imaState.chainProperties.mn.strCoinNameErc1155,
            );
          }
          if (n2 > 0 && isPrintGathered) {
            log.information(
              "Loaded S-Chain ERC1155 ABI {}",
              imaState.chainProperties.sc.strCoinNameErc1155,
            );
          }
        }
        ...
        if (
          n2 === 0 &&
          imaState.chainProperties.sc.strPathJsonErc1155.length > 0
        )
          log.fatal(
      ...
      if (n1 === 0) log.fatal("Main-net ERC1155 JSON is invalid");
  ...
  if (n1 !== 0 && n2 === 0) {
    if (imaState.strAddrErc1155Explicit.length === 0) {
      if (isPrintGathered) {
        log.warning(
          "IMPORTANT NOTICE: Both S-Chain ERC1155 JSON and " +
      ...
      if (isPrintGathered)
        log.attention(
          "IMPORTANT NOTICE: S-Chain ERC1155 ABI will be auto-generated",
        );
      imaState.chainProperties.sc.strCoinNameErc1155 =
        imaState.chainProperties.mn.strCoinNameErc1155 ?? ""; // assume same
      imaState.chainProperties.sc.joErc1155 = JSON.parse(
        JSON.stringify(imaState.chainProperties.mn.joErc1155),
      ); // clone
      imaState.chainProperties.sc.joErc1155[
  ...
  if (imaState.chainProperties.tc.strPathJsonErc1155.length > 0) {
    if (isPrintGathered) {
      log.information(
        "Loading S<->S Target S-Chain ERC1155 ABI from {}",
        imaState.chainProperties.tc.strPathJsonErc1155,
      );
    }
    imaState.chainProperties.tc.joErc1155 = imaUtils.jsonFileLoad(
      imaState.chainProperties.tc.strPathJsonErc1155,
      null,
    );
...
function commonInitCheckGeneralArgs(): void {
  ...
  if (isPrintGathered) {
    printAbout(true);
    log.information(
      "IMA AGENT is using Ethers JS version ",
      att(
...
function commonInitTransferringArgs(): void {
  ...
  ensureHaveValue(
    "Transfer loop period(seconds)",
    imaState.nLoopPeriodSeconds,
    false,
    isPrintGathered,
    null,
    (x: any) => {
  ...
  if (imaState.nTimeFrameSeconds > 0) {
    ensureHaveValue(
      "Time framing(seconds)",
      imaState.nTimeFrameSeconds,
      false,
      isPrintGathered,
    );
    ensureHaveValue(
      "Next frame gap(seconds)",
      imaState.nNextFrameGap,
      false,
      isPrintGathered,
    );
  } else {
...
function commonInitErcTokensArgs(): void {
  ...
  if (imaState.chainProperties.tc.strCoinNameErc20.length > 0) {
    ...
    if (isPrintGathered) {
      log.information(
        "ERC20 explicit S-Chain address is {}",
  ...
  if (imaState.chainProperties.mn.strCoinNameErc721.length > 0) {
    ...
    if (isPrintGathered) {
      log.information(
        "ERC721 explicit S-Chain address is {}",
  ...
  if (imaState.chainProperties.mn.strCoinNameErc1155.length > 0) {
    ...
    if (isPrintGathered) {
      log.information(
        "ERC1155 explicit S-Chain address is {}",
...
function commonInitGasMultipliersAndTransactionArgs(): void {
  ...
  if (isPrintGathered) {
    log.debug(
      log.fmtInformation("Main Net Gas Price Multiplier is"),
      "....................." +
        (imaState.chainProperties.mn.transactionCustomizer.gasPriceMultiplier
          ? log.fmtInformation(
              imaState.chainProperties.mn.transactionCustomizer.gasPriceMultiplier.toString(),
            )
          : log.fmtError("disabled")),
    );
    log.debug(
      log.fmtInformation("S-Chain Gas Price Multiplier is"),
      "......................" +
        (imaState.chainProperties.sc.transactionCustomizer.gasPriceMultiplier
          ? log.fmtInformation(
              imaState.chainProperties.sc.transactionCustomizer.gasPriceMultiplier.toString(),
            )
          : log.fmtError("disabled")),
    );
    log.debug(
      log.fmtInformation("Target S-Chain Gas Price Multiplier is"),
      "..............." +
        (imaState.chainProperties.tc.transactionCustomizer.gasPriceMultiplier
          ? log.fmtInformation(
              imaState.chainProperties.tc.transactionCustomizer.gasPriceMultiplier.toString(),
            )
          : log.fmtError("disabled")),
    );
    log.debug(
      log.fmtInformation("Main Net Gas Value Multiplier is"),
      "....................." +
        (imaState.chainProperties.mn.transactionCustomizer.gasMultiplier
          ? log.fmtInformation(
              imaState.chainProperties.mn.transactionCustomizer.gasMultiplier.toString(),
            )
          : log.fmtNotice("default")),
    );
    log.debug(
      log.fmtInformation("S-Chain Gas Value Multiplier is"),
      "......................" +
        (imaState.chainProperties.sc.transactionCustomizer.gasMultiplier
          ? log.fmtInformation(
              imaState.chainProperties.sc.transactionCustomizer.gasMultiplier.toString(),
            )
          : log.fmtNotice("default")),
    );
    log.debug(
      log.fmtInformation("Target S-Chain Gas Value Multiplier is"),
      "..............." +
        (imaState.chainProperties.tc.transactionCustomizer.gasMultiplier
          ? log.fmtInformation(
              imaState.chainProperties.tc.transactionCustomizer.gasMultiplier.toString(),
            )
          : log.fmtNotice("default")),
    );
    log.debug(
      log.fmtInformation("Pending work analysis(PWA) is"),
      "........................" +
        (imaState.isPWA ? log.fmtSuccess("enabled") : log.fmtError("disabled")),
    );
    log.debug(
      log.fmtInformation("Expose PWA details to log is"),
      "........................." +
        (imaState.isPrintPWA
    ...
    log.debug(
      log.fmtInformation("Oracle based gas reimbursement is"),
      "...................." +
    ...
    log.debug(
      log.fmtInformation("S-Chain to S-Chain transferring is") +
        "..................." +
    ...
    log.debug(
      log.fmtInformation("SKALE network browser file path is"),
      "................." +
    ...
    log.debug(
      log.fmtInformation("S<->S transfer mode is"),
      "..............................." +
        imaHelperAPIs.getS2STransferModeDescriptionColorized(),
    );
    log.debug(
      log.fmtInformation("IMA JSON RPC server port is"),
      ".........................." +
    ...
    log.debug(
      log.fmtInformation("Cross-IMA mode is"),
      "...................................." +
    ...
    log.debug(
      log.fmtInformation("Dry-run is enabled"),
      "..................................." + log.yn(imaTx.dryRunIsEnabled()),
    );
    log.debug(
      log.fmtInformation("Dry-run execution result is ignored") +
        ".................." +
...
export function imaInitEthersProviders(): void {
  ...
  if (
    ...
    log.warning(
      "No Main-net URL specified in command line arguments" +
        "(needed for particular operations only)",
    );
  }
  ...
  if (
    ...
    log.warning(
      "No S-Chain URL specified in command line arguments" +
        "(needed for particular operations only)",
    );
  }
  ...
  if (
    ...
    log.warning(
      "No S<->S Target S-Chain URL specified in command line arguments" +
        "(needed for particular operations only)",
    );
  }
...
function initContractsIMA(): void {
  ...
  if (imaState.chainProperties.mn.bHaveAbiIMA) {
    ...
    if (cp.ethersProvider) {
      const ep: owaspUtils.ethersMod.ethers.providers.JsonRpcProvider =
        cp.ethersProvider;
      ...
      imaState.joLinker = new owaspUtils.ethersMod.ethers.Contract(
        joABI.linker_address,
        joABI.linker_abi,
        ep,
      ); // only main net
      imaState.joMessageProxyMainNet = new owaspUtils.ethersMod.ethers.Contract(
        joABI.message_proxy_mainnet_address,
        joABI.message_proxy_mainnet_abi,
        ep,
      );
    }
  }
