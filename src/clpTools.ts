...
export async function registerAll(
  isPrintSummaryRegistrationCosts: boolean,
): Promise<boolean> {
  if (!(await registerStep1(false))) return false;
  if (isPrintSummaryRegistrationCosts) printSummaryRegistrationCosts(log);
  return true;
}
...
export async function registerStep1(
  ...
  const jarrReceipts: any = await imaReg.registerSChainInDepositBoxes(
    // step 1
    imaState.chainProperties.mn.ethersProvider,
    imaState.joLinker,
    imaState.chainProperties.mn.joAccount,
  ...
  if (bSuccess) {
    gInfoRegistrationCost.mn = gInfoRegistrationCost.mn.concat(
      gInfoRegistrationCost.mn,
      jarrReceipts,
    );
  }
  if (isPrintSummaryRegistrationCosts) printSummaryRegistrationCosts();
  if (!bSuccess) {
    const nRetCode = 163;
    log.fatal(
      "{p}failed to register S-Chain in deposit box, will return code {}",
      strLogPrefix,
      nRetCode,
    );
    ...
    fn: async function (): Promise<boolean> {
      ...
      if (imaState.chainProperties.tc.strCoinNameErc20.length > 0) {
        try {
          ...
          bMintIsOK = !!(await imaToken.mintErc20(
            ...
            imaState.chainProperties.tc.joErc20[
              imaState.chainProperties.tc.strCoinNameErc20 + "_abi"
            ],
        ...
        try {
          ...
          const idTokens: any[] =
            imaState.haveArrayOfTokenIdentifiers && imaState.idTokens
              ? imaState.idTokens
              : [];
          if (imaState.haveOneTokenIdentifier) idTokens.push(imaState.idToken);
          if (idTokens.length > 0) {
            for (let i = 0; i < idTokens.length; ++i) {
              const idToken = idTokens[i];
              if (!imaState.chainProperties.tc.ethersProvider)
                throw new Error("No provider for target chain");
              bMintIsOK = !!(await imaToken.mintErc721(
                imaState.chainProperties.tc.ethersProvider,
                imaState.chainProperties.tc.chainId.toString(),
                imaState.chainProperties.tc.strChainName,
                imaState.chainProperties.tc.joAccount,
                ...
                imaState.chainProperties.tc.joErc721[
                  imaState.chainProperties.tc.strCoinNameErc721 + "_abi"
                ],
                imaState.chainProperties.tc.transactionCustomizer,
              ));
            }
        ...
        try {
          ...
          if (idTokens.length > 0) {
            for (let i = 0; i < idTokens.length; ++i) {
              ...
              if (!imaState.chainProperties.tc.ethersProvider)
                throw new Error("No provider for target chain");
              bMintIsOK = !!(await imaToken.mintErc1155(
                imaState.chainProperties.tc.ethersProvider,
                imaState.chainProperties.tc.chainId.toString(),
                imaState.chainProperties.tc.strChainName,
                imaState.chainProperties.tc.joAccount,
                ...
                imaState.chainProperties.tc.joErc1155[
                  imaState.chainProperties.tc.strCoinNameErc1155 + "_abi"
                ],
                imaState.chainProperties.tc.transactionCustomizer,
              ));
            }
    ...
    fn: async function (): Promise<boolean> {
      ...
      if (imaState.chainProperties.tc.strCoinNameErc20.length > 0) {
        try {
          ...
          if (!imaState.chainProperties.tc.ethersProvider)
            throw new Error("No provider for target chain");
          bBurnIsOK = !!(await imaToken.burnErc20(
            imaState.chainProperties.tc.ethersProvider,
            imaState.chainProperties.tc.chainId.toString(),
            imaState.chainProperties.tc.strChainName,
            imaState.chainProperties.tc.joAccount,
        ...
        try {
          ...
          if (idTokens.length > 0) {
            for (let i = 0; i < idTokens.length; ++i) {
              ...
              if (!imaState.chainProperties.tc.ethersProvider)
                throw new Error("No provider for target chain");
              bBurnIsOK = !!(await imaToken.burnErc721(
                imaState.chainProperties.tc.ethersProvider,
                imaState.chainProperties.tc.chainId.toString(),
                imaState.chainProperties.tc.strChainName,
                imaState.chainProperties.tc.joAccount,
        ...
        try {
          ...
          if (idTokens.length > 0) {
            for (let i = 0; i < idTokens.length; ++i) {
              ...
              if (!imaState.chainProperties.tc.ethersProvider)
                throw new Error("No provider for target chain");
              bBurnIsOK = !!(await imaToken.burnErc1155(
                imaState.chainProperties.tc.ethersProvider,
                imaState.chainProperties.tc.chainId.toString(),
                imaState.chainProperties.tc.strChainName,
                imaState.chainProperties.tc.joAccount,
...
export async function commandLineTaskShowBalanceEth(
  ...
  if (imaState.chainProperties.mn.ethersProvider) {
    arrBalancesMN.push({
      assetName: "RealETH",
      balance: await imaEth.getBalanceEth(
        true, // isMainNet
        imaState.chainProperties.mn.ethersProvider,
        imaState.chainProperties.mn.chainId.toString(),
        imaState.chainProperties.mn.joAccount,
    ...
    if (!imaState.joDepositBoxETH) throw new Error("No DepositBoxETH contract");
    arrBalancesMN.push({
  ...
  if (imaState.chainProperties.sc.ethersProvider) {
    ...
    arrBalancesSC.push({
      ...
      balance: await imaEth.getBalanceEth(
        true, // isMainNet=true here, but we call S-Chain
        imaState.chainProperties.sc.ethersProvider,
        imaState.chainProperties.sc.chainId.toString(),
        imaState.chainProperties.sc.joAccount,
        null,
  ...
  if (
    ...
    for (let i = 0; i < idTokens.length; ++i) {
      ...
      arrBalancesMN.push({
        ...
        assetAddress,
        idToken,
        owner: await imaToken.getOwnerOfErc721(
          true, // isMainNet
          imaState.chainProperties.mn.ethersProvider,
          imaState.chainProperties.mn.chainId.toString(),
          imaState.chainProperties.mn.joAccount,
  ...
  if (
    ...
    for (let i = 0; i < idTokens.length; ++i) {
      ...
      arrBalancesMN.push({
        ...
        assetAddress,
        idToken,
        balance: await imaToken.getBalanceErc1155(
          true, // isMainNet
          imaState.chainProperties.mn.ethersProvider,
          imaState.chainProperties.mn.chainId.toString(),
          imaState.chainProperties.mn.joAccount,
...
export function commandLineTaskShowBalance(): void {
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      ...
      const arrBalancesSC: any = [];
      const arrBalancesTC: any = [];
      ...
      if (
        arrBalancesMN.length > 0 ||
        arrBalancesSC.length > 0 ||
        arrBalancesTC.length > 0
      ) {
      ...
      } else log.warning("No balances to scan.");
      return true;
    },
  });
}
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      ...
      if (
        ...
        // ERC1155 payment
        log.information(
          "one M->S single ERC1155 payment: {} {}",
          imaState.idToken,
          imaState.nAmountOfToken,
        );
        if (!imaState.chainProperties.mn.ethersProvider)
      ...
      if (
        ...
        log.information(
          "one M->S single ERC1155 Batch payment: {} {}",
          imaState.idTokens,
          imaState.arrAmountsOfTokens,
        );
        if (!imaState.chainProperties.mn.ethersProvider)
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      ...
      if (
        ...
        // ERC1155 payment
        log.information(
          "one S->M single ERC1155 payment: {} {}",
          imaState.idToken,
          imaState.nAmountOfToken,
        );
        if (!imaState.chainProperties.mn.ethersProvider)
...
export function commandLineTaskPaymentS2S(): void {
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      ...
      const tc = imaState.chainProperties.tc;
      ...
      const joTokenManagerERC20Src: owaspUtils.ethersMod.ethers.Contract | null =
        isForward
          ? imaState.joTokenManagerERC20
          : imaState.joTokenManagerERC20Target;
      const joTokenManagerERC721Src: owaspUtils.ethersMod.ethers.Contract | null =
        isForward
      ...
      const joTokenManagerERC1155Src: owaspUtils.ethersMod.ethers.Contract | null =
        isForward
      ...
      const strCoinNameErc20Src = isForward
        ? sc.strCoinNameErc20
        : tc.strCoinNameErc20;
      const strCoinNameErc721Src = isForward
        ? sc.strCoinNameErc721
        : tc.strCoinNameErc721;
      const strCoinNameErc1155Src = isForward
        ? sc.strCoinNameErc1155
        : tc.strCoinNameErc1155;
      ...
      if (!strAddrErc20ExplicitTarget && tc.joErc20 && tc.strCoinNameErc20)
        strAddrErc20ExplicitTarget =
          tc.joErc20[tc.strCoinNameErc20 + "_address"];
      ...
      if (!strAddrErc721ExplicitTarget && tc.joErc721 && tc.strCoinNameErc721)
        strAddrErc721ExplicitTarget =
          tc.joErc721[tc.strCoinNameErc721 + "_address"];
      if (!strAddrErc1155Explicit && sc.joErc1155 && sc.strCoinNameErc1155)
        strAddrErc1155Explicit =
          sc.joErc1155[sc.strCoinNameErc1155 + "_address"];
      if (
        ...
        strAddrErc1155ExplicitTarget =
          tc.joErc1155[tc.strCoinNameErc1155 + "_address"];
      ...
      const strAddrErc1155Dst = isForward
        ? strAddrErc1155ExplicitTarget
        : strAddrErc1155Explicit;
      const txCustomizer: imaTx.TransactionCustomizer = isForward
        ? sc.transactionCustomizer
        : tc.transactionCustomizer;
      if (strCoinNameErc721Src.length > 0) {
        // ERC721 payment
        log.information("one S->S single ERC721 payment: {}", imaState.idToken);
        if (!ethersProviderSrc) throw new Error("No S2S source provider");
      ...
      if (
        ...
        // ERC1155 payment
        log.information(
          "one S->S single ERC1155 payment: {} {}",
          imaState.idToken,
          imaState.nAmountOfToken,
        );
        if (!ethersProviderSrc) throw new Error("No S2S source provider");
      ...
      if (
        ...
        // ERC1155 Batch payment
        log.information(
          "one S->S single ERC1155 Batch payment: {} {}",
          imaState.idTokens,
          imaState.arrAmountsOfTokens,
        );
        if (!ethersProviderSrc) throw new Error("No S2S source provider");
...
export function commandLineTaskViewS2M(): void {
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      ...
      if (xWei === null || xWei === undefined) return false;
      const xEth = owaspUtils.ethersMod.ethers.utils.formatEther(
...
export function commandLineTaskTransferS2S(): void {
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      if (!imaState.optsS2S.isEnabled) return true;
      if (!imaState.bNoWaitSChainStarted)
        await discoveryTools.waitUntilSChainStarted(); // s-chain --> main-net transfer
      const joRuntimeOpts: loop.TRuntimeOpts = {
        isInsideWorker: false,
...
async function handleBrowseSkaleModesRpcInfoResult(
  ...
  for (let i = 0; i < jarrNodes.length; ++i) {
    ...
    if (!joNode) {
      log.critical(
        "{p}Discovery node {} is completely unknown and will be skipped",
        strLogPrefix,
        i,
      );
      continue;
...
export function commandLineTaskBrowseSChain(): void {
  ...
  imaState.arrActions.push({
    ...
    fn: async function (): Promise<boolean> {
      ...
      log.information(
        "{p}Downloading S-Chain network information...",
        strLogPrefix,
      );
      const rpcCallOpts: rpcCall.TRPCCallOpts | null = null;
      let joCall: rpcCall.TRPCCall | null = null;
      try {
        joCall = await rpcCall.create(
