...
export type TFunctionAfterRediscovery = (status: boolean) => void;
...
export function formatBalanceInfo(bi: any, strAddress: string): string {
  ...
  if ("idToken" in bi) s += log.fmtDebug(" token ID {}", bi.idToken);
  ...
  if (bi.assetName == "ERC721") {
    ...
    s +=
      ...
      (isSame
        ? log.fmtSuccess(
            "same (as account {} specified in the command line arguments)",
            strAddress,
          )
        : log.fmtError(
            "different (than account {} specified in the command line arguments)",
            strAddress,
          ));
  }
  return s;
...
function getSChainNodesCount(joSChainNetworkInfo: TSChainNetworkInfo): number {
  try {
    if (!joSChainNetworkInfo) return 0;
...
export function getSChainDiscoveredNodesCount(
  ...
  try {
    ...
    const jarrNodes = joSChainNetworkInfo.network;
    const cntNodes = jarrNodes.length;
    if (cntNodes <= 0) return 0;
    let cntDiscovered = 0;
    for (let i = 0; i < cntNodes; ++i) {
      try {
        const joNode = joSChainNetworkInfo.network[i];
        if (isSChainNodeFullyDiscovered(joNode)) ++cntDiscovered;
      } catch (err) {
        return 0;
      }
    }
...
export async function waitUntilSChainStarted(): Promise<void> {
  ...
  for (; !bSuccess; ) {
    try {
      log.attention(
        "This S-Chain discovery will be done for startup pre-requisite",
      );
      ...
      if (
        !isError &&
        joSChainNetworkInfo &&
        typeof joSChainNetworkInfo === "object"
      ) {
    ...
    if (!bSuccess) ++idxWaitAttempt;
    if (idxWaitAttempt >= imaState.nMaxWaitSChainAttempts) {
      log.warning(
        "Incomplete, S-Chain sanity check failed after {} attempts.",
        idxWaitAttempt,
      );
      return;
    }
...
function composeStillUnknownNodesMessage(
  ...
  cntStillUnknown: number,
  cntNodesOnChain: number,
): string {
  let strMessage = log.fmtSuccess(
    ", {} of {} still unknown (",
    cntStillUnknown,
    cntNodesOnChain,
  );
  try {
    ...
    for (let i = 0; i < jarrNodes.length; ++i) {
      ...
      try {
        if (isSChainNodeFullyDiscovered(joNode)) continue;
        if (cntBad > 0) strMessage += log.fmtSuccess(", ");
...
async function handlePeriodicDiscoveryAttemptActions(
  ...
  if (gFlagIsInSChainDiscovery) {
    log.information(
      "Notice: long this S-Chain re-discovery is in progress now...",
    );
    return;
  }
  const imaState: state.TIMAState = state.get();
  ...
  try {
    let nCountToWait =
      cntNodesOnChain > 2
        ...
        : cntNodesOnChain;
    if (nCountToWait > cntNodesOnChain) nCountToWait = cntNodesOnChain;
    const cntDiscovered = getSChainDiscoveredNodesCount(
      imaState.joSChainNetworkInfo,
    );
    if (cntDiscovered >= cntNodesOnChain) {
      if (!isSilentReDiscovery) {
        log.information(
          "Everything is discovered about this S-Chain. ",
    ...
    if (cntDiscovered < cntNodesOnChain) {
      if (!isSilentReDiscovery) {
        ...
        log.information(
          "Have {} of {} nodes of this S-Chain not discovered yet " +
    ...
    if (!isSilentReDiscovery) {
      log.information(
        "This S-Chain discovery will be done for re-discover task",
      );
      log.information(
        "Will re-discover {}-nodes S-Chain network, {} node(s) already " +
          "discovered...",
        nCountToWait,
        cntDiscovered,
      );
    ...
    let isError = false;
    const joSChainNetworkInfo: TSChainNetworkInfo =
    ...
    if (!isError) {
      ...
      let strMessage = log.fmtSuccess(
        "S-Chain network was re-discovered, {} of {} node(s)" +
          "({} nodes known)",
        cntDiscoveredNow,
        nCountToWait,
        cntDiscoveredNow,
      ...
      if (!isSilentReDiscovery) {
        strMessage += log.fmtSuccess(
          ", complete re-discovered S-Chain " + "network info: {}",
          joSChainNetworkInfo,
        );
      }
    ...
    continueSChainDiscoveryInBackgroundIfNeeded(isSilentReDiscovery, null)
      ...
      .catch(function (err): void {
        log.error(
          "Failed to continue S-chain discovery, reported error is: {err}",
          err,
        );
      });
  } catch (err) {}
...
export async function continueSChainDiscoveryInBackgroundIfNeeded(
  ...
  if (gTimerSChainDiscovery != null) return;
  fnAfter = fnAfter ?? function (): void {};
  const imaState: state.TIMAState = state.get();
  ...
  if (cntDiscovered >= cntNodesOnChain) {
    ...
    if (gTimerSChainDiscovery != null) {
      clearInterval(gTimerSChainDiscovery);
      gTimerSChainDiscovery = null;
      if (!isSilentReDiscovery) log.notice("This S-Chain re-discovery stopped");
    }
    fnAfter();
    return;
  }
  if (cntDiscovered < cntNodesOnChain) {
    if (!isSilentReDiscovery) {
      const cntUnDiscoveredYet = cntNodesOnChain - cntDiscovered;
      log.information(
        "Have {} of {} nodes of this S-Chain not discovered yet before " +
...
function handleDiscoverSkaleImaInfoResult(
  optsDiscover: TDiscoveryOptions,
  strNodeDescColorized: string,
  joNode: any,
  ...
  if (!optsDiscover.isSilentReDiscovery) {
    log.success(
      "{p}OK, got {} node {} IMA information({} of {}).",
      optsDiscover.strLogPrefix,
...
async function discoverSChainWalkNodes(
  optsDiscover: TDiscoveryOptions,
): Promise<void> {
  ...
  for (let i = 0; i < optsDiscover.cntNodes; ++i) {
    ...
    if (!optsDiscover.isSilentReDiscovery) {
      log.information(
        "{p}Will try to discover S-Chain node {}...",
        optsDiscover.strLogPrefix,
        strNodeDescColorized,
      );
    }
    try {
      if (
        optsDiscover.joPrevSChainNetworkInfo &&
    ...
    try {
      ...
      if (!joCall)
        throw new Error(
          `Failed to create JSON RPC call object to ${strNodeURL}`,
        );
...
async function discoverSChainWait(
  optsDiscover: TDiscoveryOptions,
): Promise<void> {
  if (!optsDiscover.isSilentReDiscovery) {
    log.debug(
      "{p}Waiting for response from at least {} node(s)...",
      optsDiscover.strLogPrefix,
  ...
  if (cntWaitAttempts < 1) cntWaitAttempts = 1;
  const iv = setInterval(function (): void {
    ...
    optsDiscover.nCountReceivedImaDescriptions = getSChainDiscoveredNodesCount(
      optsDiscover.joSChainNetworkInfo,
    );
    if (!optsDiscover.isSilentReDiscovery) {
      log.debug(
    ...
    if (!optsDiscover.isSilentReDiscovery) {
      log.information(
        "{p}Have S-Chain description response about {} of {} node(s).",
        optsDiscover.strLogPrefix,
        optsDiscover.nCountReceivedImaDescriptions,
        optsDiscover.cntNodes,
      );
    ...
    if (
      optsDiscover.nCountReceivedImaDescriptions >= optsDiscover.nCountToWait
    ) {
      if (!optsDiscover.isSilentReDiscovery) {
        log.success(
    ...
    if (nWaitAttempt >= cntWaitAttempts) {
      ...
      if (!optsDiscover.isSilentReDiscovery) {
        log.warning(
          "{p}This S-Chain discovery will finish due to: {err}",
          optsDiscover.strLogPrefix,
          strErrorDescription,
        );
      }
    ...
    if (!optsDiscover.isSilentReDiscovery) {
      log.debug(
        "{p}S-Chain discovery waiting attempt {} of {} for {} node answer(s)",
        optsDiscover.strLogPrefix,
        nWaitAttempt,
        cntWaitAttempts,
...
async function handleDiscoverSkaleNodesRpcInfoResult(
  ...
  if (!optsDiscover.isSilentReDiscovery) {
    log.trace(
      "{p}OK, got (own) S-Chain network information: {}",
      optsDiscover.strLogPrefix,
      joOut.result,
    );
    log.success(
      "{p}OK, got S-Chain {url} network information.",
      optsDiscover.strLogPrefix,
      scURL,
    );
  }
  ...
  if (!optsDiscover.joSChainNetworkInfo) {
    ...
    if (!optsDiscover.isSilentReDiscovery) {
      log.critical(
        "{p}Network was not detected via call to {url}: {err}",
        optsDiscover.strLogPrefix,
        scURL,
        err2,
      );
    }
    if (optsDiscover.fnAfter) optsDiscover.fnAfter(err2, null);
  ...
  if (!optsDiscover.isSilentReDiscovery) {
    log.information(
      "{p}Will gather details of {} of {} node(s)...",
      optsDiscover.strLogPrefix,
      optsDiscover.nCountToWait,
      optsDiscover.cntNodes,
    );
  ...
  if (!optsDiscover.isSilentReDiscovery) {
    log.debug(
      "Waiting for S-Chain nodes, total {}, available {}, expected at least {}",
      optsDiscover.cntNodes,
      optsDiscover.nCountAvailable,
      optsDiscover.nCountToWait,
    );
  ...
  await discoverSChainWait(optsDiscover)
    ...
    .catch(function (err: Error | string): void {
      log.error(
        "Failed to wait until S-chain discovery complete, reported error is: {err}",
        err,
      );
      if (optsDiscover.fnAfter) optsDiscover.fnAfter(err, null);
    });
...
export async function discoverSChainNetwork(
  ...
  if (
    optsDiscover.nCountToWait == null ||
  ...
  if (!optsDiscover.isSilentReDiscovery)
    log.information(
      "{p}This S-Chain discovery will start...",
      optsDiscover.strLogPrefix,
    );
  let joCall: rpcCall.TRPCCall | null = null;
  try {
    ...
    if (optsDiscover.fnAfter) optsDiscover.fnAfter(err as Error, null);
    if (joCall) await joCall.disconnect();
    throw err;
  }
...
function checkPeriodicDiscoveryNoLongerNeeded(
  ...
  if (!joSChainNetworkInfo) return false;
  const imaState: state.TIMAState = state.get();
  if (!imaState.joSChainNetworkInfo)
    throw new Error("No own S-Chain network information");
  const cntNodesOnChain = getSChainNodesCount(imaState.joSChainNetworkInfo);
  const cntAlreadyDiscovered =
    getSChainDiscoveredNodesCount(joSChainNetworkInfo);
  if (!isSilentReDiscovery) {
    log.notice(
      "Periodic S-Chain re-discovery already have {} of {} node(s) discovered",
      cntAlreadyDiscovered,
      cntNodesOnChain,
    );
  }
...
export async function doPeriodicSChainNetworkDiscoveryIfNeeded(
  isSilentReDiscovery: boolean,
  fnAfterRediscover?: TFunctionAfterRediscovery,
): Promise<void> {
  if (gIntervalPeriodicDiscovery) return; // already started
  const imaState: state.TIMAState = state.get();
  let joPrevSChainNetworkInfo = imaState.joSChainNetworkInfo;
  if (
    checkPeriodicDiscoveryNoLongerNeeded(
      joPrevSChainNetworkInfo,
      isSilentReDiscovery,
    )
  ) {
    if (!isSilentReDiscovery)
      log.success(
        "Periodic S-Chain re-discovery is not needed right from startup",
      );
  ...
  if (!imaState.joSChainNetworkInfo)
    throw new Error("No own S-Chain network information");
  const cntNodesOnChain = getSChainNodesCount(imaState.joSChainNetworkInfo);
  let periodicDiscoveryInterval =
    imaState.joSChainDiscovery.periodicDiscoveryInterval;
  if (periodicDiscoveryInterval <= 0) periodicDiscoveryInterval = 5 * 60 * 1000;
  if (!isSilentReDiscovery) {
    log.debug(
      "Periodic S-Chain re-discovery will be done with {} interval...",
  ...
  gIntervalPeriodicDiscovery = setInterval(function (): void {
    ...
    if (!isSilentReDiscovery)
      log.information(
        "This S-Chain discovery will be done for periodic discovery update",
      );
    discoverSChainNetwork(
      ...
      .then(function (): void {
        ...
        if (
          ...
          if (fnAfterRediscover) fnAfterRediscover(true);
          return; // not needed anymore, all nodes completely discovered
        }
        if (!isSilentReDiscovery)
          log.information("Partial periodic S-Chain re-discovery done");
        if (fnAfterRediscover) fnAfterRediscover(false);
      })
      .catch(function (): void {});
  }, periodicDiscoveryInterval);
  if (!isSilentReDiscovery) {
    log.information(
      "Periodic S-Chain re-discovery was started with interval {}" +
        " millisecond(s)",
      periodicDiscoveryInterval,
    );
  }
