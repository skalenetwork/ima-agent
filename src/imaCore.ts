...
async function findOutReferenceLogRecord(
  ...
  bnBlockId: any,
  nMessageNumberToFind: number,
  isVerbose?: boolean,
): Promise<any | null> {
  const bnMessageNumberToFind = owaspUtils.toBN(
    nMessageNumberToFind.toString(),
  );
  ...
  for (let idxLogRecord = 0; idxLogRecord < cntLogRecord; ++idxLogRecord) {
    ...
    const bnCurrentMessage = owaspUtils.toBN(
      joReferenceLogRecord.currentMessage.toString(),
    );
    if (bnCurrentMessage.eq(bnMessageNumberToFind)) {
      if (isVerbose) {
        details.success(
          "{p}Found {} log record {} for message {}",
          strLogPrefix,
          strEventName,
          joReferenceLogRecord,
  ...
  if (isVerbose) {
    details.error(
      "{p}Failed to find {} log record for message {}",
      strLogPrefix,
      strEventName,
      nMessageNumberToFind,
    );
...
async function findOutAllReferenceLogRecords(
  ...
  if (cntExpected <= 0) {
    if (isVerbose) {
      details.success(
        "{p}Optimized IMA message search algorithm success, " +
  ...
  for (; nWalkMsgNumber >= nIncMsgCnt; --nWalkMsgNumber) {
    ...
    if (joReferenceLogRecord == null) break;
    nWalkBlockId = owaspUtils.toBN(
      joReferenceLogRecord.previousOutgoingMessageBlockId,
    );
  ...
  if (cntFound != cntExpected) {
    ...
    if (isVerbose) {
      details.success(
        "{p}Optimized IMA message search algorithm success, found all {}" +
          " log record(s): {}",
        strLogPrefix,
        cntFound,
        arrLogRecordReferences,
...
async function doQueryOutgoingMessageCounter(
  ...
  optsTransfer.details.debug(
    "{p}SRC MessageProxy address is.....{}",
    optsTransfer.strLogPrefixShort,
    optsTransfer.joMessageProxySrc.address,
  );
  optsTransfer.details.debug(
    "{p}DST MessageProxy address is.....",
    optsTransfer.strLogPrefixShort,
    optsTransfer.joMessageProxyDst.address,
  );
  optsTransfer.strActionName =
    "src-chain.MessageProxy.getOutgoingMessagesCounter()";
  try {
    optsTransfer.details.debug(
      "{p}Will call {bright}...",
    ...
    if (!owaspUtils.validateInteger(nPossibleIntegerValue)) {
      throw new Error(
        `DST chain ${optsTransfer.chainNameDst} returned outgoing ` +
    ...
    optsTransfer.details.information(
      "{p}Result of {bright} call: {}",
      optsTransfer.strLogPrefix,
      optsTransfer.strActionName,
      optsTransfer.nOutMsgCnt,
    );
  } catch (err) {
  ...
  optsTransfer.strActionName =
    "dst-chain.MessageProxy.getIncomingMessagesCounter()";
  optsTransfer.details.debug(
    "{p}Will call {bright}...",
    optsTransfer.strLogPrefix,
    optsTransfer.strActionName,
  ...
  if (!owaspUtils.validateInteger(nPossibleIntegerValue)) {
    throw new Error(
      `SRC chain ${optsTransfer.chainNameSrc} returned incoming message ` +
  ...
  optsTransfer.details.debug(
    ...
    optsTransfer.strLogPrefix,
    optsTransfer.strActionName,
    optsTransfer.nIncMsgCnt,
  );
  optsTransfer.strActionName =
    "src-chain.MessageProxy.getIncomingMessagesCounter()";
  nPossibleIntegerValue =
  ...
  if (!owaspUtils.validateInteger(nPossibleIntegerValue)) {
    throw new Error(
      `DST chain ${optsTransfer.chainNameDst} returned incoming ` +
        `message counter ${nPossibleIntegerValue} + which is not a valid integer`,
    );
  }
  ...
  try {
    ...
    const bnBlockId = owaspUtils.toBN(
      ...
      if (bnBlockId) {
        ...
        optsTransfer.arrLogRecordReferences =
          await findOutAllReferenceLogRecords(
            ...
            optsTransfer.strLogPrefixShort,
            optsTransfer.ethersProviderSrc,
            optsTransfer.joMessageProxySrc,
            bnBlockId,
            optsTransfer.nIncMsgCnt,
            optsTransfer.nOutMsgCnt,
            true,
    ...
    optsTransfer.arrLogRecordReferences = [];
    optsTransfer.details.error(
      "{p}Optimized log search is un-available.",
      optsTransfer.strLogPrefix,
    );
  }
  ...
  for (
    let nWalkMsgNumber = optsTransfer.nIncMsgCnt;
...
async function analyzeGatheredRecords(
  ...
  const strChainHashWeAreLookingFor = owaspUtils.ethersMod.ethers.utils.id(
    optsTransfer.chainNameDst,
  );
  optsTransfer.details.debug(
    "{p}Will review {} found event records(in reverse order, newest to oldest) while looking " +
      "for hash {} of destination chain {}",
    optsTransfer.strLogPrefix,
    r.length,
    strChainHashWeAreLookingFor,
    optsTransfer.chainNameDst,
  );
  for (let i = r.length - 1; i >= 0; i--) {
    const joEvent = r[i];
    optsTransfer.details.debug(
      "{p}Will review found event record {} with data {}",
      optsTransfer.strLogPrefix,
      i,
      joEvent,
    );
    ...
    if (ev.dstChainHash == strChainHashWeAreLookingFor) {
      ...
      optsTransfer.details.success(
        "{p}Found event record {} reviewed and accepted for processing, found event " +
          "values are {}, found block number is {}",
        optsTransfer.strLogPrefix,
        i,
        joValues,
      ...
      optsTransfer.details.debug(
        "{p}Found event record {} reviewed and skipped",
        optsTransfer.strLogPrefix,
        i,
      );
    }
  ...
  if (joValues == "") {
    optsTransfer.details.critical(
      "{p}Can't get events from MessageProxy",
      optsTransfer.strLogPrefix,
    );
...
async function gatherMessages(
  ...
  optsTransfer.nIdxCurrentMsgBlockStart = owaspUtils.toInteger(
    optsTransfer.nIdxCurrentMsg,
  );
  let r;
  optsTransfer.cntAccumulatedForBlock = 0;
  for (
    let idxInBlock = 0; // inner loop wil create block of transactions
    optsTransfer.nIdxCurrentMsg < optsTransfer.nOutMsgCnt &&
    idxInBlock < optsTransfer.nTransactionsCountInBlock;
    ++optsTransfer.nIdxCurrentMsg,
      ++idxInBlock,
      ++optsTransfer.cntAccumulatedForBlock
  ) {
    const idxProcessing = optsTransfer.cntProcessed + idxInBlock;
    if (idxProcessing > optsTransfer.nMaxTransactionsCount) break;
    let nBlockFrom = 0;
    let nBlockTo = "latest";
    if (optsTransfer.arrLogRecordReferences.length > 0) {
      ...
      if (
        joReferenceLogRecord &&
        "currentBlockId" in joReferenceLogRecord &&
    ...
    if (optsTransfer.nBlockAwaitDepth > 0) {
      ...
      try {
        ...
        optsTransfer.details.debug(
          "{p}Event transactionHash is {}",
          optsTransfer.strLogPrefix,
          transactionHash,
        );
        const blockNumber = r[0].blockNumber;
        optsTransfer.details.debug(
          "{p}Event blockNumber is {}",
          optsTransfer.strLogPrefix,
          blockNumber,
        );
        ...
        optsTransfer.details.debug(
          "{p}Latest blockNumber is {}",
          optsTransfer.strLogPrefix,
          nLatestBlockNumber,
        );
        const nDist = nLatestBlockNumber - blockNumber;
        if (nDist < optsTransfer.nBlockAwaitDepth) bSecurityCheckPassed = false;
        optsTransfer.details.debug(
          "{p}Distance by blockNumber is {}, await check is {}",
          optsTransfer.strLogPrefix,
          nDist,
      ...
      if (!bSecurityCheckPassed) {
        optsTransfer.details.warning(
          "{p}Block depth check was not passed, canceling " +
    ...
    if (optsTransfer.nBlockAge > 0) {
      ...
      try {
        ...
        const joBlock =
          await optsTransfer.ethersProviderSrc.getBlock(blockNumber);
        if (!owaspUtils.validateInteger(joBlock.timestamp)) {
          throw new Error(
            "Block timestamp is not a valid " +
        ...
        optsTransfer.details.debug(
          "{p}Expected diff {}",
          optsTransfer.strLogPrefix,
          optsTransfer.nBlockAge,
        );
        if (tsDiff < optsTransfer.nBlockAge) bSecurityCheckPassed = false;
        optsTransfer.details.debug(
          "{p}Block age check is {}",
          optsTransfer.strLogPrefix,
          log.posNeg(bSecurityCheckPassed, "PASSED", "FAILED"),
        );
      } catch (err) {
      ...
      if (!bSecurityCheckPassed) {
        optsTransfer.details.warning(
          "{p}Block age check was not passed, " +
    ...
    optsTransfer.details.success(
      "{p}Got event details from getPastEvents() event invoked with msgCounter set to {} " +
        "and dstChain set to {}, event description: ",
      optsTransfer.strLogPrefix,
      optsTransfer.nIdxCurrentMsg,
      optsTransfer.chainNameDst,
      joValues,
    ...
    optsTransfer.details.debug(
      "{p}Will process message counter value {}",
      optsTransfer.strLogPrefix,
      optsTransfer.nIdxCurrentMsg,
    );
...
async function preCheckAllMessagesSign(
  ...
  const strDidInvokedSigningCallbackMessage = log.fmtDebug(
    ...
    optsTransfer.jarrMessages.length,
    optsTransfer.jarrMessages,
  );
  ...
  if (
    !loop.checkTimeFraming(
      null,
      optsTransfer.strDirection,
      optsTransfer.joRuntimeOpts,
    )
  ) {
    optsTransfer.details.warning(
      "{p}Time framing overflow (after signing messages)",
      optsTransfer.strLogPrefix,
    );
...
async function callbackAllMessagesSign(
  ...
  let signature: owaspUtils.TXYSignature | null = joGlueResult
    ? joGlueResult.signature
    : null;
  if (!signature) signature = { X: "0", Y: "0" };
  let hashPoint = joGlueResult ? joGlueResult.hashPoint : null;
  if (!hashPoint) hashPoint = { X: "0", Y: "0" };
  ...
  optsTransfer.details.debug(
    "{p}....debug args for msgCounter set to {}: {}",
    optsTransfer.strLogPrefix,
    optsTransfer.nIdxCurrentMsgBlockStart,
    joDebugArgs,
  );
  ...
  let estimatedGasPostIncomingMessages =
    await optsTransfer.transactionCustomizerDst.computeGas(
      ...
      arrArgumentsPostIncomingMessages,
      optsTransfer.joAccountDst,
      optsTransfer.strActionName,
      gasPrice,
      10000000,
  ...
  if (optsTransfer.strDirection == "S2M") {
    ...
    estimatedGasPostIncomingMessages = Math.max(
      estimatedGasPostIncomingMessages,
      expectedGasLimit,
    );
  }
  ...
  if (strErrorOfDryRun) throw new Error(strErrorOfDryRun);
  const opts: imaTx.TCustomPayedCallOptions = {
    isCheckTransactionToSchain: optsTransfer.chainNameDst !== "Mainnet",
  };
  ...
  if (optsTransfer.chainNameDst == "Mainnet") {
    optsTransfer.details.debug(
      "{p}Validating transfer to Main Net via MessageProxy error " +
        "absence on Main Net...",
      optsTransfer.strLogPrefix,
    );
    if (optsTransfer.joDepositBoxMainNet) {
      if (
        ...
        if (joEvents.length == 0) {
          ...
          throw new Error(
            "Verification failed for the PostMessageError event " +
              `of the MessageProxy ${optsTransfer.joMessageProxyDst.address}  ` +
              "contract, error events found",
          );
        }
        optsTransfer.details.success(
          "{p}Done, validated transfer to Main Net via " +
            "MessageProxy error absence on Main Net",
          optsTransfer.strLogPrefix,
        );
      } else {
        optsTransfer.details.warning(
          "{p}Cannot validate transfer to Main Net via " +
...
async function handleAllMessagesSigning(
  optsTransfer: TTransferOptions,
): Promise<boolean> {
  try {
    ...
    await optsTransfer
      ...
      .catch(function (err: Error | string): void {
        ...
        optsTransfer.bErrorInSigningMessages = true;
        optsTransfer.details.error(
          "{p}Problem in transfer handler(in signer): {err}",
          optsTransfer.strLogPrefix,
          err,
        );
        imaTransferErrorHandling.saveTransferError(
    ...
    if (strErrFinal) throw new Error(strErrFinal);
    return true;
  } catch (err) {
    optsTransfer.details.error(
      "{p}Problem in transfer handler(general): {err}",
      optsTransfer.strLogPrefix,
      err,
    );
    imaTransferErrorHandling.saveTransferError(
...
async function checkOutgoingMessageEventInOneNode(
  ...
  optsTransfer.details.trace(
    ...
    optsOutgoingMessageAnalysis.joNode.name,
    strUrlHttp,
  );
  ...
  try {
    ...
    for (let idxEvent = 0; idxEvent < cntEvents; ++idxEvent) {
      ...
      if (
        owaspUtils.ensureStartsWith0x(joMessage.sender).toLowerCase() ==
          owaspUtils
            .ensureStartsWith0x(eventValuesByName.srcContract)
            .toLowerCase() &&
        owaspUtils
          .ensureStartsWith0x(joMessage.destinationContract)
          .toLowerCase() ==
          owaspUtils
            .ensureStartsWith0x(eventValuesByName.dstContract)
            .toLowerCase()
      ) {
        bEventIsFound = true;
        break;
...
async function checkOutgoingMessageEvent(
  optsTransfer: TTransferOptions,
  joSChain: skaleObserver.TSChainInformation,
): Promise<boolean> {
  ...
  for (let idxMessage = 0; idxMessage < cntMessages; ++idxMessage) {
    ...
    try {
      for (
        optsOutgoingMessageAnalysis.idxNode = 0;
        ...
        const isContinueNodesAnalysis =
          await checkOutgoingMessageEventInOneNode(
            optsTransfer,
            optsOutgoingMessageAnalysis,
          );
        if (!isContinueNodesAnalysis) break;
      }
    } catch (err) {
...
async function doMainTransferLoopActions(
  optsTransfer: TTransferOptions,
): Promise<boolean> {
  ...
  while (optsTransfer.nIdxCurrentMsg < optsTransfer.nOutMsgCnt) {
    if (optsTransfer.nStepsDone > optsTransfer.nTransferSteps) {
      optsTransfer.details.warning(
        "{p}Transfer step count overflow",
        optsTransfer.strLogPrefix,
      );
      optsTransfer.details.close();
      imaTransferErrorHandling.saveTransferSuccessAll();
    ...
    if (
      ...
      optsTransfer.details.warning(
        "{p}WARNING: Time framing overflow" +
    ...
    if (optsTransfer.cntAccumulatedForBlock == 0) break;
    if (
      !loop.checkTimeFraming(
        null,
        optsTransfer.strDirection,
      ...
      optsTransfer.details.warning(
        "{p}Time framing overflow" + "(after forming block of messages)",
        optsTransfer.strLogPrefix,
      );
      optsTransfer.details.close();
      imaTransferErrorHandling.saveTransferSuccessAll();
    ...
    if (optsTransfer.strDirection == "S2S") {
      ...
      if (idxSChain < 0) {
        throw new Error(
          "Could not validate S2S messages, source " +
    ...
    optsTransfer.strGatheredDetailsName =
      ...
      `-->${optsTransfer.chainNameDst}`;
    try {
      if (!(await handleAllMessagesSigning(optsTransfer))) return false;
    } catch (err) {
      optsTransfer.details.critical(
    ...
    if (optsTransfer.bErrorInSigningMessages) break;
    ++optsTransfer.nStepsDone;
  }
  return true;
}
...
export async function doTransfer(
  ...
  joRuntimeOpts: loop.TRuntimeOpts,
  ethersProviderSrc: owaspUtils.ethersMod.ethers.providers.JsonRpcProvider,
  joMessageProxySrc: owaspUtils.ethersMod.ethers.Contract,
  joAccountSrc: state.TAccount,
  ethersProviderDst: owaspUtils.ethersMod.ethers.providers.JsonRpcProvider,
  ...
  optsTransfer.details = optsTransfer.imaState.isDynamicLogInDoTransfer
    ? log.globalStream()
    : log.createMemoryStream();
  optsTransfer.strLogPrefixShort = `${optsTransfer.strDirection}/#${optsTransfer.nTransferLoopCounter} `;
  optsTransfer.strLogPrefix =
    `${optsTransfer.strLogPrefixShort}transfer loop from ` +
    `${optsTransfer.chainNameSrc} to ${optsTransfer.chainNameDst}: `;
  if (gIsOneTransferInProgressInThisThread) {
    optsTransfer.details.warning(
      "{p}Transfer loop step is skipped because previous one " +
  ...
  try {
    ...
    optsTransfer.details.debug(
      "{p}Message signing is {oo}",
      optsTransfer.strLogPrefix,
      optsTransfer.imaState.bSignMessages,
    );
    if (
      optsTransfer.fnSignMessages == null ||
      optsTransfer.fnSignMessages == undefined ||
      !optsTransfer.imaState.bSignMessages
    ) {
      optsTransfer.details.debug(
        "{p}Using internal signing stub function",
        optsTransfer.strLogPrefix,
      );
      optsTransfer.fnSignMessages = async function (
        ...
        optsTransfer.details.debug(
          "{p}Message signing callback was not provided to IMA, first real message " +
            "index is: {}, have {} message(s) to process {}",
          optsTransfer.strLogPrefix,
        ...
        if (fnAfter)
          // null - no error, null - no signatures
          await fnAfter(null, jarrMessages, null);
      };
    } else {
      optsTransfer.details.debug(
        "{p}Using externally provided signing function",
        optsTransfer.strLogPrefix,
      );
    }
    optsTransfer.nTransactionsCountInBlock =
      optsTransfer.nTransactionsCountInBlock || 5;
    optsTransfer.nTransferSteps =
      optsTransfer.nTransferSteps || Number.MAX_SAFE_INTEGER;
    optsTransfer.nMaxTransactionsCount =
    ...
    if (optsTransfer.nBlockAwaitDepth < 0) optsTransfer.nBlockAwaitDepth = 0;
    if (optsTransfer.nBlockAge < 0) optsTransfer.nBlockAge = 0;
    try {
      ...
      optsTransfer.details.critical(
        "{p}Error in {} during {bright}: {err}, " + "stack is:\n{stack}",
        optsTransfer.strLogPrefix,
        optsTransfer.strGatheredDetailsName,
        optsTransfer.strActionName,
        err,
...
export async function doAllS2S( // s-chain --> s-chain
  ...
  log.information(
    "Have {} S-Chain(s) connected to this S-Chain for performing " +
      "S2S transfers in {}.",
    cntSChains,
    threadInfo.threadDescription(),
  );
  for (let idxSChain = 0; idxSChain < cntSChains; ++idxSChain) {
    ...
    log.information(
      "S2S transfer walk trough {}/{} S-Chain in {}...",
      chainNameSrc,
      chainIdSrc,
      threadInfo.threadDescription(),
    );
    ...
    try {
      ...
      if (!(await pwa.checkOnLoopStart(imaState, "s2s", nIndexS2S))) {
        ...
        log.notice(
          "Skipped(s2s) due to cancel mode reported from PWA in {}",
          threadInfo.threadDescription(),
        );
      } else {
        if (loop.checkTimeFraming(null, "s2s", joRuntimeOpts)) {
          ...
          log.notice(
            "Skipped(s2s) in {} due to time framing check",
            strLogPrefix,
            threadInfo.threadDescription(),
          );
        }
      ...
      bOK = false;
      log.error(
        "S2S step error from S-Chain {}, error is: {err} in {}, stack is:\n{stack}",
        chainNameSrc,
        err,
        threadInfo.threadDescription(),
        err,
  ...
  if ("joExtraSignOpts" in joRuntimeOpts) delete joRuntimeOpts.joExtraSignOpts; // reset/clear
  if (cntOK > 0 || cntFail > 0) {
    let s = log.fmtDebug(
      "Stats for S2S steps in {}: ",
      threadInfo.threadDescription(),
    );
    if (cntOK > 0) {
      s +=
        " " +
        log.fmtInformation("{p}", cntOK) +
        " " +
        log.fmtSuccess("S-Chain(s) processed OKay") +
        log.fmtDebug(", ");
    }
    if (cntFail > 0) {
      s +=
        " " +
        log.fmtInformation("{p}", cntFail) +
        " " +
