...
export async function getBalanceEth(
  ...
  try {
    if (!(ethersProvider && joAccount)) return "<no-data>";
    ...
    log.error(
      "{p}balance fetching error details: {err}, stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
export async function doEthPaymentFromMainNet(
  ...
  try {
    details.debug(
      "{p}Doing payment from mainnet with chainIdSChain={}...",
      strLogPrefix,
      chainIdSChain,
    );
    strActionName = "ETH payment from Main Net, deposit";
    const arrArguments = [chainIdSChain];
...
export async function doEthPaymentFromSChain(
  ...
  try {
    ...
    const arrArguments = [owaspUtils.toBN(weiHowMuch)];
...
export async function receiveEthPaymentFromSchainOnMainNet(
  ...
  try {
    ...
    details.critical(
      "{p}Receive payment error in {bright}: {err}, stack is:\n{stack}",
      strLogPrefix,
      strActionName,
      err,
      err,
...
export async function viewEthPaymentFromSchainOnMainNet(
  ...
  try {
    if (!(ethersProviderMainNet && joAccountMN && joDepositBoxETH)) return null;
    const addressFrom = joAccountMN.address();
    const xWei = await joDepositBoxETH.callStatic.approveTransfers(
      addressFrom,
      { from: addressFrom },
    );
    ...
    details.critical(
      "{p}View payment error in {bright}: {err}, stack is:\n{stack}",
      strLogPrefix,
      strActionName,
      err,
      err,
