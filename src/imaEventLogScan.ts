...
export function createProgressiveEventsScanPlan(
  ...
  const arrProgressiveEventsScanPlanA = [
    {
      nBlockFrom: nLatestBlockNumber - blocksInOneDay,
      nBlockTo: "latest",
      type: "1 day",
    },
    {
      nBlockFrom: nLatestBlockNumber - blocksInOneWeek,
      nBlockTo: "latest",
      type: "1 week",
    },
    {
      nBlockFrom: nLatestBlockNumber - blocksInOneMonth,
      nBlockTo: "latest",
      type: "1 month",
    },
    {
    ...
    {
      nBlockFrom: nLatestBlockNumber - blocksInThreeYears,
      nBlockTo: "latest",
      type: "3 years",
    },
  ];
  ...
  for (
    let idxPlan = 0;
    idxPlan < arrProgressiveEventsScanPlanA.length;
    ++idxPlan
  ) {
    const joPlan = arrProgressiveEventsScanPlanA[idxPlan];
    if (joPlan.nBlockFrom >= 0) arrProgressiveEventsScanPlan.push(joPlan);
  }
...
export async function safeGetPastEventsProgressive(
  ...
  details.information(
    "{p}Will run progressive logs search for event {} via URL {url}, " +
      "from block {}, to block...",
    strLogPrefix,
    strEventName,
    strURL,
  ...
  if (nBlockTo == "latest") {
    ...
    details.trace(
      "{p}Progressive event log records scan up to latest block #{} " +
        "assumed instead of {}",
      strLogPrefix,
    ...
    if (nBlockTo.gte(nLatestBlockNumber)) isLastLatest = true;
  }
  ...
  if (!(isFirstZero && isLastLatest)) {
    details.trace(
      "{p}Will skip progressive event log records scan and use scan in block " +
        "range from {} to {}",
      strLogPrefix,
  ...
  details.trace(
    "{p}Current latest block number is {}",
    strLogPrefix,
    nLatestBlockNumber.toHexString(),
  );
  const arrProgressiveEventsScanPlan = createProgressiveEventsScanPlan(
    details,
    nLatestBlockNumberPlus1,
  );
  details.trace(
    "Composed progressive event log records scan plan is: {}",
  ...
  for (
    let idxPlan = 0;
    idxPlan < arrProgressiveEventsScanPlan.length;
    ++idxPlan
  ) {
    const joPlan = arrProgressiveEventsScanPlan[idxPlan];
    if (joPlan.nBlockFrom < 0) continue;
    joLastPlan = joPlan;
    details.trace(
...
export async function getContractCallEvents(
  ...
  let nBlockFrom = nBlockNumber.sub(n10);
  let nBlockTo = nBlockNumber.add(n10);
  const nBlockZero = owaspUtils.toBN(0);
  const nLatestBlockNumber = owaspUtils.toBN(
    await imaHelperAPIs.safeGetBlockNumber(details, 10, ethersProvider),
  );
  const nLatestBlockNumberPlus1 = nLatestBlockNumber.add(owaspUtils.toBN(1));
  if (nBlockFrom.lt(nBlockZero)) nBlockFrom = nBlockZero;
  if (nBlockTo.gte(nLatestBlockNumber)) nBlockTo = nLatestBlockNumberPlus1;
  const joAllEventsInBlock = await safeGetPastEventsIterative(
    details,
    strLogPrefix,
    ethersProvider,
...
export async function safeGetTransactionCount(
  ...
  if (retValOnFail == null || retValOnFail == undefined) retValOnFail = "";
  ...
  for (; idxAttempt <= cntAttempts; ++idxAttempt) {
    ...
    if (!isOnLine) {
      ret = retValOnFail;
      if (!throwIfServerOffline) return ret;
      details.error(
        "Cannot call {} via {url} because server is off-line, attempt {} of {}",
        strFnName + "()",
        u,
        idxAttempt,
        cntAttempts,
      ...
      throw new Error(
        `Cannot ${strFnName}() via ${u} because server is off-line`,
      );
    }
    details.trace(
      "Call to {} via {url}, attempt {} of {}",
      strFnName + "()",
      u,
      idxAttempt,
      cntAttempts,
    ...
    try {
      ...
      ret = retValOnFail;
      details.error(
        "Failed call attempt {} of {} to {} via {url}, error is: {err}, " +
          "stack is:\n{stack}",
        idxAttempt,
        cntAttempts,
        strFnName + "()",
  ...
  if (idxAttempt + 1 > cntAttempts && ret === "") {
    details.error(
      "Failed call to {} via {url} after {} attempts",
      strFnName + "()",
      u,
      cntAttempts,
    );
    throw new Error(
      `Failed call to ${strFnName}() via ${u} after ${cntAttempts} attempts`,
    );
  }
  return ret;
}
...
export async function safeGetPastEvents(
  ...
  if (nBlockTo == "latest") {
    const nLatestBlockNumberPlus1 = nLatestBlockNumber.add(owaspUtils.toBN(1));
    nBlockTo = nLatestBlockNumberPlus1;
  } else nBlockTo = owaspUtils.toBN(nBlockTo);
  nBlockFrom = owaspUtils.toBN(nBlockFrom);
  for (; idxAttempt <= cntAttempts; ++idxAttempt) {
    const isOnLine = await rpcCall.checkUrl(u, nWaitStepMilliseconds);
    if (!isOnLine) {
    ...
    try {
      ...
      if (
        ...
        details.error(
          "{p}Did stopped {} event filtering because no such event exist " +
            "in smart contract",
          strLogPrefix,
          strEventName,
        );
  ...
  if (idxAttempt + 1 === cntAttempts && ret === "") {
    ...
    throw new Error(
      `Failed filtering attempt for ${strEventName} event, ` +
        `from block ${nBlockFrom.toHexString()}, to block ${nBlockTo.toHexString()} ` +
...
export async function safeGetPastEventsIterative(
  ...
  if (
    imaHelperAPIs.getBlocksCountInInIterativeStepOfEventsScan() <= 0 ||
  ...
  if (nBlockTo == "latest") {
    ...
    nBlockTo = nLatestBlockNumberPlus1;
    details.trace(
      "{p}Iterative scan up to latest block #{} assumed instead of {}",
      strLogPrefix,
      nBlockTo.toHexString(),
      "latest",
    );
  ...
  if (isFirstZero && isLastLatest) {
    if (
      nLatestBlockNumber
        .div(
          owaspUtils.toBN(
            imaHelperAPIs.getBlocksCountInInIterativeStepOfEventsScan(),
          ),
        )
        .gt(
          owaspUtils.toBN(imaHelperAPIs.getMaxIterationsInAllRangeEventsScan()),
        )
    ) {
      details.warning(
  ...
  details.trace(
    "{p}Iterative scan in {}/{} block range...",
    strLogPrefix,
    nBlockFrom.toHexString(),
    nBlockTo.toHexString(),
  );
  ...
  for (; true; ) {
    ...
    if (idxBlockSubRangeFrom.lt(nBlockFrom)) idxBlockSubRangeFrom = nBlockFrom;
    try {
      details.trace(
        "{p}Iterative scan of {}/{} block sub-range in {}/{} block range...",
        strLogPrefix,
        idxBlockSubRangeFrom.toHexString(),
        idxBlockSubRangeTo.toHexString(),
        nBlockFrom.toHexString(),
        nBlockTo.toHexString(),
      );
      ...
      if (
        joAllEventsInBlock &&
        joAllEventsInBlock != "" &&
        joAllEventsInBlock.length > 0
      ) {
        details.success(
          "{p}Result of iterative scan in {}/{} block range is {}",
          strLogPrefix,
          nBlockFrom.toHexString(),
          nBlockTo.toHexString(),
          joAllEventsInBlock,
    ...
    if (idxBlockSubRangeTo.lte(nBlockFrom)) break;
  }
  details.debug(
    "{p}Result of iterative scan in {}/{} is {}",
    strLogPrefix,
    nBlockFrom.toHexString(),
    nBlockTo.toHexString(),
    "empty block range",
