...
async function run() {
    ...
    try {
        if( gIsDebugLogging )
            log.fmtDebug( "Process startup arguments array is ", log.v( process.argv ) );
        if( process.argv.length != 3 )
            throw new Error( "Wrong number of command line arguments" );

        if( gIsDebugLogging )
            log.fmtDebug( "Main argument text is ", log.v( process.argv[2] ) );
        const joArg = JSON.parse( process.argv[2] );
        if( gIsDebugLogging )
            log.fmtDebug( "Main argument JSON is ", log.v( joArg ) );
        ...
        if( gIsDebugLogging )
            log.fmtError( "Failed to create RPC call: ", log.v( err ) );
        finalizeOutput( {
