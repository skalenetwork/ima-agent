...
export function setMillisecondsSleepBeforeFetchOutgoingMessageEvent(
  val?: number,
): void {
...
export function setSleepBetweenTransactionsOnSChainMilliseconds(
  val?: number,
): void {
...
export async function safeWaitForNextBlockToAppear(
  ...
  const nBlockNumber: any = owaspUtils.toBN(
    await safeGetBlockNumber(details, 10, ethersProvider),
  );
  ...
  for (; true; ) {
    ...
    const nBlockNumber2 = owaspUtils.toBN(
      await safeGetBlockNumber(details, 10, ethersProvider),
    );
    details.trace("    ...have block {}", nBlockNumber2.toHexString());
    if (nBlockNumber2.gt(nBlockNumber)) break;
  }
}
...
export async function safeGetBlockNumber(
  ...
  for (; idxAttempt <= cntAttempts; ++idxAttempt) {
    ...
    if (!isOnLine) {
      ...
      details.error(
        "Cannot call {} via {url} because server is off-line",
        strFnName + "()",
        u,
      );
      throw new Error(
    ...
    details.trace(
      "Repeat call to {} via {url}, attempt {}",
      strFnName + "()",
      u,
      idxAttempt,
    );
    try {
      ...
      ret = retValOnFail;
      details.error(
        "Failed call attempt {} to  via {url}, error is: {err}, " +
          "stack is:\n{stack}",
        idxAttempt,
        strFnName + "()",
        u,
...
export function setBlocksCountInInIterativeStepOfEventsScan(n?: number): void {
  if (!n) gCountOfBlocksInIterativeStep = 0;
  else {
    gCountOfBlocksInIterativeStep = owaspUtils.parseIntOrHex(n);
    if (gCountOfBlocksInIterativeStep < 0) gCountOfBlocksInIterativeStep = 0;
  }
}
...
export function setMaxIterationsInAllRangeEventsScan(n?: number): void {
  if (!n) gMaxBlockScanIterationsInAllRange = 0;
  else {
