...
export interface TGasPriceSetupOptions {
  ...
  latestBlockNumber: any | null;
  latestBlock: any | null;
  bnTimestampOfBlock: any | null;
  bnTimeZoneOffset: any | null;
  gasPriceOnMainNet: any | null;
}
...
async function prepareOracleGasPriceSetup(
  ...
  optsGasPriceSetup.latestBlockNumber =
    await optsGasPriceSetup.ethersProviderMainNet.getBlockNumber();
  optsGasPriceSetup.details.trace(
    "Latest block on Main Net is {}",
    optsGasPriceSetup.latestBlockNumber,
  );
  optsGasPriceSetup.strActionName =
    "prepareOracleGasPriceSetup.optsGasPriceSetup.bnTimestampOfBlock()";
  optsGasPriceSetup.latestBlock =
    await optsGasPriceSetup.ethersProviderMainNet.getBlock(
      optsGasPriceSetup.latestBlockNumber,
    );
  optsGasPriceSetup.bnTimestampOfBlock = owaspUtils.toBN(
    optsGasPriceSetup.latestBlock.timestamp,
  );
  optsGasPriceSetup.details.trace(
    "Local timestamp on Main Net is {}={} (original)",
  ...
  optsGasPriceSetup.bnTimeZoneOffset = owaspUtils.toBN(
    new Date(
      parseInt(optsGasPriceSetup.bnTimestampOfBlock.toString(), 10),
    ).getTimezoneOffset(),
  );
  optsGasPriceSetup.details.trace(
    "Local time zone offset is {}={} (original)",
    optsGasPriceSetup.bnTimeZoneOffset.toString(),
    owaspUtils.ensureStartsWith0x(
      optsGasPriceSetup.bnTimeZoneOffset.toHexString(),
    ),
  );
  optsGasPriceSetup.bnTimestampOfBlock =
    optsGasPriceSetup.bnTimestampOfBlock.add(
      optsGasPriceSetup.bnTimeZoneOffset,
    );
  optsGasPriceSetup.details.trace(
    "UTC timestamp on Main Net is {}={} (original)",
    optsGasPriceSetup.bnTimestampOfBlock.toString(),
    owaspUtils.ensureStartsWith0x(
      optsGasPriceSetup.bnTimestampOfBlock.toHexString(),
    ),
  ...
  optsGasPriceSetup.details.trace(
    "Timestamp on Main Net is {}={} (adjusted to past a bit)",
    optsGasPriceSetup.bnTimestampOfBlock.toHexString(),
    owaspUtils.ensureStartsWith0x(
      optsGasPriceSetup.bnTimestampOfBlock.toHexString(),
    ),
  ...
  if (getEnabledOracle()) {
    const oracleOpts = {
      url: owaspUtils.ethersProviderToUrl(
      ...
      cntAttempts: 40,
      isVerbose: log.verboseGet() >= log.verboseName2Number("information"),
    ...
    try {
      ...
      optsGasPriceSetup.gasPriceOnMainNet = null;
      optsGasPriceSetup.details.error(
        "Failed to fetch Main Net gas price via call " +
  ...
  if (optsGasPriceSetup.gasPriceOnMainNet === null) {
    optsGasPriceSetup.details.debug(
      "Will fetch Main Net gas price directly...",
    );
    optsGasPriceSetup.gasPriceOnMainNet = owaspUtils.ensureStartsWith0x(
      owaspUtils
        .toBN(await optsGasPriceSetup.ethersProviderMainNet.getGasPrice())
        .toHexString(),
    );
  }
  optsGasPriceSetup.details.success(
    "Done, Oracle did computed new Main Net gas price={}={}",
  ...
  optsGasPriceSetup.details.trace(
    ...
    bnGasPriceOnMainNetOld.toString(),
    bnGasPriceOnMainNetOld.toHexString(),
  );
  if (
    bnGasPriceOnMainNetOld.eq(
      owaspUtils.toBN(optsGasPriceSetup.gasPriceOnMainNet),
    )
  ) {
    optsGasPriceSetup.details.trace(
      "Previous Main Net gas price is equal to new one, " +
        " will skip setting it in CommunityLocker",
    );
    if (log.exposeDetailsGet()) {
      optsGasPriceSetup.details.exposeDetailsTo(
        log.globalStream(),
        "doOracleGasPriceSetup",
        true,
      );
    }
...
async function handleOracleSigned(
  ...
  if (strError) {
    ...
    imaTransferErrorHandling.saveTransferError(
      "oracle",
      optsGasPriceSetup.details.toString(),
    );
    optsGasPriceSetup.details.close();
    return;
  }
  ...
  const gasPrice =
    await optsGasPriceSetup.transactionCustomizerSChain.computeGasPrice(
      optsGasPriceSetup.ethersProviderSChain,
      200000000000,
    );
  optsGasPriceSetup.details.trace(
  ...
  const estimatedGasSetGasPrice =
    await optsGasPriceSetup.transactionCustomizerSChain.computeGas(
      optsGasPriceSetup.details,
      optsGasPriceSetup.ethersProviderSChain,
      "CommunityLocker",
      optsGasPriceSetup.joCommunityLocker,
      "setGasPrice",
      arrArgumentsSetGasPrice,
      optsGasPriceSetup.joAccountSC,
      optsGasPriceSetup.strActionName,
      gasPrice,
      10000000,
    );
  ...
  if (joReceipt) {
    ...
    imaGasUsage.printGasUsageReportFromArray(
      ...
      optsGasPriceSetup.jarrReceipts,
      optsGasPriceSetup.details,
    );
  }
...
export async function doOracleGasPriceSetup(
  ...
  if (!getEnabledOracle()) return true;
  ...
  if (
    optsGasPriceSetup.fnSignMsgOracle == null ||
    optsGasPriceSetup.fnSignMsgOracle == undefined
  ) {
    optsGasPriceSetup.details.trace(
      "{p}Using internal u256 signing stub function",
      optsGasPriceSetup.strLogPrefix,
    );
    optsGasPriceSetup.fnSignMsgOracle = async function (
      u256: any,
      details: log.TLogger,
      fnAfter: IMA.TFunctionAfterSigningMessages,
    ): Promise<void> {
      details.trace(
        "{p}u256 signing callback was not provided",
        optsGasPriceSetup.strLogPrefix,
      );
    ...
    optsGasPriceSetup.details.trace(
      "{p}Using externally provided u256 signing function",
      optsGasPriceSetup.strLogPrefix,
    );
  }
  try {
