...
export async function invokeHasChain(
  ...
  try {
    ...
    const bHasSchain = await joLinker.callStatic.hasSchain(chainIdSChain, {
      from: addressFrom,
    });
    details.success(
      "{p}Got joLinker.hasSchain() status is: {}",
      strLogPrefix,
      bHasSchain,
    );
    return bHasSchain;
    ...
    details.critical(
      "{p}Error in invokeHasChain() during {bright}: {err}, stack is:\n{stack}",
      strLogPrefix,
      strActionName,
      err,
      err,
...
export async function waitForHasChain(
  ...
  if (!cntWaitAttempts) cntWaitAttempts = 100;
  if (!nSleepMilliseconds) nSleepMilliseconds = 5;
  for (
    let idxWaitAttempts = 0;
    idxWaitAttempts < cntWaitAttempts;
    ++idxWaitAttempts
  ) {
    if (
      await invokeHasChain(
        details,
        ethersProvider,
        joLinker,
        joAccount,
...
export async function checkIsRegisteredSChainInDepositBoxes( // step 1
  ...
  details.debug(
    "Main-net Linker address is...........{}",
    joLinker ? joLinker.address : "N/A",
  );
  ...
  details.debug(
    "{p}{p}",
    strLogPrefix,
    "checkIsRegisteredSChainInDepositBoxes(reg-step1)",
  );
  details.debug("{p}{p}", strLogPrefix, imaHelperAPIs.longSeparator);
  let strActionName = "";
  try {
    ...
    if (!joLinker) throw new Error("No Linker contract");
    const bIsRegistered = await joLinker.callStatic.hasSchain(chainIdSChain, {
      from: addressFrom,
    });
    details.success(
      "{p}checkIsRegisteredSChainInDepositBoxes(reg-step1) status is: {}",
      strLogPrefix,
      bIsRegistered,
    );
...
export async function registerSChainInDepositBoxes( // step 1
  ...
  try {
    ...
    details.debug(
      "{p}Will register S-Chain in lock_and_data on Main-net",
      strLogPrefix,
    );
    if (!joTokenManagerLinker)
      throw new Error("No TokenManagerLinker contract");
    if (!joCommunityLocker) throw new Error("No CommunityLocker contract");
    if (!joTokenManagerETH) throw new Error("No TokenManagerETH contract");
    if (!joTokenManagerERC20) throw new Error("No TokenManagerERC20 contract");
    ...
    details.critical(
      "{p}Error in registerSChainInDepositBoxes() during {bright}: {err}" +
        ", stack is:\n{stack}",
      strLogPrefix,
      strActionName,
      err,
