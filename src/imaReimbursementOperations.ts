...
export async function reimbursementShowBalance(
  ...
  try {
    ...
    if (isForcePrintOut) log.information(s);
    ...
    details.critical(
      "{p}Payment error in reimbursementShowBalance(): {err}, " +
        "stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
...
export async function reimbursementEstimateAmount(
  ...
  try {
    ...
    s =
      strLogPrefix + log.fmtSuccess("MinTransactionGas: {}", minTransactionGas);
    if (isForcePrintOut) log.information(s);
    details.information(s);
    ...
    s =
      strLogPrefix + log.fmtSuccess("Minimum recharge balance: {}", minAmount);
    if (isForcePrintOut) log.information(s);
    details.information(s);

    let amountToRecharge = 0;
    if (xWei >= minAmount) amountToRecharge = 1;
    else amountToRecharge = minAmount - xWei;

    s =
      strLogPrefix +
      log.fmtSuccess("Estimated amount to recharge(wei): {}", amountToRecharge);
    if (isForcePrintOut) log.information(s);
    details.information(s);

    const amountToRechargeEth = owaspUtils.ethersMod.ethers.utils.formatEther(
      owaspUtils.toBN(amountToRecharge.toString()),
    );
    s =
      strLogPrefix +
      log.fmtSuccess(
        "Estimated amount to recharge(eth): {}",
        amountToRechargeEth,
      );
    if (isForcePrintOut) log.information(s);
    details.information(s);
    ...
    details.critical(
      "{p} Payment error in reimbursementEstimateAmount(): {err}, " +
        "stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
...
export async function reimbursementSetRange(
  ...
  let strActionName = "";
  const strLogPrefix =
    "Gas Reimbursement - Set Minimal time interval from S2M transfers ";
  try {
    details.debug(
      "{p}Setting minimal S2M interval to {}...",
      strLogPrefix,
      nReimbursementRange,
    );
    ...
    const arrArguments = [
      ...
      owaspUtils.ensureStartsWith0x(
        owaspUtils.toBN(nReimbursementRange).toHexString(),
      ),
    ];
