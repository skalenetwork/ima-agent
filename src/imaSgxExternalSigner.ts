...
async function run(): Promise<void> {
  try {
    ...
    if (gIsDebugLogging) log.debug("SGX Wallet URL is {url}", strSgxWalletURL);
    const strSgxKeyName = process.argv[4];
    if (gIsDebugLogging) log.debug("SGX key name is {url}", strSgxWalletURL);
    const strURL = process.argv[5];
    if (gIsDebugLogging) log.debug("Chain URL is {url}", strURL);
    const chainId = process.argv[6];
    if (gIsDebugLogging) log.debug("Chain ID is {}", chainId);
    const tcData = process.argv[7];
    if (gIsDebugLogging) log.debug("TX data is {}", tcData);
    const txTo = process.argv[8];
    if (gIsDebugLogging) log.debug("TX destination is {}", txTo);
    const txValue = process.argv[9];
    if (gIsDebugLogging) log.debug("TX value is {}", txValue);
    const gasPrice = process.argv[10];
    if (gIsDebugLogging) log.debug("TX gas price is {}", gasPrice);
    const gasLimit = process.argv[11];
    if (gIsDebugLogging) log.debug("TX gas limit is {}", gasLimit);
    const txNonce = process.argv[12];
    if (gIsDebugLogging) log.debug("TX nonce is {}", txNonce);
    const strPathCert = process.argv[13];
    if (gIsDebugLogging)
      log.debug("Path to SGX certificate file is {}", strPathCert);

    const strPathKey = process.argv[14];
    if (gIsDebugLogging) log.debug("Path to SGX key file is {}", strPathKey);
    ...
    if (gIsDebugLogging) log.debug("--- Source TX ---> {}", tx);
    let rawTX = owaspUtils.ethersMod.ethers.utils.serializeTransaction(tx);
    if (gIsDebugLogging) log.debug("--- RAW unsigned TX ---> {}", rawTX);
    const txHash = owaspUtils.ethersMod.ethers.utils.keccak256(rawTX);
    if (gIsDebugLogging) log.debug("--- TX hash ---> {}", txHash);
    ...
    const joCall: rpcCall.TRPCCall = await rpcCall.create(
      strSgxWalletURL,
      rpcCallOpts,
    );
    if (!joCall)
      throw new Error(
        `Failed to create JSON RPC call object to ${strSgxWalletURL}`,
      );
    const joIn: any = {
    ...
    try {
      ...
      rawTX = owaspUtils.ethersMod.ethers.utils.serializeTransaction(
        tx,
        joExpanded,
      );
      if (gIsDebugLogging)
      ...
      if (gIsDebugLogging) log.debug("--- Call error ---> {}", log.em, err);
      finalizeOutput({ error: owaspUtils.extractErrorMessage(err) });
      process.exit(1);
    }
  } catch (err) {
    if (gIsDebugLogging) log.error("RPC call to SGX failed: {err}", err);
    finalizeOutput({ error: owaspUtils.extractErrorMessage(err) });
    process.exit(1);
  }
}
