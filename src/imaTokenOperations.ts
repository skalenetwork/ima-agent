...
export async function getBalanceErc20(
  ...
  try {
    ...
    const balance = await contractERC20.callStatic.balanceOf(strAddress, {
      from: strAddress,
    });
    return balance;
  } catch (err) {
    log.error(
      "{p}ERC20 balance fetching error: {err}, stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
export async function getOwnerOfErc721(
  ...
  try {
    ...
    const owner = await contractERC721.callStatic.ownerOf(idToken, {
      from: strAddress,
    });
    return owner;
  } catch (err) {
    log.error(
      "{p}ERC721 owner fetching error: {err}, stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
export async function getBalanceErc1155(
  ...
  try {
    ...
    log.error(
      "{p}ERC1155 balance fetching error: {err}, stack is:\n{stack}",
      strLogPrefix,
      err,
      err,
    );
...
export async function doErc721PaymentFromMainNet(
  ...
  try {
    ...
    const erc721ABI =
      erc721PrivateTestnetJsonMainNet[strCoinNameErc721MainNet + "_abi"];
    ...
    if (strErrorOfDryRunApprove) throw new Error(strErrorOfDryRunApprove);
    ...
    const estimatedGasDeposit = await transactionCustomizerMainNet.computeGas(
      ...
      joDepositBoxERC721,
      "depositERC721",
      arrArgumentsDepositERC721,
      joAccountSrc,
      strActionName,
      gasPrice,
      8000000,
      weiHowMuchDepositERC721,
      null,
    );
    details.trace(
      "{p}Using estimated(deposit) gas={}",
      strLogPrefix,
      estimatedGasDeposit,
    );
    ...
    if (joMessageProxyMainNet) {
      ...
      const joEvents = await imaEventLogScan.getContractCallEvents(
        ...
        strLogPrefix,
        ethersProviderMainNet,
        joMessageProxyMainNet,
        strEventName,
        joReceiptDeposit.blockNumber,
        joReceiptDeposit.transactionHash,
      ...
      if (joEvents.length > 0) {
        ...
        throw new Error(
          "Verification failed for the OutgoingMessage event of the " +
    ...
    details.critical(
      "{p}Payment error in {bright}: {err}, stack is:\n{stack}",
      strLogPrefix,
      strActionName,
      err,
      err,
...
export async function doErc1155PaymentFromMainNet(
  ...
  try {
    ...
    const estimatedGasDeposit = await transactionCustomizerMainNet.computeGas(
      ...
      joDepositBoxERC1155,
      "depositERC1155",
      arrArgumentsDepositERC1155,
      joAccountSrc,
      strActionName,
      gasPrice,
      8000000,
      weiHowMuchDepositERC1155,
      null,
    );
    details.trace(
      "{p}Using estimated(deposit) gas={}",
    ...
    const strErrorOfDryRunDepositERC1155 = await imaTx.dryRunCall(
      details,
      ethersProviderMainNet,
      "DepositBoxERC1155",
      joDepositBoxERC1155,
    ...
    if (joMessageProxyMainNet) {
      ...
      if (joEvents.length > 0) {
        ...
        throw new Error(
          "Verification failed for theOutgoingMessage event of the " +
            `MessageProxy ${joMessageProxyMainNet.address} contract, no events found`,
        );
      }
    }
  ...
  imaGasUsage.printGasUsageReportFromArray(
    "ERC-1155 PAYMENT FROM MAIN NET",
    jarrReceipts,
    details,
  );
  if (log.exposeDetailsGet())
...
export async function doErc1155BatchPaymentFromMainNet(
  ...
  try {
    ...
    const estimatedGasDeposit = await transactionCustomizerMainNet.computeGas(
      ...
      joDepositBoxERC1155,
      "depositERC1155Batch",
      arrArgumentsDepositERC1155Batch,
      joAccountSrc,
      strActionName,
      gasPrice,
      8000000,
      weiHowMuchDepositERC1155Batch,
      null,
    );
    details.trace(
      "{p}Using estimated(deposit) gas={}",
    ...
    details.exposeDetailsTo(
      log.globalStream(),
      "doErc1155BatchPaymentFromMainNet",
      false,
    );
    details.close();
    return false;
...
export async function doErc20PaymentFromSChain(
  ...
  try {
    ...
    const erc20AddressSChain =
      joErc20SChain[strCoinNameErc20SChain + "_address"];
    ...
    const erc20AddressMainNet =
      joErc20MainNet[strCoinNameErc20MainNet + "_address"];
    ...
    if (imaHelperAPIs.getWaitForNextBlockOnSChain())
      await imaHelperAPIs.safeWaitForNextBlockToAppear(
        details,
        ethersProviderSChain,
      );
    ...
    const estimatedGasExitToMainERC20 =
      await transactionCustomizerSChain.computeGas(
        ...
        joTokenManagerERC20,
        "exitToMainERC20",
        arrArgumentsExitToMainERC20,
        joAccountSrc,
        strActionName,
        gasPrice,
        8000000,
        weiHowMuchExitToMainERC20,
        null,
      );
    details.trace(
      "{p}Using estimated(approve) gas={}",
      strLogPrefix,
      estimatedGasExitToMainERC20,
    );
    ...
    if (joMessageProxySChain) {
      ...
      const joEvents = await imaEventLogScan.getContractCallEvents(
        ...
        strEventName,
        joReceiptExitToMainERC20.blockNumber,
...
export async function doErc721PaymentFromSChain(
  ...
  try {
    ...
    const erc721AddressSChain =
      joErc721SChain[strCoinNameErc721SChain + "_address"];
    ...
    details.trace(
      "{p}Using estimated(transfer from) gas={}",
      strLogPrefix,
      estimatedGasApprove,
    );
    const isIgnoreApprove = false;
    ...
    const estimatedGasExitToMainERC721 =
      await transactionCustomizerSChain.computeGas(
        ...
        ethersProviderSChain,
        "TokenManagerERC721",
        joTokenManagerERC721,
        "exitToMainERC721",
        arrArgumentsExitToMainERC721,
        joAccountSrc,
        strActionName,
        gasPrice,
        8000000,
        weiHowMuchExitToMainERC721,
        null,
      );
    details.trace(
      "{p}Using estimated(exit to main) gas={}",
      strLogPrefix,
      estimatedGasExitToMainERC721,
    );
...
export async function doErc1155PaymentFromSChain(
  ...
  try {
    ...
    const erc1155AddressSChain =
      joErc1155Chain[strCoinNameErc1155SChain + "_address"];
    const tokenManagerAddress = joTokenManagerERC1155.address;
    const contractERC1155 = new owaspUtils.ethersMod.ethers.Contract(
      erc1155AddressSChain,
      erc1155ABI,
      ethersProviderSChain,
    );
    ...
    const estimatedGasExitToMainERC1155 =
      await transactionCustomizerSChain.computeGas(
        ...
        ethersProviderSChain,
        "TokenManagerERC1155",
        joTokenManagerERC1155,
        "exitToMainERC1155",
        arrArgumentsExitToMainERC1155,
        joAccountSrc,
        strActionName,
        gasPrice,
        8000000,
        weiHowMuchExitToMainERC1155,
        null,
      );
    details.trace(
      "{p}Using estimated(exit to main) gas={}",
      strLogPrefix,
      estimatedGasExitToMainERC1155,
    );
...
export async function doErc1155BatchPaymentFromSChain(
  ...
  try {
    ...
    strActionName =
      "ERC1155 batch-payment from S-Chain, exitToMainERC1155Batch";
    ...
    const estimatedGasExitToMainERC1155Batch =
      await transactionCustomizerSChain.computeGas(
        ...
        "TokenManagerERC1155",
        joTokenManagerERC1155,
        "exitToMainERC1155Batch",
        arrArgumentsExitToMainERC1155Batch,
        joAccountSrc,
        strActionName,
        gasPrice,
        8000000,
        weiHowMuchExitToMainERC1155Batch,
        null,
      );
    details.trace(
      "{p}Using estimated(exit to main) gas={}",
      strLogPrefix,
      estimatedGasExitToMainERC1155Batch,
    );
    ...
    const strErrorOfDryRunExitToMainERC1155Batch = await imaTx.dryRunCall(
      ...
      joAccountSrc,
      strActionName,
      isIgnoreExitToMainERC1155Batch,
      gasPrice,
      estimatedGasExitToMainERC1155Batch,
      weiHowMuchExitToMainERC1155Batch,
      null,
    ...
    details.exposeDetailsTo(
      log.globalStream(),
      "doErc1155BatchPaymentFromSChain",
      false,
    );
    details.close();
    return false;
...
export async function doErc20PaymentS2S(
  ...
  try {
    ...
    if (!strChainNameDst) throw new Error("No destination chain name provided");
    if (!joAccountSrc) throw new Error("No account or sign TX way provided");
    if (!strCoinNameErc20Src)
      throw new Error("Need full source ERC20 information, like ABI");
    if (!joSrcErc20) throw new Error("No source ERC20 ABI provided");
    if (isReverse) {
      if (!ercDstAddress20)
        throw new Error("No destination ERC20 address provided");
    }
    if (!tc) throw new Error("No transaction customizer provided");
    const ercSrcAbi20 = joSrcErc20[strCoinNameErc20Src + "_abi"];
    const ercSrcAddress20 = joSrcErc20[strCoinNameErc20Src + "_address"];
    details.trace(
      "{p}Token Manager ERC20 address on source chain....{}",
      strLogPrefix,
      joTokenManagerERC20Src.address,
    );
    details.trace(
      "{p}Source ERC20 coin name.........................{}",
      strLogPrefix,
      strCoinNameErc20Src,
    );
    details.trace(
      "{p}Source ERC20 token address.....................{}",
      strLogPrefix,
      ercSrcAddress20,
    );
    if (isReverse || ercDstAddress20) {
      details.trace(
        "{p}Destination ERC20 token address................{}",
        strLogPrefix,
        ercDstAddress20,
      );
    }
    details.trace(
      "{p}Destination chain name.........................{}",
      strLogPrefix,
      strChainNameDst,
    );
    details.trace(
      "{p}Amount of tokens to transfer...................{}",
      strLogPrefix,
      nAmountOfToken,
    );
    ...
    const arrArgumentsApprove = [
      ...
      owaspUtils.ensureStartsWith0x(
        owaspUtils.toBN(nAmountOfToken).toHexString(),
      ),
    ];
    ...
    const estimatedGasTransfer = await tc.computeGas(
      ...
      arrArgumentsTransfer,
      joAccountSrc,
      strActionName,
      gasPrice,
      8000000,
      weiHowMuchTransferERC20,
      null,
    );
    details.trace(
      "{p}Using estimated(transfer) gas={}",
      strLogPrefix,
      estimatedGasTransfer,
    );
    ...
    details.exposeDetailsTo(
      log.globalStream(),
      `doErc20PaymentS2S/${isForward ? "forward" : "reverse"}`,
      false,
    );
    details.close();
    return false;
...
export async function doErc721PaymentS2S(
  ...
  try {
    ...
    if (!joSrcErc721) throw new Error("No source ERC721 ABI provided");
    if (isReverse) {
      if (!ercDstAddress721)
    ...
    details.trace(
      "{p}Token Manager ERC721 address on source chain....{}",
      strLogPrefix,
      joTokenManagerERC721Src.address,
    );
    details.trace(
      "{p}Source ERC721 coin name.........................{}",
      strLogPrefix,
      strCoinNameErc721Src,
    );
    details.trace(
      "{p}Source ERC721 token address.....................{}",
      strLogPrefix,
      ercSrcAddress721,
    );
    if (isReverse || ercDstAddress721) {
      details.trace(
        "{p}Destination ERC721 token address................{}",
        strLogPrefix,
        ercDstAddress721,
      );
    }
    ...
    if (joReceiptApprove) {
      jarrReceipts.push({
        description: `doErc721PaymentS2S/approve/${isForward ? "forward" : "reverse"}`,
        receipt: joReceiptApprove,
      });
    }
...
export async function doErc1155PaymentS2S(
  ...
  try {
    ...
    if (!joSrcErc1155) throw new Error("No source ERC1155 ABI provided");
    if (isReverse) {
    ...
    details.trace(
      "{p}Token Manager ERC1155 address on source chain....{}",
      strLogPrefix,
      joTokenManagerERC1155Src.address,
    );
    details.trace(
      "{p}Source ERC1155 coin name.........................{}",
      strLogPrefix,
      strCoinNameErc1155Src,
    );
    details.trace(
      "{p}Source ERC1155 token address.....................{}",
      strLogPrefix,
      ercSrcAddress1155,
    );
    if (isReverse || ercDstAddress1155) {
      details.trace(
        "{p}Destination ERC1155 token address................{}",
        strLogPrefix,
        ercDstAddress1155,
      );
    }
    ...
    if (joReceiptApprove) {
      jarrReceipts.push({
        description: `doErc1155PaymentS2S/approve/${isForward ? "forward" : "reverse"}`,
        receipt: joReceiptApprove,
      });
    }
    ...
    details.exposeDetailsTo(
      log.globalStream(),
      `doErc1155PaymentS2S/${isForward ? "forward" : "reverse"}`,
      false,
    );
    details.close();
    return false;
...
export async function doErc1155BatchPaymentS2S(
  ...
  try {
    strActionName = `validateArgs/doErc1155BatchPaymentS2S/${isForward ? "forward" : "reverse"}`;
    if (!ethersProviderSrc)
      throw new Error("No provider for source of transfer");
    if (!strChainNameDst) throw new Error("No destination chain name provided");
    if (!joAccountSrc) throw new Error("No account or sign TX way provided");
    ...
    details.trace(
      "{p}Token IDs to transfer..........................{}",
      strLogPrefix,
      arrTokenIds,
    );
    details.trace(
      "{p}Amounts of tokens to transfer..................{}",
      strLogPrefix,
      arrTokenAmounts,
    );
    ...
    if (joReceiptApprove) {
      jarrReceipts.push({
        description: `doErc1155BatchPaymentS2S/approve/${isForward ? "forward" : "reverse"}`,
        receipt: joReceiptApprove,
      });
    }
    ...
    details.exposeDetailsTo(
      log.globalStream(),
      `doErc1155BatchPaymentS2S/${isForward ? "forward" : "reverse"}`,
      false,
    );
    details.close();
    return false;
...
export async function mintErc20(
  ...
  try {
    ...
    if (
      !(
        strAddressMintTo.length > 0 &&
        strTokenContractAddress.length > 0 &&
        joTokenContractABI
      )
    )
  ...
  try {
    ...
    if (
      !(
        ...
        joAccount &&
        strAddressMintTo &&
        typeof strAddressMintTo === "string" &&
        strAddressMintTo.length > 0 &&
        strTokenContractAddress &&
        typeof strTokenContractAddress === "string" &&
        strTokenContractAddress.length > 0 &&
        joTokenContractABI
      )
    )
...
export async function mintErc1155(
  ...
  try {
    details.debug(
      "{p}Mint ERC1155 token ID {} token amount {}",
      strLogPrefix,
      idToken,
      nAmount,
    );
  ...
  try {
    ...
    if (
      !(
        ...
        joAccount &&
        strAddressBurnFrom &&
        typeof strAddressBurnFrom === "string" &&
...
export async function burnErc1155(
  ...
  try {
    details.debug(
      "{p}Burn ERC1155 token ID {} token amount {}",
      strLogPrefix,
      idToken,
      nAmount,
    );
