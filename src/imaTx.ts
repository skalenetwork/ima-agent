...
export function dryRunEnable(isEnable: any): boolean {
  gFlagDryRunIsEnabled =
    isEnable != null && isEnable != undefined ? !!isEnable : true;
  return !!gFlagDryRunIsEnabled;
}
...
export function dryRunIgnore(isIgnored: boolean): boolean {
  gFlagDryRunIsIgnored =
    isIgnored != null && isIgnored != undefined ? !!isIgnored : true;
  return !!gFlagDryRunIsIgnored;
}
...
export async function dryRunCall(
  ...
  strContractName: string,
  joContract: owaspUtils.ethersMod.ethers.Contract,
  strMethodName: string,
  arrArguments: any[],
  joAccount: state.TAccount,
  strActionName: string,
  isDryRunResultIgnore: boolean,
  gasPrice: any,
  gasValue: any,
  weiHowMuch?: any,
  opts?: any,
): Promise<string | null> {
  if (!dryRunIsEnabled()) return null; // success
  isDryRunResultIgnore =
    isDryRunResultIgnore != null && isDryRunResultIgnore != undefined
      ? !!isDryRunResultIgnore
      : false;
  ...
  if (arrArguments.length > 0) {
    ...
    for (let i = 0; i < arrArguments.length; ++i) {
      if (i > 0) strArgumentsDescription += log.fmtDebug(", ");
      strArgumentsDescription += log.fmtInformation("{}", arrArguments[i]);
    }
    strArgumentsDescription += log.fmtDebug(" )");
  } else strArgumentsDescription += log.fmtDebug("()");
  ...
  try {
    ...
    if (gasPrice) callOpts.gasPrice = owaspUtils.toBN(gasPrice).toHexString();
    if (gasValue) callOpts.gasLimit = owaspUtils.toBN(gasValue).toHexString();
    if (weiHowMuch) callOpts.value = owaspUtils.toBN(weiHowMuch).toHexString();
    const joDryRunResult = await joContract.callStatic[strMethodName](
      ...arrArguments,
      callOpts,
    );
    ...
    if (dryRunIsIgnored()) return null;
    return strError;
  }
}

async function payedCallPrepare(
  ...
  if (optsPayedCall.gasPrice) {
    optsPayedCall.callOpts.gasPrice = owaspUtils
      .toBN(optsPayedCall.gasPrice)
      .toHexString();
  }
  if (optsPayedCall.estimatedGas) {
    optsPayedCall.callOpts.gasLimit = owaspUtils
      .toBN(optsPayedCall.estimatedGas)
      .toHexString();
  }
  if (optsPayedCall.weiHowMuch) {
    optsPayedCall.callOpts.value = owaspUtils
      .toBN(optsPayedCall.weiHowMuch)
      .toHexString();
  }
  optsPayedCall.details.trace(
    "{p}payed-call of action {bright} will do payed-call {p} with call options {} " +
      "via {sunny}-sign-and-send using from address {}...",
    optsPayedCall.strLogPrefix,
    optsPayedCall.strActionName,
    optsPayedCall.strContractCallDescription,
    optsPayedCall.callOpts,
    optsPayedCall.joACI.strType,
    optsPayedCall.joAccount.address(),
  );
  optsPayedCall.unsignedTx = await optsPayedCall.joContract.populateTransaction[
  ...
  if (optsPayedCall.opts?.isCheckTransactionToSchain) {
    optsPayedCall.unsignedTx = await checkTransactionToSchain(
      ...
      optsPayedCall.details,
      optsPayedCall.ethersProvider,
      optsPayedCall.joAccount,
    );
  }
  optsPayedCall.details.trace(
    "{p}populated transaction: {}",
    optsPayedCall.strLogPrefix,
    optsPayedCall.unsignedTx,
  );
  ...
  optsPayedCall.txHash = owaspUtils.ethersMod.ethers.utils.keccak256(
    optsPayedCall.rawTx,
  );
  optsPayedCall.details.trace(
    "{p}transaction hash: {}",
...
async function payedCallTM(
  ...
  const txAdjusted: any = optsPayedCall.unsignedTx; // JSON.parse( JSON.stringify( optsPayedCall.rawTx ) );
  const arrNamesConvertToHex = [
    "gas",
    "gasLimit",
    "optsPayedCall.gasPrice",
    "value",
  ];
  for (let idxName = 0; idxName < arrNamesConvertToHex.length; ++idxName) {
    const strName = arrNamesConvertToHex[idxName];
    if (
      strName in txAdjusted &&
  ...
  if ("gasLimit" in txAdjusted) delete txAdjusted.gasLimit;
  if ("chainId" in txAdjusted) delete txAdjusted.chainId;
  const { chainId } = await optsPayedCall.ethersProvider.getNetwork();
  txAdjusted.chainId = chainId;
  optsPayedCall.details.trace(
    "{p}Adjusted transaction: {}",
    optsPayedCall.strLogPrefix,
    txAdjusted,
  );
  if (redis == null)
...
async function payedCallSGX(
  ...
  const strCmd =
    process.argv[0] +
    " --no-warnings ./imaSgxExternalSigner.js " +
  ...
  optsPayedCall.joReceipt = JSON.parse(
    strStdOutFromExternalInvocation.toString(),
  );
  optsPayedCall.details.trace(
    "{p}Result from external SGX signer is: {}",
    optsPayedCall.strLogPrefix,
...
function postConvertBN(jo: any, name: any): void {
  if (!jo) return;
  if (!(name in jo)) return;
  if (typeof jo[name] === "object") return;
  jo[name] = owaspUtils.toBN(jo[name]);
}

async function payedCallDirect(
  optsPayedCall: TRunTimePayedCallOptions,
): Promise<void> {
  ...
  if (
    !(chainId in optsPayedCall.unsignedTx) ||
  ...
  const joSignedTX = await ethersWallet.signTransaction(
    optsPayedCall.unsignedTx,
  );
  optsPayedCall.details.trace(
    "{p}Signed transaction: {}",
    optsPayedCall.strLogPrefix,
    joSignedTX,
  );
...
export async function payedCall(
  ...
  const strContractMethodDescription = log.fmtDebug(
    "{p}({}).{sunny}",
    optsPayedCall.strContractName,
    optsPayedCall.joContract.address,
    optsPayedCall.strMethodName,
  );
  let strArgumentsDescription = "";
  if (optsPayedCall.arrArguments.length > 0) {
    ...
    for (let i = 0; i < optsPayedCall.arrArguments.length; ++i) {
      if (i > 0) strArgumentsDescription += log.fmtDebug(", ");
      strArgumentsDescription += log.fmtInformation(
        "{}",
        optsPayedCall.arrArguments[i],
      );
    }
    strArgumentsDescription += log.fmtDebug(" )");
  } else strArgumentsDescription += log.fmtDebug("()");
  ...
  try {
    ...
    switch (optsPayedCall.joACI.strType) {
      ...
      default: {
        ...
        throw new Error(
          `${strErrorPrefix} bad credentials information specified, ` +
    ...
    throw new Error(
      `${strErrorPrefix} invoking ` +
        `the ${optsPayedCall.strContractCallDescription}, ` +
        `error is: ${owaspUtils.extractErrorMessage(err)}`,
    );
  }
  optsPayedCall.details.success(
    "{p}Done, TX was {sunny}-signed-and-sent, receipt is {}",
    optsPayedCall.strLogPrefix,
    optsPayedCall.joACI ? optsPayedCall.joACI.strType : "N/A",
    optsPayedCall.joReceipt,
  );
  try {
    const bnGasSpent = owaspUtils.toBN(
      optsPayedCall.joReceipt.cumulativeGasUsed,
    );
    ...
    optsPayedCall.details.warning(
      "{p}TX stats computation error {err}, stack is:\n{stack}",
      optsPayedCall.strLogPrefix,
      err,
      err,
    );
...
export async function checkTransactionToSchain(
  ...
  try {
    ...
    const balance = owaspUtils.toBN(
      await ethersProvider.getBalance(strFromAddress),
    );
    details.trace(
      ...
      strLogPrefix,
      strFromAddress,
      owaspUtils.toHexStringSafe(balance),
    ...
    if (balance.lt(requiredBalance)) {
      ...
      details.trace(
        "{p}Trimmed PoW-mining number is {}",
        strLogPrefix,
        powNumber,
      );
      if (!powNumber)
      ...
      details.success(
        "{p}Success, finally (after PoW-mining) modified unsigned " +
          "transaction is {}",
        strLogPrefix,
        unsignedTx,
      );
      ...
      details.success(
        "{p}Have sufficient funds for {}, PoW-mining is not needed and " +
          "will be skipped",
        strLogPrefix,
        strFromAddress,
      );
...
async function tmSend(
  ...
  details.trace(
    "TM - sending tx {} ts: {}",
    tx,
    imaHelperAPIs.currentTimestamp(),
  );
...
async function tmWait(
  ...
  details.debug(
    "{p}TM - will wait TX {} to complete for {} second(s) maximum",
    strLogPrefix,
    txId,
    nWaitSeconds,
  );
  ...
  while (
    !tmIsFinished(await tmGetRecord(txId)) &&
  ...
  if (!tmIsFinished(r) || r.status == "DROPPED") {
    details.error(
      "{p}TM - TX {} was unsuccessful, wait failed",
      strLogPrefix,
      txId,
    );
    return null;
  ...
  if (!joReceipt) {
    details.error(
      "{p}TM - TX {} was unsuccessful, failed to fetch transaction receipt",
      strLogPrefix,
      txId,
    );
    return null;
...
async function tmEnsureTransaction(
  ...
  for (; idxAttempt < cntAttempts; ++idxAttempt) {
    ...
    if (joReceipt) break;
    details.error(
      "{p}TM - unsuccessful TX {} sending attempt {} of {} receipt: {}",
      strLogPrefix,
      txId,
      idxAttempt,
      cntAttempts,
  ...
  if (!joReceipt) {
    details.error(
      "{p}TM TX {} transaction has been dropped",
      strLogPrefix,
      txId,
    );
  ...
  details.information(
    "{p}TM - successful TX {}, sending attempt {} of {}",
    strLogPrefix,
    txId,
    idxAttempt,
    cntAttempts,
...
export class TransactionCustomizer {
  ...
  constructor(gasPriceMultiplier: any, gasMultiplier: any) {
    ...
    this.gasMultiplier = gasMultiplier
      ? owaspUtils.toFloat(gasMultiplier)
      : 1.25;
  }
  ...
  async computeGasPrice(
    ...
    const gasPrice = owaspUtils.parseIntOrHex(
      owaspUtils.toBN(await ethersProvider.getGasPrice()).toHexString(),
    );
    if (
      gasPrice == 0 ||
      gasPrice == null ||
      ...
      if (gasPriceMultiplied > maxGasPrice) gasPriceMultiplied = maxGasPrice;
  ...
  async computeGas(
    ...
    estimatedGas = owaspUtils.parseIntOrHex(
      owaspUtils.toBN(estimatedGas).toString(),
    );
    if (estimatedGas == 0) {
      estimatedGas = gasValueRecommended;
      details.warning(
        "{p}Will use recommended gas {} instead of estimated",
        strLogPrefix,
        estimatedGas,
      );
    }
    if (this.gasMultiplier > 0.0) {
      estimatedGas = owaspUtils.parseIntOrHex(
        (estimatedGas * this.gasMultiplier).toString(),
      );
    }
...
export function getTransactionCustomizerForMainNet(): TransactionCustomizer {
  if (gTransactionCustomizerMainNet) return gTransactionCustomizerMainNet;
...
export function getTransactionCustomizerForSChain(): TransactionCustomizer {
  if (gTransactionCustomizerSChain) return gTransactionCustomizerSChain;
