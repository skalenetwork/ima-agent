...
export function generateTimestampString(
  ...
  isColorized = typeof isColorized === "undefined" ? true : !!isColorized;
  if (isColorized) isColorized = cc.isEnabled();
  ts = ts instanceof Date ? ts : new Date();
  const ccDate = function (x?: any): string {
    return isColorized ? cc.date(x) : x;
  };
  const ccTime = function (x?: any): string {
    return isColorized ? cc.time(x) : x;
  };
  const ccFractionPartOfTime = function (x?: any): string {
    return isColorized ? cc.fracTime(x) : x;
  };
  const ccBright = function (x?: any): string {
    return isColorized ? cc.bright(x) : x;
  };
  const s =
    ccDate(n2s(ts.getUTCFullYear(), 4)) +
    ccBright("-") +
    ccDate(n2s(ts.getUTCMonth() + 1, 2)) +
    ccBright("-") +
    ccDate(n2s(ts.getUTCDate(), 2)) +
    " " +
    ccTime(n2s(ts.getUTCHours(), 2)) +
    ccBright(":") +
    ccTime(n2s(ts.getUTCMinutes(), 2)) +
    ccBright(":") +
    ccTime(n2s(ts.getUTCSeconds(), 2)) +
    ccBright(".") +
    ccFractionPartOfTime(n2s(ts.getUTCMilliseconds(), 3));
  return s;
}

export function generateTimestampPrefix(
  ts?: any,
  isColorized?: boolean,
): string {
...
export function createStandardOutputStream(): TLogger | null {
  try {
    const objEntry: TLogger = {
      ...
      rotate: function (nBytesToWrite: number) {},
      toString: function (): string {
        return this.strPath.toString();
      },
      exposeDetailsTo: function (
        otherStream: TLogger,
        strTitle: string,
        isSuccess: boolean,
      ): void {},
      ...
      attention: function (...args: any[]): void {
        if (verboseGet() >= verboseName2Number("attention")) {
          this.write(getLogLinePrefixAttention() + fmtAttention(...args));
        }
      },
      information: function (...args: any[]): void {
        if (verboseGet() >= verboseName2Number("information")) {
          this.write(getLogLinePrefixInformation() + fmtInformation(...args));
        }
      },
  ...
  try {
    const objEntry: TLoggerMemory = {
      ...
      isBeginningOfAccumulatedLog: function (): boolean {
        if (this.arrAccumulatedLogTextLines.length == 0) return true;
        return false;
      },
      isLastLineEndsWithCarriageReturn: function (): boolean {
        if (this.arrAccumulatedLogTextLines.length == 0) return false;
        const s =
          this.arrAccumulatedLogTextLines[
      ...
      write: function (...args: any[]): void {
        ...
        for (let i = 0; i < arr.length; ++i) {
          ...
          let strHeader = "";
          if (
            this.isLastLineEndsWithCarriageReturn() ||
...
export function createFileOutput(
  ...
  try {
    const objEntry: TLogger = {
      ...
      close: function (): void {
        if (!this.objStream) return;
        fs.closeSync(this.objStream);
        this.objStream = null;
      },
      open: function (): void {
        this.objStream = fs.openSync(
          this.strPath,
          "a",
          fs.constants.O_NONBLOCK | fs.constants.O_RDWR,
        );
      },
      size: function (): number {
        try {
          return fs.lstatSync(this.strPath).size;
        } catch (err) {
          return 0;
        }
      },
      rotate: function (nBytesToWrite: number) {
        try {
          ...
          let i = 0;
          const cnt = cc.toInteger(this.nMaxFilesCount);
          for (i = 0; i < cnt; ++i) {
            const j = this.nMaxFilesCount - i - 1;
            const strPath = this.strPath.toString() + (j === 0 ? "" : "." + j);
            if (j == cnt - 1) {
              try {
            ...
            const strPathPrev = this.strPath.toString() + "." + (j + 1);
            try {
              fs.unlinkSync(strPathPrev);
            } catch (err) {}
            try {
              fs.renameSync(strPath, strPathPrev);
            } catch (err) {}
          }
        } catch (err) {}
        try {
    ...
    console.log(
      "CRITICAL ERROR: Failed to open file system log stream for " +
        strFilePath +
        ", error is " +
        JSON.stringify(err),
    );
...
export function insertFileOutput(
  ...
  if (objEntry !== null) return true;
  objEntry = createFileOutput(
    strFilePath,
    nMaxSizeBeforeRotation,
    nMaxFilesCount,
  );
  if (!objEntry) return false;
  gArrStreams.push(objEntry);
...
export function extractErrorMessage(
  jo?: any,
  strDefaultErrorText?: string,
): string {
  strDefaultErrorText =
    strDefaultErrorText ?? "unknown error or error without a description";
  if (!jo) return strDefaultErrorText;
  try {
...
function tryToSplitFormatString(
  strFormat?: string,
  cntArgsMax?: number,
): any[] | null {
  if (!(strFormat && typeof strFormat === "string")) return null;
  if (!cntArgsMax) cntArgsMax = 0;
  const arrParts: any[] = [];
  let s = strFormat;
  let cntFoundArgs = 0;
  for (; true; ) {
    if (cntFoundArgs >= cntArgsMax) break; // nothing to do split for
    ...
    if (strPart.length > 0) arrParts.push({ type: "text", text: strPart });
    arrParts.push({ type: "arg", text: strArgDesc });
    ++cntFoundArgs;
    if (s.length == 0) break;
  }
  if (cntFoundArgs == 0) return null;
  if (s.length > 0) arrParts.push({ type: "text", text: s });
  return arrParts;
}
...
export function fmtArgumentsArray(arrArgs: any[], fnFormatter?: any): string {
  ...
  const fnDefaultOneArgumentFormatter = function (
    arg?: any,
    fnCustomFormatter?: any,
  ): string {
    if (!fnCustomFormatter) fnCustomFormatter = fnFormatter;
    const t = typeof arg;
    if (t == "string") {
      if (arg.length > 0) {
        if (arg == " " || arg == "\n") {
    ...
    } else return cc.logArgToString(arg);
    return arg;
  };
  const fnFormatOneArgument = function (arg: any, fmt?: any): string {
    if (!arg) return arg;
    if (arg == " " || arg == "\n") return arg;
    if (!isValueMode) return fnDefaultOneArgumentFormatter(arg, null);
    if (fmt && typeof "fmt" === "string") {
      if (fmt == "raw") return arg;
      if (fmt == "p") return fnDefaultOneArgumentFormatter(arg, null);
      if (fmt == "url") return u(arg);
      if (fmt == "yn") return yn(arg);
      if (fmt == "oo") return onOff(arg);
      if (fmt == "stack") return stack(arg);
      if (fmt == "em") return em(arg);
      if (fmt == "err") return em(extractErrorMessage(arg));
      if (fmt == "bright") return fnDefaultOneArgumentFormatter(arg, cc.bright);
      if (fmt == "sunny") return fnDefaultOneArgumentFormatter(arg, cc.sunny);
      if (fmt == "rainbow")
    ...
    for (let i = 0; i < gArrStreams.length; ++i) {
      try {
        ...
        if (
          objEntry &&
          "write" in objEntry &&
          typeof objEntry.write === "function"
        )
          objEntry.write(s);
      } catch (err) {}
    }
...
export function write(...args: any[]): void {
  let s: string = getPrintTimestamps()
    ? generateTimestampPrefix(null, true)
    : "";
  s += fmtArgumentsArray(args);
...
export function add(
  ...
  if (!nMaxSizeBeforeRotation) nMaxSizeBeforeRotation = 0;
  if (!nMaxFilesCount) nMaxFilesCount = 0;
...
export function verboseReversed(): Map<string, number> {
  if (!gMapReversedVerbose) gMapReversedVerbose = computeVerboseAlias();
  return gMapReversedVerbose;
}
export function verboseLevelAsTextForLog(vl: any): string {
  if (typeof vl === "undefined") vl = verboseGet();
...
export function verboseName2Number(s: string): number {
  ...
  if (typeof n === "undefined") return 9;
  return n;
}
...
export function verboseParse(s: string): number {
  ...
  try {
    ...
    if (isNumbersOnly) n = cc.toInteger(s);
    else {
...
export function posNeg(
  ...
  return condition
    ? cc.isStringAlreadyColorized(strPositive)
      ? strPositive
      : cc.success(strPositive)
    : cc.isStringAlreadyColorized(strNegative)
      ? strNegative
      : cc.error(strNegative);
}
