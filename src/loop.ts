...
export function checkTimeFraming(
  ...
  try {
    ...
    if (aDateTime == null || aDateTime == undefined) aDateTime = new Date(); // now
    ...
    const nSecondsRangeForAllSChains =
      imaState.nTimeFrameSeconds * imaState.nNodesCount;
    ...
    const nFrameStart =
      nRangeStart + imaState.nNodeNumber * imaState.nTimeFrameSeconds;
    const nGapStart =
      nFrameStart + imaState.nTimeFrameSeconds - imaState.nNextFrameGap;
    if (!bSkip) {
    ...
    let strFrameInfo = log.fmtDebug(
      ...
      "    All Chains Range",
      "...........",
      nSecondsRangeForAllSChains,
      "\n",
      "    S-Chain Range Mod",
      "..........",
      log.fmtInformation("{}", nMod),
      "\n",
    ...
    strFrameInfo += log.fmtDebug(
      ...
      "    Range Start",
      "................",
      log.fmtInformation("{}", nRangeStart),
      "\n",
      "    Frame Start",
      "................",
      log.fmtInformation("{}", nFrameStart),
      "\n",
      "    Gap Start",
      "..................",
      log.fmtInformation("{}", nGapStart),
      "\n",
    );
    log.write(strFrameInfo);
    if (bSkip) return false;
  } catch (err) {
    log.error(
      "Exception in time framing check in {}: {err}, stack is:{}{stack}",
...
async function singleTransferLoopPartOracle(
  ...
  if (optsLoop.enableStepOracle && imaOracleOperations.getEnabledOracle()) {
    log.notice(
      "{p}Will invoke Oracle gas price setup in {}...",
      strLogPrefix,
      threadInfo.threadDescription(),
    );
    try {
      if (!(await pwa.checkOnLoopStart(imaState, "oracle"))) {
        ...
        log.notice(
          "{p}Skipped(oracle) in {} due to cancel mode reported from PWA",
          strLogPrefix,
          threadInfo.threadDescription(),
        );
      } else {
        if (checkTimeFraming(null, "oracle", optsLoop.joRuntimeOpts)) {
          ...
          log.notice(
            "{p}Skipped(oracle) in {} due to time framing check",
            strLogPrefix,
            threadInfo.threadDescription(),
          );
        }
      ...
      log.error(
        "{p}Oracle operation exception: {} in {err}, stack is:{}{stack}",
        strLogPrefix,
        err,
        threadInfo.threadDescription(),
        "\n",
    ...
    log.information(
      "{p}Oracle gas price setup done in {}: {}",
      strLogPrefix,
      threadInfo.threadDescription(),
      b0,
    );
  ...
  if (optsLoop.enableStepM2S) {
    ...
    try {
      if (!(await pwa.checkOnLoopStart(imaState, "m2s"))) {
        ...
        log.notice(
          "{p}Skipped(m2s) in {} due to cancel mode reported from PWA",
          strLogPrefix,
          threadInfo.threadDescription(),
        );
      } else {
        if (checkTimeFraming(null, "m2s", optsLoop.joRuntimeOpts)) {
          ...
          log.notice(
            "{p}Skipped(m2s) in {} due to time framing check",
            strLogPrefix,
            threadInfo.threadDescription(),
          );
        }
      ...
      log.error(
        "{p}M2S transfer exception in {}: {err}, stack is:{}{stack}",
        strLogPrefix,
        threadInfo.threadDescription(),
        err,
        "\n",
  ...
  if (optsLoop.enableStepS2M) {
    ...
    try {
      if (!(await pwa.checkOnLoopStart(imaState, "s2m"))) {
        ...
        log.notice(
          "{p}Skipped(s2m) in {} due to cancel mode reported from PWA",
          strLogPrefix,
          threadInfo.threadDescription(),
        );
      } else {
        if (checkTimeFraming(null, "s2m", optsLoop.joRuntimeOpts)) {
          ...
          log.notice(
            "{p}Skipped(s2m) in {} due to time framing check",
            strLogPrefix,
            threadInfo.threadDescription(),
          );
        }
      ...
      log.error(
        "{p}S2M transfer exception in {err}: , stack is:{}{stack}",
        strLogPrefix,
        threadInfo.threadDescription(),
        err,
        "\n",
...
async function singleTransferLoopPartS2S(
  ...
  if (optsLoop.enableStepS2S && imaState.optsS2S.isEnabled) {
    ...
    try {
      ...
      log.error(
        "{p}S2S transfer exception in {}: {err}, stack is:{}{stack}",
        strLogPrefix,
        threadInfo.threadDescription(),
        err,
        "\n",
...
function printLoopPartSkippedWarning(strLoopPartName: string): void {
  log.warning(
    "Skipped {} transfer loop part due to other single transfer loop is in " +
      "progress right now",
    strLoopPartName,
  );
}
  ...
  try {
    ...
    if (optsLoop.enableStepOracle) {
      if (imaState.loopState.oracle.isInProgress) {
        ...
        b0 = true;
      } else b0 = await singleTransferLoopPartOracle(optsLoop, strLogPrefix);
    } else b0 = true;
    // M2S loop part:
    if (optsLoop.enableStepM2S) {
      if (imaState.loopState.m2s.isInProgress) {
        ...
        b1 = true;
      } else b1 = await singleTransferLoopPartM2S(optsLoop, strLogPrefix);
    } else b1 = true;
    // S2M loop part:
    if (optsLoop.enableStepS2M) {
      if (imaState.loopState.s2m.isInProgress) {
        ...
        b2 = true;
      } else b2 = await singleTransferLoopPartS2M(optsLoop, strLogPrefix);
    } else b2 = true;
    // S2S loop part:
    if (optsLoop.enableStepS2S) {
      if (imaState.loopState.s2s.isInProgress) {
        ...
        b3 = true;
      } else b3 = await singleTransferLoopPartS2S(optsLoop, strLogPrefix);
    } else b3 = true;
    // Final status check loop part:
    const bResult = b0 && b1 && b2 && b3;
    log.notice(
      "{p}Final completion status for all performed transfer loop parts is {}",
      strLogPrefix,
      bResult,
    );
    return bResult;
    ...
    log.error(
      "{p}Exception in transfer loop: {err}, stack is:{}{stack}",
      strLogPrefix,
      err,
      "\n",
      err,
...
export async function runTransferLoop(
  optsLoop: TLoopOptions,
): Promise<boolean> {
  const imaState: state.TIMAState = state.get();
  ...
  } else await singleTransferLoopWithRepeat(optsLoop);
  return true;
}
...
export async function ensureHaveWorkers(
  ...
  if (gArrWorkers.length > 0) return gArrWorkers;
  const cntWorkers = 2;
  log.debug(
    "Loop module will create its ",
    cntWorkers,
    " worker(s) in ",
    threadInfo.threadDescription(),
    "...",
  ...
  for (let idxWorker = 0; idxWorker < cntWorkers; ++idxWorker) {
    ...
    gArrWorkers.push(
      new threadInfo.Worker(path.join(__dirname, "loopWorker.js"), {
        // "type": "module",
        workerData,
      }),
    );
      ...
      message: {
        opts: {
          imaState: {
            ...
            isShowReimbursementBalance:
              opts.imaState.isShowReimbursementBalance,
            ...
            optsS2S: {
              ...
              isEnabled: true,
              strNetworkBrowserPath:
                opts.imaState.optsS2S.strNetworkBrowserPath,
            },
...
export async function runParallelLoops(
  opts: TParallelLoopRunOptions,
): Promise<boolean> {
  log.notice(
    "Will start parallel IMA transfer loops in {}...",
    threadInfo.threadDescription(),
  );
  await ensureHaveWorkers(opts);
  log.success(
    "Done, did started parallel IMA transfer loops in {}, have {} worker(s) and {} " +
      "clients(s).",
    threadInfo.threadDescription(),
    gArrWorkers.length,
    gArrClients.length,
...
export async function spreadArrivedStateOfPendingWorkAnalysis(
  ...
  if (
    !(
      joMessage &&
      typeof joMessage === "object" &&
      "method" in joMessage &&
      joMessage.method == "skale_imaNotifyLoopWork"
    )
  )
    return;
...
export async function spreadUpdatedSChainNetwork(
  isFinal: boolean,
): Promise<void> {
  const imaState: state.TIMAState = state.get();
  const joMessage: any = {
