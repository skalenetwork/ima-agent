...
class ObserverServer extends SocketServer {
  ...
  constructor(acceptor: any) {
    ...
    self.mapApiHandlers.init = function (
      joMessage: any,
      joAnswer: any,
      eventData: any,
      socket: any,
      ...
      if (self.initComplete) return joAnswer;
      ...
      imaTransferErrorHandling.saveTransferEvents.on(
        "error",
      ...
      self.opts.imaState.chainProperties.mn.joAccount.address =
        function (): string {
          return owaspUtils.fnAddressImpl_(this);
        };
      ...
      if (
        self.opts.imaState.chainProperties.mn.strURL &&
        ...
        self.warning(
          "WARNING: No Main-net URL specified in command line arguments(needed for " +
            "particular operations only) in {}",
          threadInfo.threadDescription(),
        );
      }

      if (
        self.opts.imaState.chainProperties.sc.strURL &&
      ...
      /* await */
      loop
        .runTransferLoop(self.opts.imaState.optsLoop)
        .then(function (): void {})
        .catch(function (): void {});
      self.information(
        "Full init compete for in-worker IMA loop {} in {}",
        workerData.url,
        threadInfo.threadDescription(),
      );
      return joAnswer;
    };
    self.mapApiHandlers.spreadUpdatedSChainNetwork = function (
      joMessage: any,
      joAnswer: any,
      eventData: any,
      socket: any,
      ...
      self.debug(
        "New own S-Chains network information is arrived to {} loop worker " +
          "in {}: {}, this own S-Chain update is {}",
        workerData.url,
        threadInfo.threadDescription(),
        joMessage.joSChainNetworkInfo,
        log.posNeg(joMessage.isFinal, "final", "partial"),
      );
      imaState.joSChainNetworkInfo = joMessage.joSChainNetworkInfo;
    };
    self.mapApiHandlers.skale_imaNotifyLoopWork = function (
      joMessage: any,
      joAnswer: any,
      eventData: any,
      socket: any,
      ...
      self.initLogMethods();
      pwa
        .handleLoopStateArrived(
          // NOTICE: no await here, executed async
          imaState,
  ...
  initLogMethods(): void {
    ...
    self.notice = function (...args: any[]) {
      if (log.verboseGet() >= log.verboseName2Number("notice")) {
        self.log(log.getLogLinePrefixNotice() + log.fmtNotice(...args));
      }
    };
    self.note = function (...args: any[]) {
      if (log.verboseGet() >= log.verboseName2Number("notice")) {
        self.log(log.getLogLinePrefixNote() + log.fmtNote(...args));
      }
    };
    self.debug = function (...args: any[]) {
      if (log.verboseGet() >= log.verboseName2Number("debug")) {
        self.log(log.getLogLinePrefixDebug() + log.fmtDebug(...args));
      }
    };
    self.trace = function (...args: any[]) {
      if (log.verboseGet() >= log.verboseName2Number("trace")) {
        self.log(log.getLogLinePrefixTrace() + log.fmtTrace(...args));
      }
    };
    self.success = function (...args: any[]) {
      if (log.verboseGet() >= log.verboseName2Number("information")) {
        self.log(log.getLogLinePrefixSuccess() + log.fmtSuccess(...args));
      }
    };
  }
}
...
server.on("dispose", function (): void {
  ...
  self.debug(
    "Disposed in-worker in {} IMA loop {}",
    threadInfo.threadDescription(),
    workerData.url,
  );
});
