...
process
  .on("unhandledRejection", function (reason: any, p: any): void {
    log.fatal(
...
function parseCommandLine(): void {
  ...
  if (imaState.isShowReimbursementBalance) {
    ...
    log.trace(
      "Will require reimbursement chain name to show reimbursement balance",
    );
  ...
  if (imaState.isReimbursementEstimate) {
    haveReimbursementCommands = true;
    log.trace(
      "Will require reimbursement chain name to do reimbursement estimation",
    );
  ...
  if (imaState.nReimbursementRecharge) {
    haveReimbursementCommands = true;
    log.trace(
      "Will require reimbursement chain name to do reimbursement recharge",
    );
  ...
  if (imaState.nReimbursementWithdraw) {
    haveReimbursementCommands = true;
    log.trace(
      "Will require reimbursement chain name to do reimbursement withdraw",
    );
  ...
  if (haveReimbursementCommands) {
    if (imaState.strReimbursementChain == "") {
      log.fatal(
        "Runtime init error: missing value for reimbursement-chain parameter, " +
  ...
  if (imaState.nAutoExitAfterSeconds > 0) {
    log.warning(
      "Automatic exit after {} second(s) is requested.",
      imaState.nAutoExitAfterSeconds,
    );
    const iv = owaspUtils.setInterval2(function (): void {
      log.warning(
        "Performing automatic exit after {} second(s)...",
        imaState.nAutoExitAfterSeconds,
      );
      owaspUtils.clearInterval2(iv);
      process.exit(0);
    }, imaState.nAutoExitAfterSeconds * 1000);
  } else log.warning("Automatic exit was not requested, skipping it.");
  ...
  if (imaState.isPrintSecurityValues) {
    log.information(
      "Agent was started with {} command line argument(s) as: {}",
      process.argv.length,
      strPrintedArguments,
    );
  }
...
function initMonitoringServer(): void {
  const imaState: state.TIMAState = state.get();
  if (imaState.nMonitoringPort <= 0) return;
  const strLogPrefix = "Monitoring: ";
  if (imaState.bLogMonitoringServer) {
    log.trace(
      "{p}Will start monitoring WS server on port {}",
      strLogPrefix,
      imaState.nMonitoringPort,
    );
  }
  try {
    gServerMonitoringWS = new ws.WebSocketServer({
      port: imaState.nMonitoringPort,
    });
  } catch (err) {
    log.error(
      "Failed start monitoring WS server on port {}, error is: {err}",
      imaState.nMonitoringPort,
      err,
    );
  }
  gServerMonitoringWS.on("connection", function (wsPeer: any, req: any): void {
    ...
    wsPeer.on("message", function (message: any): void {
      ...
      try {
        ...
        switch (joMessage.method) {
          ...
          case "get_runtime_params":
            {
              ...
              for (const paramName of arrRuntimeParamNames) {
                if (paramName in imaState)
                  joAnswer.runtime_params[paramName] = (imaState as any)[
                    paramName
                  ];
              }
            }
          ...
          case "get_last_transfer_errors":
            joAnswer.last_transfer_errors =
              imaTransferErrorHandling.getLastTransferErrors(
                !!(
        ...
        log.error(
          "{p}Bad message from {}: {}, error is: {err}, stack is:\n{stack}",
          strLogPrefix,
          ip,
          message,
          err,
      ...
      try {
        ...
        log.error(
          "{p}Failed to sent answer to {}, error is: {err}, stack is:\n{stack}",
          strLogPrefix,
          ip,
          err,
          err,
...
function initJsonRpcServer(): void {
  if (gExpressJsonRpcAppIMA) return;
  const imaState: state.TIMAState = state.get();
  if (imaState.nJsonRpcPort <= 0) return;
  ...
  const postHandler = async function (
    ...
    req.on("error", function () {
      log.error(
        "IMA-to-IMA peer {} connection error, cannot process request",
        ip,
      );
    });
    res.on("error", function () {
      log.error(
        "IMA-to-IMA peer {} connection error, cannot send responses",
        ip,
      );
    });
    ...
    try {
      ...
      if (!("method" in joMessage))
        throw new Error('"method" field was not specified');
      joAnswer.method = joMessage.method;
      if (!("id" in joMessage)) throw new Error('"id" field was not specified');
      if ("id" in joMessage) joAnswer.id = joMessage.id;
      if ("method" in joMessage) joAnswer.method = joMessage.method.toString();
      switch (joMessage.method) {
        case "echo":
        ...
        case "skale_imaNotifyLoopWork":
          if (
            await pwa.handleLoopStateArrived(
              imaState,
    ...
    if (!isSkipMode) fnSendAnswer(joAnswer);
  };
...
async function doTheJob(): Promise<void> {
  ...
  for (idxAction = 0; idxAction < cntActions; ++idxAction) {
    ...
    log.debug(
      "{p}Will execute action: {bright} ({} of {})",
      strLogPrefix,
      joAction.name,
      idxAction + 1,
      cntActions,
    ...
    try {
      ...
      ++cntFalse;
      log.critical(
        "{p}Exception occurred while executing action: {err}, stack is:\n{stack}",
        strLogPrefix,
        err,
        err,
      );
  ...
  log.information(
    ...
    strLogPrefix,
    cntTrue,
    log.fmtSuccess(" task(s) succeeded"),
  );
  log.information(
    "{p}{}{}",
    strLogPrefix,
    cntFalse,
    log.fmtError(" task(s) failed"),
  );
  log.information("{p}{p}", strLogPrefix, imaHelperAPIs.longSeparator);
  process.exitCode = cntFalse > 0 ? cntFalse : 0;
  if (!state.isPreventExitAfterLastAction()) process.exit(process.exitCode);
}
...
function handleFirstSChainDiscoveryAttemptDone(
  ...
  fnOnPeriodicDiscoveryResultAvailable: any,
): void {
  if (err) {
  ...
  discoveryTools
    .continueSChainDiscoveryInBackgroundIfNeeded(
      ...
      function (): void {
        discoveryTools
          .doPeriodicSChainNetworkDiscoveryIfNeeded(
            isSilentReDiscovery,
...
async function main(): Promise<void> {
  log.autoEnableColorizationFromCommandLineArgs();
  const imaState: state.TIMAState = state.get();
  const strTmpAddressFromEnvMainNet = owaspUtils.toEthPrivateKey(
    process.env.ACCOUNT_FOR_ETHEREUM,
  );
  const strTmpAddressFromEnvSChain = owaspUtils.toEthPrivateKey(
    process.env.ACCOUNT_FOR_SCHAIN,
  );
  const strTmpAddressFromEnvSChainTarget = owaspUtils.toEthPrivateKey(
    process.env.ACCOUNT_FOR_SCHAIN_TARGET,
  );
  if (
    strTmpAddressFromEnvMainNet &&
    typeof strTmpAddressFromEnvMainNet === "string" &&
    strTmpAddressFromEnvMainNet.length > 0
  )
    imaState.chainProperties.mn.joAccount.address_ =
      strTmpAddressFromEnvMainNet.toString();
  if (
    strTmpAddressFromEnvSChain &&
    typeof strTmpAddressFromEnvSChain === "string" &&
    strTmpAddressFromEnvSChain.length > 0
  )
    imaState.chainProperties.sc.joAccount.address_ =
      strTmpAddressFromEnvSChain.toString();
  if (
    strTmpAddressFromEnvSChainTarget &&
  ...
  const fnOnPeriodicDiscoveryResultAvailable = function (
    isFinal: boolean,
  ): void {
    loop
      .spreadUpdatedSChainNetwork(isFinal)
      .then(function (): void {})
      .catch(function (): void {});
  };
  if (imaState.bSignMessages) {
    ...
    log.information(
      "S-Chain network was discovery uses {} mode",
      isSilentReDiscovery
