...
export function findSChainIndexInArrayByName(
  arrSChains: TSChainInformation[],
  strSChainName: string,
): number {
  for (let idxSChain = 0; idxSChain < arrSChains.length; ++idxSChain) {
    const joSChain = arrSChains[idxSChain];
    if (joSChain.name.toString() == strSChainName.toString()) return idxSChain;
  }
  return -1;
}
...
export function autoUpdateLastCachedSChains(): boolean {
  ...
  if (!imaState.optsS2S.strNetworkBrowserPath) return false;
  const jo: TSChainsInformation = imaUtils.jsonFileLoad(
...
export function getLastCachedSChains(): TSChainInformation[] {
  ...
  if (!gConnectedChainsData) return [];
  return JSON.parse(JSON.stringify(gConnectedChainsData.schains));
}

export function pickRandomSChainNodeIndex(
  joSChain: TSChainInformation,
): number {
  const min = 0;
  const max = joSChain.nodes.length - 1;
  const idxNode = Math.floor(Math.random() * (max - min + 1)) + min;
  return idxNode;
}
export function pickRandomSChainNode(
  joSChain: TSChainInformation,
): TSChainNode {
