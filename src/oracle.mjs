...
async function handleCheckRequestResult(
    oracleOpts, details, joOut, joCall, idxAttempt, cntAttempts,
    nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
    isVerbose, isVerboseTraceDetails
) {
...
async function performCheckRequestCall(
    oracleOpts, details, joCall, idxAttempt, cntAttempts,
    nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
    isVerbose, isVerboseTraceDetails
) {
    let gp = null;
    ...
    try {
        joIn = { "method": "oracle_checkResult", "params": [ joOut.result ] };
        if( isVerboseTraceDetails ) {
            details.debug( "RPC call oracle_checkResult attempt {} of {}...",
                idxAttempt, cntAttempts );
            details.debug( "RPC call(oracle_checkResult) is {}", joIn );
        }
        await joCall.call( joIn, async function( joIn, joOut, err ) {
            if( err ) {
                if( isVerboseTraceDetails )
                    details.error( "JSON RPC call(oracle_checkResult) failed, error: {err}", err );
                await joCall.disconnect();
                return;
            }
            gp = await handleCheckRequestResult(
                oracleOpts, details, joOut, joCall, idxAttempt, cntAttempts,
                nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
                isVerbose, isVerboseTraceDetails );
        } );
...
async function handleSubmitRequestResult(
    oracleOpts, details, joOut, joCall, cntAttempts,
    nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
    isVerbose, isVerboseTraceDetails
) {
    let gp = null;
    ...
    if( !( "result" in joOut && typeof joOut.result == "string" && joOut.result.length > 0 ) ) {
        details.error( "Bad unexpected result(oracle_submitRequest), error description is {}",
    ...
    for( let idxAttempt = 0; idxAttempt < cntAttempts; ++idxAttempt ) {
        gp = await performCheckRequestCall(
            oracleOpts, details, joCall, idxAttempt, cntAttempts,
            nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
            isVerbose, isVerboseTraceDetails );
        if( gp )
...
async function performSubmitRequestCall(
    oracleOpts, details, joCall, cntAttempts,
    nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
    isVerbose, isVerboseTraceDetails
) {
    let gp = null;
    try {
        ...
        await joCall.call( joIn, async function( joIn, joOut, err ) {
            if( err ) {
                if( isVerboseTraceDetails ) {
                    details.error( "JSON RPC call(oracle_submitRequest) failed, error: {err}",
                        err );
                }
                await joCall.disconnect();
                throw new Error( "JSON RPC call(oracle_submitRequest) failed, error: " +
                    owaspUtils.extractErrorMessage( err ) );
            }
            gp = await handleSubmitRequestResult(
                oracleOpts, details,
                joOut, joCall, cntAttempts,
                nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
                isVerbose, isVerboseTraceDetails );
        } );
    } catch ( err ) {
...
export async function oracleGetGasPrice( oracleOpts, details ) {
    ...
    try {
        ...
        if( cntAttempts < 1 )
            cntAttempts = 1;
        await rpcCall.create( url, callOpts || { }, async function( joCall, err ) {
            if( err ) {
                details.error( "RPC connection problem for URL {url}, error description: {err}",
                    url, err );
                if( joCall )
                    await joCall.disconnect();
                throw new Error( "CRITICAL ORACLE ERROR: RPC connection problem for url \"" +
                    url + "\", error description: " + owaspUtils.extractErrorMessage( err ) );
            }
            gp = await performSubmitRequestCall( oracleOpts, details,
                joCall, cntAttempts,
                nMillisecondsSleepBefore, nMillisecondsSleepPeriod,
                isVerbose, isVerboseTraceDetails );
        } );
