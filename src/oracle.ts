...
export function findPowNumber(
  ...
  if (isVerbose)
    details.debug(
      "source part of request to find PoW number is {}",
      strRequestPart,
    );
  ...
  let n = 0;
  let s = "";
  if (isVerbose) details.debug("source t={}, this is UTC timestamp", t);
...
async function handleOracleCheckResultResult(
  ...
  if (isVerboseTraceDetails)
    details.debug(
      "RPC call(oracle_checkResult) parsed result field is: {}",
      joResult,
    );
  const gp = owaspUtils.toBN(joResult.rslts[0]);
  if (isVerboseTraceDetails) {
    details.success(
      "success, computed Gas Price={}={}",
      gp.toString(),
...
async function handleOracleSubmitRequestResult(
  ...
  const nMillisecondsSleepBefore =
    "nMillisecondsSleepBefore" in oracleOpts
      ? oracleOpts.nMillisecondsSleepBefore
      : 1000;
  const nMillisecondsSleepPeriod =
    "nMillisecondsSleepPeriod" in oracleOpts
      ? oracleOpts.nMillisecondsSleepPeriod
      : 3000;
  let cntAttempts = "cntAttempts" in oracleOpts ? oracleOpts.cntAttempts : 40;
  if (cntAttempts < 1) cntAttempts = 1;
  if (isVerboseTraceDetails)
  ...
  if (
    ...
    await joCall.disconnect();
    details.error(
      "Bad unexpected result(oracle_submitRequest), malformed " +
        "non-successful result is {}",
      joOut,
    );
    throw new Error(
      "ORACLE ERROR: Bad unexpected result(oracle_submitRequest)",
    );
  }
  let gp = null;
  for (let idxAttempt = 0; idxAttempt < cntAttempts; ++idxAttempt) {
    ...
    if (nMillisecondsToSleep > 0) await threadInfo.sleep(nMillisecondsToSleep);
    try {
      const joIn: any = {
      ...
      if (isVerboseTraceDetails) {
        details.debug(
          "RPC call oracle_checkResult attempt {} " + "of {}...",
          idxAttempt,
          cntAttempts,
        );
...
export async function oracleGetGasPrice(
  ...
  try {
    ...
    let isVerboseTraceDetails =
      "isVerboseTraceDetails" in oracleOpts
    ...
    details.error(
      "ORACLE RPC call problem for URL {url}, error description: {err}",
      url,
      err,
    );
    if (joCall) await joCall.disconnect();
    throw new Error(
      `ORACLE ERROR: RPC connection problem for url ${url}, ` +
