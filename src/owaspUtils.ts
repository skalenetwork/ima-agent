...
export function toURL(s: any): URL | null {
  try {
    ...
    if (!objURL.hostname) return null;
    if (objURL.hostname.length === 0) return null;
    const anyURL: any = objURL;
...
export function toStringURL(s?: any, defValue?: string): string {
  ...
  try {
    const url = toURL(s);
    if (url == null || url == undefined) return defValue;
    return url.toString();
  } catch (err) {
    return defValue;
  }
...
export function isUrlHTTP(strURL?: any): boolean {
  try {
    if (!validateURL(strURL)) return false;
...
export function toBoolean(value?: any): boolean {
  ...
  try {
    ...
    if (typeof value === "string") {
      ...
      if (ch == "y" || ch == "t") b = true;
      else if (validateInteger(value)) b = !!toInteger(value);
      else if (validateFloat(value)) b = !!toFloat(value);
      else b = !!b;
    } else b = !!b;
  } catch (err) {
    b = false;
...
export function validateEthAddress(value?: any): boolean {
  try {
    if (validateInputAddresses(ensureStartsWith0x(value))) return true;
  } catch (err) {}
  return false;
}
...
export function validateEthPrivateKey(value?: any): boolean {
  try {
    ...
    if (ethersWallet.address) return true;
  } catch (err) {}
  return false;
}
...
export function verifyArgumentWithNonEmptyValue(joArg?: any): any {
  if (
    !joArg.value ||
    (typeof joArg.value === "string" && joArg.value.length === 0)
  ) {
    console.log(
      log.fmtFatal(
        "(OWASP) Value {} of argument {} must not be empty",
        joArg.value,
        joArg.name,
      ),
    );
...
export function verifyArgumentIsURL(joArg?: any): any {
  try {
    ...
    if (url == null) {
      console.log(
        log.fmtFatal(
          "(OWASP) Value {} of argument {} must be valid URL",
          joArg.value,
          joArg.name,
        ),
      );
...
export function verifyArgumentIsInteger(joArg?: any): any {
  try {
    ...
    if (!validateInteger(joArg.value)) {
      console.log(
        log.fmtFatal(
          "(OWASP) Value {} of argument {} must be valid integer",
          joArg.value,
          joArg.name,
        ),
      );
...
export function verifyArgumentIsIntegerIpPortNumber(joArg: any): any {
  try {
    ...
    console.log(
      log.fmtFatal(
        "(OWASP) Value {} of argument {} must be " +
          "valid integer IP port number",
        joArg.value,
        joArg.name,
      ),
    );
  ...
  try {
    ...
    if (stats.isDirectory()) {
      console.log(
        log.fmtFatal(
          "(OWASP) Value {} of argument {} must be " +
            "path to existing file, path to folder provided",
          joArg.value,
          joArg.name,
        ),
      );
    ...
    if (!stats.isFile()) {
      console.log(
        log.fmtFatal(
          "(OWASP) Value {} of argument {} must be " +
            "path to existing file, bad path provided",
          joArg.value,
          joArg.name,
        ),
      );
    ...
    console.log(
      log.fmtFatal(
        "(OWASP) Value {} of argument {} must be " + "path to existing file",
        joArg.value,
        joArg.name,
      ),
    );
  ...
  try {
    ...
    if (stats.isFile()) {
      console.log(
        log.fmtFatal(
          "(OWASP) Value {} of argument {} must be " +
            "path to existing folder, path to file provided",
          joArg.value,
          joArg.name,
        ),
      );
    ...
    if (!stats.isDirectory()) {
      console.log(
        log.fmtFatal(
          "(OWASP) Value {} of argument }{} must be " +
            "path to existing folder, bad path provided",
          joArg.value,
          joArg.name,
        ),
      );
    ...
    console.log(
      log.fmtFatal(
        "(OWASP) Value {} of argument {} must be " + "path to existing folder",
        joArg.value,
        joArg.name,
      ),
    );
...
export function verifyArgumentIsArrayOfIntegers(joArg?: any): any[] {
  try {
    ...
    if (joArg.value.length < 3) {
      console.log(
        log.fmtFatal(
          "(OWASP) Length {} of argument {} must be " + "bigger than 2",
          joArg.value.length,
          joArg.name,
        ),
      );
    ...
    if (joArg.value[0] !== "[" || joArg.value[joArg.value.length - 1] !== "]") {
      console.log(
        log.fmtFatal(
          "(OWASP) First and last symbol {} of argument {} must be " +
            "brackets",
          joArg.value,
          joArg.name,
        ),
    ...
    console.log(
      log.fmtFatal(
        "(OWASP) Value {} of argument {} must be valid integer array",
        joArg.value,
        joArg.name,
      ),
    );
...
export function cloneObjectByRootKeys(joIn?: any): any {
  const joOut: any = {};
  const arrKeys = Object.keys(joIn);
...
export function parseMoneySpecToWei(
  ...
  try {
    ...
    if (s == null || s == undefined) {
      if (isThrowException) throw new Error("no parse-able value provided");
      return "0";
    }
    s = s.toString().trim();
    let strNumber = "";
    ...
    if (strNumber == "") throw new Error("no number or float value found");
    ...
    if (ddr != null) s = s.div(toBN(ddr));
    if (mlr != null) s = s.mul(toBN(mlr));
    s = s.toString();
    return s;
  } catch (err: any) {
    if (isThrowException) {
      throw new Error(
        `Parse error in parseMoneySpecToWei( ${s} ), ` + `error is: ${err}`,
      );
    }
  }
  return "0";
...
export function privateKeyToAccountAddress(keyPrivate: string): string {
  return ethersMod.ethers.utils.computeAddress(ensureStartsWith0x(keyPrivate));
}

export function fnAddressImpl_(anyThis: any): string {
  if (
    anyThis.address_ == undefined ||
    anyThis.address_ == null ||
    anyThis.address_ == ""
  ) {
    if (anyThis.privateKey)
      anyThis.address_ = privateKeyToAccountAddress(
        anyThis.privateKey,
      ).toString();
  }
...
export function getEthersProviderFromURL(
  ...
  const joConnectionInfo: any = {
    // see https://docs.ethers.io/v5/api/utils/web/#ConnectionInfo
    url: strURL,
    allowInsecureAuthentication: true,
  };
  if (userName) {
    joConnectionInfo.user = userName;
    if (userPwd) joConnectionInfo.password = userPwd;
  }
...
export function ethersProviderToUrl(
  ...
  if (
    ethersProvider &&
    "connection" in ethersProvider &&
    typeof ethersProvider.connection === "object" &&
    "url" in ethersProvider.connection &&
    typeof ethersProvider.connection.url === "string"
  )
...
export function toBNbasic(x?: any, optionalRadix?: number): any {
  try {
    ...
    console.log(
      `CRITICAL ERROR: Failure in owaspUtils.toBNbasic( ${x} ): ${err}`,
    );
    throw err;
  }
}
...
export function toBN(arg: any): any {
  ...
  } else if (arg) return toBNbasic(arg); // try to convert as is
...
export function clearInterval2(iv: any): void {
  if (!iv) return;
  if (!("real_iv" in iv)) return;
  if (!iv.real_iv) return;
