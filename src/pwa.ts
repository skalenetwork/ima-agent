...
function computeWalkNodeIndices(
  nNodeNumber: number,
  nNodesCount: number,
): number[] {
  if (
    nNodeNumber === null ||
    nNodeNumber === undefined ||
    nNodesCount === undefined
  )
    return [];
  if (nNodesCount <= 1) return []; // PWA is N/A
  if (!(nNodeNumber >= 0 && nNodeNumber < nNodesCount)) return []; // PWA is N/A
  let i = nNodeNumber - 1;
  if (i < 0) i = nNodesCount - 1;
  const arrWalkNodeIndices: number[] = [];
  for (; true; ) {
    if (i == nNodeNumber) break;
...
export function checkLoopWorkTypeStringIsCorrect(
  strLoopWorkType: string,
): boolean {
  if (!strLoopWorkType) return false;
...
export async function checkOnLoopStart(
  ...
  try {
    ...
    if (!imaState.isPWA) return true; // PWA is N/A
    if (imaState.nNodesCount <= 1) return true; // PWA is N/A
    if (
      !(
        imaState.nNodeNumber >= 0 && imaState.nNodeNumber < imaState.nNodesCount
      )
    )
      return true; // PWA is N/A
    if (!imaState.joSChainNetworkInfo) return true; // PWA is N/A
    ...
    const arrWalkNodeIndices: number[] = computeWalkNodeIndices(
      imaState.nNodeNumber,
      imaState.nNodesCount,
    );
    if (imaState.isPrintPWA) {
      log.debug(
        "PWA will check loop start condition via node(s) sequence {}...",
        arrBusyNodeIndices,
      );
    }
    ...
    for (let i = 0; i < arrWalkNodeIndices.length; ++i) {
      ...
      if (
        joProps &&
        typeof joProps === "object" &&
        "isInProgress" in joProps &&
        joProps.isInProgress &&
        joProps.ts != 0 &&
        nUtcUnixTimeStamp >= joProps.ts
      ) {
          ...
          if (imaState.isPrintPWA) {
            log.warning(
              "PWA busy state timeout for node #{}, old timestamp is {}, current " +
                ...
                "allowed {} and exceeded by {} second(s)",
              walkNodeIndex,
              joProps.ts,
              nUtcUnixTimeStamp,
              d,
              imaState.nTimeoutSecondsPWA,
    ...
    if (arrBusyNodeIndices.length > 0) {
      if (imaState.isPrintPWA) {
        log.error(
          "PWA loop start condition check failed, busy node(s): {}",
          arrBusyNodeIndices,
        );
      }
      return false;
    ...
    log.critical(
      "Exception in PWA check on loop start: {err}, stack is:\n{stack}",
      err,
      err,
    );
  }
...
export async function handleLoopStateArrived(
  ...
  try {
    ...
    if (imaState.isPrintPWA) {
      log.trace(
        "PWA loop-{} state arrived for node {}, PWA state {}, arrived " +
          "signature is {}",
        se,
        nNodeNumber,
        joNode.pwaState,
    ...
    if (!isSignatureOK) throw new Error("BLS verification failed");
...
async function notifyOnLoopImpl(
  ...
  try {
    ...
    const signature = await imaBLS.doSignReadyHash(
      strMessageHash,
      imaState.isPrintPWA,
    );
    ...
    for (let i = 0; i < jarrNodes.length; ++i) {
      ...
      if (imaState.isPrintPWA) {
        log.success(
          "Was successfully sent PWA loop-{} notification to " +
            "node #{} with URL {url}",
          se,
          i,
          strNodeURL,
    ...
    log.error(
      "Exception in PWA notify on loop {}: {err}, stack is:\n{stack}",
      se,
      err,
      err,
    );
