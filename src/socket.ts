...
export const updateSocketDataStatsForMessage = function (
  joMessage: any,
  joStats: any,
): void {
  let strMethod = "_N/A_";
  if (joMessage?.method) strMethod = joMessage.method.toString();
  if (strMethod in joStats) joStats[strMethod]++;
  else joStats[strMethod] = 1;
};
export const generateSocketDataStatsJSON = function (jo: any): any {
  const joStats: any = {};
  if (
    "arrPackedMessages" in jo &&
  ...
  } else updateSocketDataStatsForMessage(jo, joStats);
  return joStats;
};
...
export class BasicServerAcceptor extends EventDispatcher {
  ...
  unregisterClientByKey(key: any): void {
    if (key in this.mapClients) {
      const entry = this.mapClients[key.toString()];
      if (entry) {
        if ("serverPipe" in entry && "clientPipe" in entry) {
          const pair = entry;
          pair.serverPipe = null;
        ...
        delete this.mapClients[key.toString()];
      }
    }
  }
  ...
  newDirectConnection(): DirectPipe | null {
    if (this.isDisposing || this.isDisposed) return null;
    ...
    this.mapClients[serverPipe.clientPort.toString()] = serverPipe;
    const self = this;
    const iv = setTimeout(function (): void {
      ...
      self.dispatchEvent(
        new UniversalDispatcherEvent("connection", {
          socket: serverPipe,
          remoteAddress: self.url ? self.url.toString() : "",
        }),
      );
...
export class BasicSocketPipe extends EventDispatcher {
  ...
  constructor() {
    ...
    this.maxAccumulatedMessagesCount = cc.toInteger(
      settings.net.pipe.maxAccumulatedMessagesCount,
    );
  ...
  disposeImpersonatedEntries(): void {
    // for external in-app usage only
    for (const [, /* keyWalk */ entryWalk] of Object.entries(
      this.mapImpersonatedEntries,
    )) {
      const entry: any = entryWalk;
      try {
  ...
  isAutoFlush(): boolean {
    if (this.maxAccumulatedMessagesCount <= 1) return true;
    const cnt = this.arrAccumulatedMessages.length;
    if (cnt == 0 || cnt < this.maxAccumulatedMessagesCount) return false;
    return true;
  }
  ...
  flush(): void {
    ...
    if (cnt == 1) {
      ...
      if (
        settings.logging.net.socket.flushOne ||
        settings.logging.net.socket.flush
      )
        console.log(this.socketLoggingTextPrefix("flush-one"), joSend);
    } else {
      joSend = { arrPackedMessages: this.arrAccumulatedMessages };
      if (
        settings.logging.net.socket.flushBlock ||
        settings.logging.net.socket.flush
      )
        console.log(
          this.socketLoggingTextPrefix("flush-block(" + cnt + ")"),
          joSend,
        );
    }
    ...
    this.arrAccumulatedMessages = [];
    if (this.relayClientSocket) this.relayClientSocket.flush();
  }
  ...
  receive(data: any): void {
    ...
    if (settings.logging.net.socket.receiveMethodStats) {
      console.log(
        this.socketLoggingTextPrefix("receive-method-stats(" + 1 + ")"),
        generateSocketDataStatsJSON(jo),
      );
    }
    if (settings.logging.net.socket.receive)
...
export const isRunningInWorker = function (): boolean {
  if (self.document === undefined) return true;
  return false;
};
  ...
  onMessage: function (worker: any, data: any): boolean {
    ...
    switch (jo.workerMessageType) {
      case "inWorkerConnect":
        {
          if (!(jo.workerUUID in gMapAwaitingInWorkerClients)) return false;
          const pipe: any =
            gMapAwaitingInWorkerClients[jo.workerUUID.toString()];
          pipe.performSuccessfulConnection();
        }
        return true;
      case "inWorkerDisconnect":
        {
          if (!(jo.workerUUID in gMapConnectedInWorkerClients)) return false;
...
export const inWorkerAPIs: any = {
  onMessage: function (data: any): boolean {
    const jo: any = socketReceivedDataReverseMarshall(data);
    if (
      !("workerMessageType" in jo) ||
      typeof jo.workerMessageType !== "string" ||
    ...
    if (
      ...
      return false; // TO-DO: send error answer and return true
    if (!(jo.workerEndPoint in gMapLocalServers)) return false; // TO-DO: send error answer and return true
...
export class InWorkerServerPipe extends BasicSocketPipe {
  ...
  constructor(acceptor: any, clientPort: string, fnSend: any) {
    ...
    this.url =
      "in_worker_server_pipe://" + acceptor.strEndPoint + ":" + clientPort;
    this.acceptor.mapClients[this.clientPort.toString()] = this;
    this.fnSend(
      "inWorkerConnect",
      this.acceptor.strEndPoint,
      this.clientPort,
      {},
    );
    const self = this;
    const iv = setTimeout(function (): void {
      ...
      self.acceptor.dispatchEvent(
        new UniversalDispatcherEvent("connection", {
          socket: self,
          remoteAddress: self.url.toString(),
        }),
      );
    }, 0);
  }
  ...
  performDisconnect(): void {
    ...
    this.isConnected = false;
    if (this.acceptor) this.acceptor.unregisterClientByKey(this.clientPort);
    this.dispatchEvent(new UniversalDispatcherEvent("close", { socket: this }));
    this.acceptor = null;
    this.fnSend = null;
    this.url = "";
  ...
  implSend(data: any): void {
    if (
      ...
      this.dispatchEvent(
        new UniversalDispatcherEvent("error", {
          socket: this,
          message: s.toString(),
        }),
      );
...
export class InWorkerSocketServerAcceptor extends BasicServerAcceptor {
  ...
  performAccept(jo: any): boolean {
    if (jo.workerUUID in this.mapClients) return false; // TO-DO: send error answer and return true
    const p: InWorkerServerPipe = new InWorkerServerPipe(
      this,
      jo.workerUUID.toString(),
      this.fnSend,
    );
    p.checkItself();
    return true;
  ...
  performDisconnect(jo: any): boolean {
    if (!(jo.workerUUID in this.mapClients)) return false; // TO-DO: send error answer and return true
    const pipe = this.mapClients[jo.workerUUID.toString()];
    pipe.performDisconnect();
    return true;
  }

  receiveForClientPort(clientPort: any, jo: any): boolean {
    if (!(clientPort in this.mapClients)) return false; // TO-DO: send error answer and return true
    const pipe = this.mapClients[clientPort.toString()];
    pipe.receive(jo);
    return true;
  }
}
...
export class OutOfWorkerSocketClientPipe extends BasicSocketPipe {
  ...
  constructor(strEndPoint: string, worker: any, fnSend?: any) {
    ...
    this.url =
      "out_of_worker_client_pipe://" + this.strEndPoint + ":" + this.clientPort;
    ...
    gMapAwaitingInWorkerClients[this.clientPort.toString()] = this;
  }
  ...
  performDisconnect(): void {
    ...
    this.isConnected = false;
    // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
    delete gMapConnectedInWorkerClients[this.clientPort.toString()];
    this.fnSend(
      this.worker,
      "inWorkerDisconnect",
      this.strEndPoint,
  ...
  performSuccessfulConnection(): void {
    // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
    delete gMapAwaitingInWorkerClients[this.clientPort];
    gMapConnectedInWorkerClients[this.clientPort.toString()] = this;
    this.isConnected = true;
...
export class OutOfWorkerRelay extends EventDispatcher {
  ...
  constructor(
    ...
    self.isAutoFlushIncoming = isAutoFlushIncoming
      ? true
      : !!isAutoFlushIncoming;
    self.isAutoFlushOutgoing = isAutoFlushOutgoing
      ? true
      : !!isAutoFlushOutgoing;
    if (!acceptor) {
      throw new Error(
        `OutOfWorkerRelay ${self.strRelayName} needs acceptor ` +
          "for normal functionality",
      );
    }
    if (typeof fnCreateClient !== "function") {
      throw new Error(
        `OutOfWorkerRelay ${self.strRelayName} needs callback ` +
    ...
    // eslint-disable-next-line max-lines-per-function
    self.onConnection_ = function (eventData: any): void {
      const pipeIncoming: any = eventData.socket;
      let pipeOutgoing: any = null;
      if (
        !("remoteAddress" in eventData) ||
      ...
      self.dispatchEvent(
        new UniversalDispatcherEvent("connection", {
          relay: self,
          socket: pipeIncoming,
          remoteAddress: pipeIncoming.strSavedRemoteAddress.toString(),
        }),
      );
      ...
      let _onExternalPipeClose: any = function (): void {
        ...
        self.dispatchEvent(
          new UniversalDispatcherEvent("close", {
            relay: self,
            socket: pipeIncoming,
            remoteAddress: pipeIncoming.strSavedRemoteAddress.toString(),
      ...
      let _onExternalPipeError: any = function (eventData: any): void {
      ...
      let _onRelayPipeError: any = function (eventData: any): void {
        if (settings.logging.net.relay.error) {
          console.warn(
            'Relay client  "' +
              self.strRelayName +
      ...
      let _onExternalPipeMessage: any = function (eventData: any): void {
        ...
        if (!pipeOutgoing) {
          throw new Error(
            `Relay ${self.strRelayName} is not completely initialized ` +
              "and cannot transfer messages",
          );
        }
        self.dispatchEvent(
          new UniversalDispatcherEvent("message", {
            relay: self,
            socket: pipeIncoming,
            remoteAddress: pipeIncoming.strSavedRemoteAddress.toString(),
            isExternalSocket: true,
        ...
        if (self.isAutoFlushIncoming) pipeOutgoing.flush();
      };
      let _onRelayPipeMessage: any = function (eventData: any): void {
        if (settings.logging.net.relay.rawMessage) {
          console.log(
            'Relay "' +
              self.strRelayName +
        ...
        pipeOutgoing.send(joMessage);
        if (self.isAutoFlushOutgoing) pipeOutgoing.flush();
      };
      ...
      if (!pipeOutgoing) {
        pipeIncoming.dispose();
        throw new Error(
          `Relay ${self.strRelayName} failed to initialize ` +
  ...
  dispose(): void {
    this.isDisposing = true;
    if (this.acceptor) this.acceptor.off("connection", this.onConnection_);
    this.onConnection_ = null;
    super.dispose();
  }

  flush(): void {
    if (this.acceptor) this.acceptor.flush();
  }
}
...
export class OneToOneRelay extends EventDispatcher {
  ...
  constructor(
    ...
    if (
      !("strSavedRemoteAddress" in pipeIncoming) ||
      pipeIncoming.strSavedRemoteAddress == null ||
      pipeIncoming.strSavedRemoteAddress == undefined
    )
      pipeIncoming.strSavedRemoteAddress =
        pipeIncoming.constructor.name.toString();
    if (
      !("strSavedRemoteAddress" in pipeOutgoing) ||
      pipeOutgoing.strSavedRemoteAddress == null ||
      pipeOutgoing.strSavedRemoteAddress == undefined
    )
      pipeOutgoing.strSavedRemoteAddress =
        pipeOutgoing.constructor.name.toString();
    ...
    let _onIncomingPipeError: any = function (eventData: any): void {
      if (settings.logging.net.relay.error) {
        console.warn(
          'Relay client  "' +
            self.strRelayName +
    ...
    let _onOutgoingPipeError: any = function (eventData: any): void {
      if (settings.logging.net.relay.error) {
        console.warn(
          'Relay client  "' +
            self.strRelayName +
    ...
    let _onIncomingPipeMessage: any = function (eventData: any): void {
      if (settings.logging.net.relay.rawMessage) {
        console.log(
          'Relay "' +
            self.strRelayName +
    ...
    let _onOutgoingPipeMessage: any = function (eventData: any): void {
      if (settings.logging.net.relay.rawMessage) {
        console.log(
      ...
      if (self.isAutoFlushOutgoing) pipeIncoming.flush();
    };
    _offAllPipeEventListeners = function (): void {
  ...
  flush(): void {
    if (this.pipeIncoming) this.pipeIncoming.flush();
    if (this.pipeOutgoing) this.pipeOutgoing.flush();
  }
}
...
export class DirectPipe extends BasicSocketPipe {
      ...
      if (isBroadcastOpenEvents) {
        ...
        const iv = setTimeout(function (): void {
          ...
          self.counterPipe.dispatchEvent(
            new UniversalDispatcherEvent("open", { socket: self.counterPipe }),
          );
        }, 0);
      }
    }
  ...
  handleServerDisposed(): void {
    // this method is for using in local client/server pipe pairs
    this.performDisconnect();
    this.isConnected = false;
    this.dispatchEvent(new UniversalDispatcherEvent("close", { socket: this }));
    this.acceptor = null;
...
export class LocalSocketClientPipe extends DirectPipe {
  constructor(strEndPoint: string) {
    ...
    if (!(this.strEndPoint in gMapLocalServers)) {
      const s =
        'Cannot connect to local socket server "' +
        this.strEndPoint +
    ...
    this.url =
      "local_client_pipe://" + this.strEndPoint + ":" + this.clientPort;
    this.isConnected = true;
    const serverPipe = new LocalSocketServerPipe(
      this,
      this.acceptor,
      cc.toInteger(this.clientPort),
    );
...
export class WebSocketServerAcceptor extends BasicServerAcceptor {
  ...
  constructor(nTcpPort: null, key?: string, cert?: string) {
    ...
    } else this.wsServer = new wsModule.WebSocketServer({ port: nTcpPort });

    const self = this;
    self.wsServer.on(
      "connection",
      function (wsConnection: any, req: any): void {
        wsConnection.strSavedRemoteAddress =
          req?.connection?.remoteAddress?.toString();
        wsConnection.serverPipe = new WebSocketServerPipe(
          self,
          wsConnection,
...
export class WebSocketClientPipe extends BasicSocketPipe {
  ...
  reconnect(): void {
    this.performDisconnect();
    this.wsConnect(this.urlWS ? this.urlWS.toString() : "");
  }
  ...
  wsConnectAttempt(
    ...
    const self = this;
    try {
      if (this.isConnected || this.wsConnection) this.wsDisconnect();
      this.wsConnection = wsModule
        ? new wsModule.WebSocket(url, {
      ...
      this._onWsClose = function (event: any): void {
    ...
    if (
      reconnectAfterMilliseconds != null &&
      reconnectAfterMilliseconds != undefined
    ) {
  ...
  wsConnect(url: string): void {
    ...
    this.wsConnectAttempt(
      url,
      settings.net.ws.client.reconnectAfterMilliseconds,
      null,
    );
  }
...
export class RTCConnection extends EventDispatcher {
  ...
  constructor(strSignalingServerURL?: string, idRtcParticipant?: string) {
    ...
    this.idRtcParticipant = idRtcParticipant
      ? idRtcParticipant.toString()
      : utils.UUIDv4();
  ...
  describe(strInstanceType?: string, arrAdditionalProps?: any[]): string {
    let strInstanceDescription = !strInstanceType
      ? "participant"
      : strInstanceType.toString();
    if (
      typeof this.idRtcParticipant === "string" &&
      this.idRtcParticipant.length > 0
    )
      strInstanceDescription += " " + this.idRtcParticipant;
    const arrProps: any[] = [];
    if (this.isDisposed) arrProps.push("disposed");
    if (this.wasIdentified) arrProps.push("identified");
    if (this.pc) arrProps.push("pc");
    if (this.dc) arrProps.push("dc");
    if (
      arrAdditionalProps != null &&
    ...
    if (this.dc) {
      try {
        ...
        if (settings.logging.net.rtc.error)
          console.warn(
            this.describe() + " error closing RTC data channel:",
            err,
          );
      }
      this.dc = null;
      this.dispatchEvent(
        new UniversalDispatcherEvent("dataChannelClose", {
          detail: { actor: this },
        }),
      );
    }
  ...
  closePeer(): void {
    if (this.pc) {
      ...
      this.dispatchEvent(
        new UniversalDispatcherEvent("peerClose", { detail: { actor: this } }),
      );
      this.pc = null;
    }
  }
  ...
  onError(err: any): void {
    this.dispatchEvent(
      new UniversalDispatcherEvent("rtcParticipantError", {
        detail: { actor: this, error: err },
      }),
    );
    if (settings.logging.net.rtc.error)
  ...
  send(data: any): void {
    ...
    if (!this.dc) {
      this.onError(
        `Attempt to send message to uninitialized RTC data channel: ${s}`,
      );
      return;
    }
    try {
  ...
  onDataChannelOpen(event: any): void {
    this.dispatchEvent(
      new UniversalDispatcherEvent("dataChannelOpen", {
        detail: { actor: this },
      }),
    );
  }
  ...
  onDataChannelError(event: any): void {
    this.dispatchEvent(
      new UniversalDispatcherEvent("dataChannelError", {
        detail: { actor: this },
      }),
    );
  ...
  onDataChannelMessage(event: any): void {
    if (event.data.size) {
      if (settings.logging.net.rtc.error) {
        console.warn(
          this.describe() + " will ignore file transfer message of size",
          event.data.size,
        );
      }
    } else {
      ...
      if (data.type === "file") {
        ...
        this.dispatchEvent(
          new UniversalDispatcherEvent("dataChannelMessage", {
            detail: { actor: this, data },
          }),
        );
      }
    }
  ...
  onIceGatheringStateChange(event: any): void {
    ...
    if (!this.pc) {
      console.log(
        'WARNING: Participant "' +
          this.idRtcParticipant +
          '" ICE gathering state changed event with no pc", event is:',
        event,
      );
      return;
    }
    ...
    if (settings.logging.net.rtc.iceSignalingStateChange) {
      console.log(
        'Participant "' +
          ...
          (this.pc && "signalingState" in this.pc
  ...
  onIceNegotiationNeeded(event: any): void {
    ...
    if (settings.logging.net.rtc.iceNegotiationNeeded) {
      console.log(
        'Participant "' +
          this.idRtcParticipant +
          '" ICE negotiation needed event is:',
        event,
      );
    }
  }
...
export class RTCActor extends RTCConnection {
  ...
  constructor(
    ...
    if (offerOptions) {
      this.offerOptions.offerToReceiveAudio = !!(
        "offerToReceiveAudio" in offerOptions &&
        offerOptions.offerToReceiveAudio
      );
      this.offerOptions.offerToReceiveVideo = !!(
        "offerToReceiveVideo" in offerOptions &&
    ...
    this.signalingOptions = {
      idCategory: settings.rtcSpace.defaultSpaceCategory.toString(),
      idSpace: settings.rtcSpace.defaultSpaceName.toString(),
    };
    if (signalingOptions) {
      if (
        "idCategory" in signalingOptions &&
        typeof signalingOptions.idCategory === "string" &&
        signalingOptions.idCategory.length > 0
      )
        this.signalingOptions.idCategory =
          signalingOptions.idCategory.toString();
      if (
        "idSpace" in signalingOptions &&
        typeof signalingOptions.idSpace === "string" &&
        signalingOptions.idSpace.length > 0
      )
        this.signalingOptions.idSpace = signalingOptions.idSpace.toString();
    }
  }
  ...
  signalingPipeOpen(): void {
    try {
      ...
      self.dispatchEvent(
        new UniversalDispatcherEvent("signalingWillStart", {
          detail: { actor: this },
        }),
      );
      self.signalingPipe = new WebSocketClientPipe(self.strSignalingServerURL);
      self.signalingPipe.on("open", function (eventData: any): void {
        self.signalingPipeOnOpen(eventData);
      });
      self.signalingPipe.on("close", function (eventData: any): void {
        self.signalingPipeOnClose(eventData);
      });
      self.signalingPipe.on("error", function (eventData: any): void {
        self.signalingPipeOnError(eventData);
      });
      self.signalingPipe.on("message", function (eventData: any): void {
        self.signalingPipeOnRawMessage(eventData);
      });
      self.dispatchEvent(
        new UniversalDispatcherEvent("signalingDidStarted", {
          detail: { actor: this },
        }),
      );
    } catch (err) {
  ...
  signalingPipeClose(): void {
    if (this.signalingPipe) {
      ...
      this.dispatchEvent(
        new UniversalDispatcherEvent("signalingClosed", {
          detail: { actor: this },
        }),
      );
    }
  ...
  signalingPipeOnOpen(eventData: any): void {
    try {
      this.dispatchEvent(
        new UniversalDispatcherEvent("signalingOpened", {
          detail: { actor: this },
        }),
      );
      ...
      const joImpersonateMessage = {
        ...
        idCategory: this.signalingOptions.idCategory.toString(),
        idSpace: this.signalingOptions.idSpace.toString(),
        idRtcParticipant: this.idRtcParticipant
          ? this.idRtcParticipant.toString()
          : "",
        role: this.isCreator ? "creator" : "joiner",
      };
      if (settings.logging.net.signaling.message)
        console.log(
          " <<< " + this.describe() + " message out",
          joImpersonateMessage,
        );
      this.signalingPipe.send(joImpersonateMessage);
    } catch (err) {
      if (settings.logging.net.signaling.error) {
        console.warn(
          this.describe() + " error sending impersonation to signaling pipe:",
          err,
        );
      }
      this.onError(err);
  ...
  signalingPipeOnClose(eventData: any): void {
    this.dispatchEvent(
      new UniversalDispatcherEvent("signalingPipeClose", {
        detail: { actor: this },
      }),
    );
    if (settings.logging.net.signaling.disconnect) {
  ...
  signalingPipeOnError(eventData: any): void {
    ...
    this.dispatchEvent(
      new UniversalDispatcherEvent("signalingPipeError", {
        detail: { actor: this, error: eventData },
      }),
    );
    if (settings.logging.net.signaling.error) {
      console.warn(
  ...
  signalingPipeOnRawMessage(eventData: any): void {
    try {
      if (settings.logging.net.signaling.rawMessage) {
        console.log(
          " >>> " + this.describe() + " raw signaling message received",
          eventData,
        );
      }
      const joMessage = eventData.message;
      if (settings.logging.net.signaling.message) {
        console.log(
          " >>> " + this.describe() + " signaling message received",
          joMessage,
        );
      }
      ...
      if (settings.logging.net.signaling.error)
        console.warn(
          "Error handling raw message in " + this.describe() + ":",
          err,
        );
      this.onError(err);
    }
        ...
        if (joMessage.error == null) {
          ...
          if (settings.logging.net.signaling.generic) {
            console.log(
              "Success, " +
                this.describe() +
                " impersonated on signaling server",
            );
          }
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingPassedImpersonation", {
              detail: { actor: this },
            }),
          );
          ...
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingFailedImpersonation", {
...
export class RTCServerPeer extends RTCConnection {
  ...
  constructor(
    ...
    this.timeToPublishMilliseconds =
  ...
  initPeer(): void {
    ...
    const self = this;
    if (self.pc) return;
    self.pc = new webRtcModule.RTCPeerConnection(
      self.peerConfiguration,
      self.peerAdditionalOptions,
    );
    if (self.localMediaStream) {
      ...
      self.dc = self.pc.createDataChannel(
        settings.net.rtc.dataChannel.label,
        settings.net.rtc.dataChannel.opts,
      );
      self.dc.addEventListener("open", function (event: any): void {
        self.onDataChannelOpen(event);
      });
      self.dc.addEventListener("close", function (event: any): void {
        self.onDataChannelClose(event);
      });
      self.dc.addEventListener("error", function (event: any): void {
        self.onDataChannelError(event);
      });
      self.dc.addEventListener("message", function (event: any): void {
  ...
  signalingNegotiationCancel(): void {
    if (!this.isSignalingNegotiation) return;
    this.isSignalingNegotiation = false;
  ...
  publish(): void {
      ...
      self.timerPublishing = setTimeout(function (): void {
        ...
        if (settings.logging.net.signaling.publishTimeout) {
          console.warn(
            " !!! " +
              self.describe() +
              " offer publish timeout " +
              self.timeToPublishMilliseconds +
              " milliseconds reached",
          );
        }
        self.dispatchEvent(
    ...
    self.dispatchEvent(
      new UniversalDispatcherEvent("publishStart", {
        detail: { participant: self },
      }),
    );
    self.pc.oniceconnectionstatechange = function (event: any): void {
      self.onIceConnectionStateChange(event);
    };
    self.pc.onicegatheringstatechange = function (event: any): void {
      self.onIceGatheringStateChange(event);
    };
    self.pc.onidentityresult = function (event: any): void {
      self.onIceIdentifyResult(event);
    };
    self.pc.onsignalingstatechange = function (event: any): void {
      self.onIceSignalingStateChange(event);
    };
    self.pc.onnegotiationneeded = function (event: any): void {
      self.onIceNegotiationNeeded(event);
    };
    self.pc.createOffer(self.offerOptions).then(
      function (offerDescription: any) {
        ...
        self.dispatchEvent(
          new UniversalDispatcherEvent("offerCreated", {
            detail: { participant: self },
          }),
        );
        self.pc.setLocalDescription(offerDescription).then(
          function (): void {
            ...
            self.dispatchEvent(
              new UniversalDispatcherEvent("localDescriptionSet", {
                detail: { participant: self },
              }),
            );
            self.pc.onicecandidate = function (event: any): void {
  ...
  onOtherSideIdentified(idSomebodyOtherSide: any): void {
    ...
    this.idSomebodyOtherSide = idSomebodyOtherSide
      ? idSomebodyOtherSide.toString()
      : "";
    this.wasIdentified = true;
    this.dispatchEvent(
      new UniversalDispatcherEvent("identified", {
        detail: {
          participant: this,
  ...
  onImpersonationCompleteForCreator(): void {
    // specific for server peer
  ...
  publishOfferOnSignalingServer(): void {
    ...
    try {
      ...
      self.publishCancel();
      self.dispatchEvent(
        new UniversalDispatcherEvent("signalingNegotiationStart", {
          detail: { participant: self },
        }),
      );
        ...
        self.timerSignalingNegotiation = setTimeout(function (): void {
          ...
          if (settings.logging.net.signaling.signalingNegotiationTimeout) {
            console.warn(
              " !!! " +
                self.describe() +
                " signaling negotiation timeout " +
                self.timeToSignalingNegotiationMilliseconds +
                " milliseconds reached",
            );
          }
          self.dispatchEvent(
            new UniversalDispatcherEvent("signalingNegotiationTimeout", {
              detail: { participant: self },
            }),
          );
          if (self.rtcCreator) {
...
export class RTCCreator extends RTCActor {
    ...
    for (const [idOfferWalk, rtcPeerWalk] of Object.entries(
      ...
      if (settings.logging.net.signaling.offerUnregister)
        console.log(
          "Unregister offer",
          idOfferWalk,
          "(one of all, RTCCreator dispose)",
        );
      rtcPeer.dispose();
    }
    for (const [, /* idSomebodyOtherSideWalk */ rtcPeerWalk] of Object.entries(
      this.mapServerPeers,
    )) {
      const rtcPeer: any = rtcPeerWalk;
      rtcPeer.dispose();
  ...
  onOtherSideIdentified(idSomebodyOtherSide: any, idOffer: any): void {
    // server peer got result
    if (settings.logging.net.signaling.impersonate) {
      console.log(
        this.describe() +
          ' did identified other side RTC joiner "' +
          idSomebodyOtherSide +
          '" via offer ID ' +
          idOffer.toString(),
      );
    ...
    if (settings.logging.net.signaling.offerUnregister) {
      console.log(
        "Unregister offer",
        idOffer,
        "(onOtherSideIdentified in RTCCreator)",
      );
    }
    // eslint-disable-next-line @typescript-eslint/no-dynamic-delete
    delete this.mapServerOffers[idOffer];
    this.mapServerPeers[idSomebodyOtherSide.toString()] = rtcPeer;
    rtcPeer.onOtherSideIdentified(idSomebodyOtherSide.toString());
  }
  ...
  onRtcPeerError(rtcPeer: any, err: any): void {
    ...
    this.dispatchEvent(
      new UniversalDispatcherEvent("rtcPeerError", {
    ...
    switch (joMessage.method) {
      case "signalingPublishOffer":
        if (joMessage.error == null) {
          ...
          if (settings.logging.net.signaling.offer)
            console.log(
              "Success, " + this.describe() + " offer published (step 1)",
            );
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingPassedOfferPublish", {
              detail: { actor: this },
            }),
          );
        } else {
          if (settings.logging.net.signaling.error) {
            console.warn(
              " !!! " +
                this.describe() +
                " signaling offer publishing (step 1) error",
              joMessage.error,
            );
          }
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingFailedOfferPublish", {
              detail: { actor: this, error: joMessage.error },
            }),
          );
          this.onError(joMessage.error);
      ...
      case "signalingPublishAnswer": // server peer got result
        if (joMessage.error == null) {
          ...
          if (!(idOffer in this.mapServerOffers)) {
            const strError =
              "not a registered pending offer(signalingPublishAnswer)";
            if (settings.logging.net.signaling.error) {
              console.warn(
                " !!! " +
                  this.describe() +
          ...
          if (settings.logging.net.signaling.generic) {
            console.log(
              "Success, " +
                this.describe() +
                " got answer from candidate (step 3)",
            );
          }
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingPassedPublishAnswer", {
              detail: {
                actor: this,
                idSomebodyOtherSide: idSomebodyOtherSide.toString(),
                idOffer,
              },
            }),
          );
          ...
          const answerDescription = new webRtcModule.RTCSessionDescription(
            answer,
          );
          if (settings.logging.net.signaling.offer) {
            console.log(
          ...
          rtcPeer.pc.setRemoteDescription(answerDescription).then(
            function (): void {
              ...
              self.dispatchEvent(
                new UniversalDispatcherEvent("remoteDescriptionSet", {
                  detail: { participant: self },
                }),
              );
          ...
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingFailedPublishAnswer", {
              detail: { actor: this, error: joMessage.error },
            }),
          );
          this.onError(joMessage.error);
  ...
  send(data: any): void {
    // implementation in RTCCreator does send to all
    try {
      const s = socketSentDataMarshall(data);
      for (const [
        ,
        /* idSomebodyOtherSideWalk */ rtcPeerWalk,
...
export class RTCJoiner extends RTCActor {
  ...
  initPeer(): void {
    ...
    self.pc.addEventListener("track", function (event: any): void {
      self.dispatchEvent(
        new UniversalDispatcherEvent("trackAvailable", {
    ...
    self.pc.ondatachannel = function (event: any): void {
      self.dispatchEvent(
        new UniversalDispatcherEvent("dataChannelAvailable", {
          detail: { participant: self, event },
        }),
      );
    ...
    self.pc.onicecandidate = function (event: any): void {
      ...
      try {
        if (!self.signalingPipe) {
          if (self.dc) return; // already connected, ignore (Firefox fix)
          throw new Error("no connection to signaling server");
        }
      ...
      if (event.candidate == null || settings.net.rtc.fastPublishMode.joiner) {
        ...
        const joPublishAnswerMessage = {
          ...
          idRtcParticipant: self.idRtcParticipant
            ? self.idRtcParticipant.toString()
            : "",
          idSomebodyCreator: self.idSomebodyCreator
            ? self.idSomebodyCreator.toString()
            : "",
          idOffer: self.idOffer,
        };
        if (settings.logging.net.signaling.message) {
          console.log(
            " <<< " + self.describe() + " signaling client message out",
  ...
  delayedInitPeer(): void {
    if (this.bWasImpersonated) this.initPeer();
  }
  ...
  onOtherSideIdentified(idSomebodyOtherSide: any, idOffer: any): void {
    // client peer got result
    if (settings.logging.net.signaling.impersonate) {
      console.log(
        this.describe() +
          ' did identified other side RTC creator "' +
          idSomebodyOtherSide +
          '" via offer ID ' +
          idOffer.toString(),
      );
    }
    this.idSomebodyOtherSide = idSomebodyOtherSide.toString();
        ...
        if (joMessage.error == null) {
          ...
          if (settings.logging.net.signaling.generic) {
            console.log(
              "Success, " +
                this.describe() +
                " fetched offer from creator (step 2)",
            );
          }
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingPassedFetchOffer", {
              detail: {
                actor: this,
                idSomebodyOtherSide: idSomebodyOtherSide.toString(),
                idOffer,
          ...
          const offerDescription = new webRtcModule.RTCSessionDescription(
            offer,
          );
          if (settings.logging.net.signaling.offer) {
            console.log(
              " <<< " + self.describe() + " got offer description:",
              offerDescription,
            );
          }
          this.pc.setRemoteDescription(offerDescription).then(
            function (): void {
              ...
              self.pc.createAnswer(self.offerOptions).then(
                function (answerDescription: any): void {
                  // success
                  ...
                  self.dispatchEvent(
                    new UniversalDispatcherEvent("answerCreated", {
                      detail: { participant: self },
                    }),
                  );
          ...
          this.dispatchEvent(
            new UniversalDispatcherEvent("signalingFailedFetchOffer", {
              detail: { actor: this, error: joMessage.error },
            }),
          );
          this.onError(joMessage.error);
...
export class WebRTCServerPipe extends BasicSocketPipe {
  ...
  constructor(acceptor: any, rtcPeer: any, strSignalingServerURL: string) {
    ...
    self.url =
      "rtc_server_pipe(" + self.clientNumber + ")://" + strSignalingServerURL;
    self.rtcPeer.on("dataChannelOpen", function (jo: any): void {
      self.isConnected = true;
      self.acceptor.mapClients[self.clientPort.toString()] = self;
      self.dispatchEvent(
        new UniversalDispatcherEvent("open", { socket: self }),
      );
      self.acceptor.dispatchEvent(
        new UniversalDispatcherEvent("connection", {
          socket: self,
          strSignalingServerURL: strSignalingServerURL.toString(),
        }),
      );
    });
    self.rtcPeer.on("dataChannelMessage", function (jo: any): void {
      self.receive(jo.detail.data);
    });
    self.rtcPeer.on("rtcParticipantError", function (jo: any): void {
      self.isConnected = false;
      self.dispatchEvent(
        new UniversalDispatcherEvent("error", { socket: self, message: jo }),
      );
    });
    self.rtcPeer.on("dataChannelError", function (jo: any): void {
      self.isConnected = false;
      self.dispatchEvent(
        new UniversalDispatcherEvent("error", { socket: self, message: jo }),
      );
    });
    self.rtcPeer.on("dataChannelClose", function (jo: any): void {
      self.isConnected = false;
      self.dispatchEvent(
        new UniversalDispatcherEvent("close", { socket: self, message: jo }),
      );
    });
    self.rtcPeer.on("peerClose", function (jo: any): void {
      self.isConnected = false;
      self.dispatchEvent(
        new UniversalDispatcherEvent("close", { socket: self, message: jo }),
      );
  ...
  implSend(data: any): void {
    if (!this.isConnected || !this.rtcPeer) {
      const err =
        "Cannot send messages to disconnected WebRTC socket server pipe";
      this.onError(err);
      throw new Error(err);
    }
    const s = socketSentDataMarshall(data);
...
export class WebRTCServerAcceptor extends BasicServerAcceptor {
  ...
  constructor(
    ...
    if (this.maxActiveOfferCount < 1) this.maxActiveOfferCount = 1;
    ...
    this.timeToSignalingNegotiationMilliseconds =
      timeToSignalingNegotiationMilliseconds ??
      settings.net.rtc.timeToSignalingNegotiationMilliseconds;
    this.rtcCreator = new RTCCreator(
      this.strSignalingServerURL.toString(),
      this.idRtcParticipant.toString(),
      this.offerOptions,
      this.signalingOptions,
    );
    ...
    this.rtcCreator.on("error", function (eventData: any): void {
      self.dispatchEvent(
        new UniversalDispatcherEvent("error", {
          detail: {
            acceptor: self,
    ...
    this.rtcCreator.on("close", function (eventData: any): void {
      self.dispatchEvent(
        new UniversalDispatcherEvent("close", {
    ...
    self.rtcCreator.on("signalingPipeError", function (jo: any): void {
      self.isConnected = false;
      self.dispatchEvent(
        new UniversalDispatcherEvent("error", {
          socket: self,
  ...
  addPendingOffer(): void {
    if (this.isDisposed) return;
    const rtcPeer = new RTCServerPeer(
      this.rtcCreator,
      this.timeToPublishMilliseconds,
      this.timeToSignalingNegotiationMilliseconds,
      this.peerConfiguration,
    ...
    const self = this;
    rtcPeer.on("identified", function (event: any): void {
      if (rtcPeer.isDisposing || rtcPeer.isDisposed) return;
      if (settings.logging.net.signaling.generic) {
        console.log(
      ...
      rtcPeer.serverPipe = new WebRTCServerPipe(
        self,
        rtcPeer,
    ...
    rtcPeer.on("localDescriptionSet", function (event: any): void {
      self.dispatchEvent(
        new UniversalDispatcherEvent("peerLocalDescriptionSet", {
  ...
  removeAllPendingOffers(): void {
    ...
    this.rtcCreator.mapServerPeers = {};
    for (const [, /* idOfferWalk */ rtcPeerWalk] of Object.entries(
      this.rtcCreator.mapPendingOffers,
    )) {
      const rtcPeer: any = rtcPeerWalk;
      rtcPeer.dispose();
    }
  ...
  updateAllPendingOffers(): void {
    if (this.isDisposed) return;
    for (
      let n = Object.keys(this.mapPendingOffers).length;
...
export class WebRTCClientPipe extends BasicSocketPipe {
  ...
  implSend(data: any): void {
    if (!this.isConnected || !this.rtcPeer) {
      const s =
        "Cannot send messages to disconnected WebRTC socket client pipe";
      this.dispatchEvent(
        new UniversalDispatcherEvent("error", {
          socket: this,
          message: s.toString(),
          errorType: "dataSendError",
        }),
      );
      throw new Error(s);
    }
  ...
  reconnect(): void {
    this.performDisconnect();
    this.rtcConnect(
      this.strSignalingServerURL ? this.strSignalingServerURL.toString() : "",
    );
  }
  ...
  rtcConnect(strSignalingServerURL: string): void {
    if (strSignalingServerURL.length == 0) {
      const s =
        'Cannot connect signaling server "' +
        strSignalingServerURL +
        '", bad url';
      this.dispatchEvent(
        new UniversalDispatcherEvent("error", {
          socket: this,
          message: s.toString(),
          errorType: "badSignalingServerURL",
        }),
      );
      throw new Error(s);
    }
    ...
    while (true) {
      try {
        if (self.isConnected || self.rtcPeer) self.rtcDisconnect();
        self.rtcPeer = new RTCJoiner(
          strSignalingServerURL.toString(),
          self.idRtcParticipant.toString(),
          self.offerOptions,
          self.signalingOptions,
          self.peerConfiguration,
          self.peerAdditionalOptions,
        ); // client side
        self.strSignalingServerURL = utils.makeValidSignalingServerURL(
        ...
        self.rtcPeer.on("identified", function (event: any): void {
          if (settings.logging.net.signaling.generic) {
            console.log(
        ...
        self.rtcPeer.on("signalingPipeError", function (jo: any): void {
          self.isConnected = false;
          self.dispatchEvent(
            new UniversalDispatcherEvent("error", {
              socket: self,
