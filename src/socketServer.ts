    ...
    acceptor.on("connection", function (eventData: any): void {
      ...
      if (
        ...
        socket.strSavedRemoteAddress = eventData.remoteAddress
          ? eventData.remoteAddress.toString()
          : "";
      }
      if (self.isLogAcceptedSocket) {
        self.log(
          log.fmtInformation(
            "New socket {url} was {bright}",
        ...
        try {
          if (joMessage.method in self.mapApiHandlers) {
            ...
            if (self.isLogSocketTraffic) {
              self.log(
                log.fmtError(
                  "Socket {url} had unhandled message {}",
                  socket.strSavedRemoteAddress,
                  joMessage,
                ),
              );
        ...
        if (joAnswer != null && joAnswer != undefined) {
          if (typeof joAnswer.error === "string" && joAnswer.error.length > 0) {
            if (self.isLogSocketErrors) {
              self.log(
                log.fmtError(
                  "Socket {url} will send {sunny} {err}",
                  socket.strSavedRemoteAddress,
                  "error-answer",
                  joAnswer,
                ),
