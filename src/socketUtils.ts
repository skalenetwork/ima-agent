...
export const randomStringABC = function (
  length: number,
  arrCharacters: string,
): string {
  if (length <= 0 || arrCharacters.length == 0) return "";
  let s = "";
...
export const randomString = function (
  ...
  if (length <= 0) return "";
  isABC = isABC == null || isABC == undefined ? true : !!isABC;
  isDigits = isDigits == null || isDigits == undefined ? false : !!isDigits;
  isSpecChr = isSpecChr == null || isSpecChr == undefined ? false : !!isSpecChr;
  isPunctuation =
    isPunctuation == null || isPunctuation == undefined
      ? false
      : !!isPunctuation;
  let arrCharacters = "";
  if (isABC)
    arrCharacters += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  if (isDigits) arrCharacters += "0123456789";
  if (isSpecChr) arrCharacters += "(){}[]~!?@#$%^&*_+-='\"/\\";
  if (isPunctuation) arrCharacters += ",.:;";
  if (arrCharacters.length == 0) return "";
...
export const makeValidSignalingServerURL = function (
  ...
  return strSignalingServerURL
    ...
    : proto +
        "://" +
        settings.net.hostname +
        ":" +
        settings.net.ports.signaling;
};

export const zeroPaddingLeft = function (
  val: any,
  cntCharsNeeded: number,
): string {
  if (val == null || val == undefined) return val;
  let s = val.toString();
  while (s.length < cntCharsNeeded) s = "0" + s;
  return s;
};
export const zeroPaddingRight = function (
  val: any,
  cntCharsNeeded: number,
): string {
  if (val == null || val == undefined) return val;
  let s = val.toString();
  while (s.length < cntCharsNeeded) s = s + "0";
  return s;
};
...
export const parseDateTime = function (ts?: any): Date | null {
  ...
  if (millisecond.length > 3) millisecond = millisecond.substring(0, 3);
  else {
...
export const formatDateTime = function (
  ...
  sepTime?: string,
  sepBetween?: string,
  sepMilliseconds?: string,
): string {
  if (dt === null) return "null-date-time";
  if (dt === undefined) return "undefined-date-time";
  if (!(dt instanceof Date)) return "not-a-date-time";
  ...
  if (isDate) {
    sepDate =
      sepDate == null || sepDate == undefined || typeof sepDate !== "string"
        ? "/"
        : sepDate;
    const strDate =
  ...
  if (isTime) {
    sepTime =
      sepTime == null || sepTime == undefined || typeof sepTime !== "string"
        ? ":"
        : sepTime;
    if (isDate) {
    ...
    isMilliseconds =
      isMilliseconds == null || isMilliseconds == undefined
        ? true
        : !!isMilliseconds;
    if (isMilliseconds) {
      sepMilliseconds =
