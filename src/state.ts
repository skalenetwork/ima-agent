...
function constructChainProperties(): TPropertiesOfChains {
  return {
    mn: {
      joAccount: {
        privateKey: owaspUtils.toEthPrivateKey(
          process.env.PRIVATE_KEY_FOR_ETHEREUM,
        ),
        address: function (): string {
          return owaspUtils.fnAddressImpl_(this);
        },
        strTransactionManagerURL: owaspUtils.toStringURL(
          process.env.TRANSACTION_MANAGER_URL_ETHEREUM,
        ),
        nTmPriority:
          owaspUtils.toInteger(
            process.env.TRANSACTION_MANAGER_PRIORITY_ETHEREUM,
          ) || 5,
        ...
        strPathSslKey: (process.env.SGX_SSL_KEY_FILE_ETHEREUM ?? "")
          .toString()
          .trim(),
        strPathSslCert: (process.env.SGX_SSL_CERT_FILE_ETHEREUM ?? "")
          .toString()
          .trim(),
      ...
      strChainName: (process.env.CHAIN_NAME_ETHEREUM ?? "Mainnet")
        .toString()
        .trim(),
    ...
    sc: {
      joAccount: {
        privateKey: owaspUtils.toEthPrivateKey(
          process.env.PRIVATE_KEY_FOR_SCHAIN,
        ),
        address: function (): string {
          return owaspUtils.fnAddressImpl_(this);
        },
        strTransactionManagerURL: owaspUtils.toStringURL(
          process.env.TRANSACTION_MANAGER_URL_S_CHAIN,
        ),
        nTmPriority:
          owaspUtils.toInteger(
            process.env.TRANSACTION_MANAGER_PRIORITY_S_CHAIN,
          ) || 5,
        ...
        strPathSslKey: (process.env.SGX_SSL_KEY_FILE_S_CHAIN ?? "")
          .toString()
          .trim(),
        strPathSslCert: (process.env.SGX_SSL_CERT_FILE_S_CHAIN ?? "")
          .toString()
          .trim(),
      ...
      strChainName: (process.env.CHAIN_NAME_SCHAIN ?? "id-S-chain")
        .toString()
        .trim(),
    ...
    tc: {
      joAccount: {
        privateKey: owaspUtils.toEthPrivateKey(
          process.env.PRIVATE_KEY_FOR_SCHAIN_TARGET,
        ),
        address: function (): string {
          return owaspUtils.fnAddressImpl_(this);
        },
        ...
        nTmPriority:
          owaspUtils.toInteger(
            process.env.TRANSACTION_MANAGER_PRIORITY_S_CHAIN_TARGET,
          ) ?? 5,
        strSgxURL: owaspUtils.toStringURL(process.env.SGX_URL_S_CHAIN_TARGET),
        strSgxKeyName: owaspUtils.toStringURL(
          process.env.SGX_KEY_S_CHAIN_TARGET,
        ),
        strPathSslKey: (process.env.SGX_SSL_KEY_FILE_S_CHAIN_TARGET ?? "")
          .toString()
          .trim(),
        strPathSslCert: (process.env.SGX_SSL_CERT_FILE_S_CHAIN_TARGET ?? "")
          .toString()
          .trim(),
      ...
      strChainName: (process.env.CHAIN_NAME_SCHAIN_TARGET ?? "id-T-chain")
        .toString()
        .trim(),
...
export function get(): TIMAState {
  ...
  imaState = {
    ...
    strChainNameOriginChain: (process.env.CHAIN_NAME_SCHAIN_ORIGIN ?? "Mainnet")
      .toString()
      .trim(),
