...
export function jsonFileLoad(
  ...
  joDefault = joDefault || {};
  if (bLogOutput) log.debug("Will load JSON file {}...", strPath);
  ...
  try {
    ...
    if (bLogOutput)
      log.debug(
        "Did loaded content of JSON file {}, will parse it...",
        strPath,
      );
    ...
    log.error(
      "Failed to load JSON file {}, error is: {err}, stack is:\n{stack}",
      strPath,
      err,
      err,
    );
...
export function jsonFileSave(
  ...
  if (bLogOutput == undefined || bLogOutput == null) bLogOutput = false;
  if (bLogOutput) log.debug("Will save JSON file {}...", strPath);
  try {
    ...
    log.error(
      " failed to save JSON file {}, error is: {err}, stack is:\n{stack}",
      strPath,
      err,
      err,
    );
...
export async function waitForClonedTokenToAppear(
  ...
  const strTokenSuffixUC = owaspUtils.replaceAll(
    strTokenSuffix.toUpperCase(),
    "_WITH_METADATA",
    "_with_metadata",
  );
  const strTokenSuffixLCshort = owaspUtils.replaceAll(
    strTokenSuffixLC,
    "_with_metadata",
    "",
  );
  ...
  let ts1;
  log.information(
    "Waiting for {} token to appear automatically deployed on S-Chain {}...",
    strTokenSuffixUC,
    sc.chainName,
  );
  ...
  for (let idxAttempt = 0; idxAttempt < cntAttempts; ++idxAttempt) {
    ...
    if (gMillisecondsToSleepStepWaitForClonedTokenToAppear > 0)
      await threadInfo.sleep(
        gMillisecondsToSleepStepWaitForClonedTokenToAppear,
      );
    const addressOnSChain = await contractTokenManager.callStatic[
    ...
    if (addressOnSChain != "0x0000000000000000000000000000000000000000") {
      ...
      log.success(
        "Discovered {} instantiated on S-Chain {} at address {}",
        strTokenSuffixUC,
        sc.chainName,
        addressOnSChain,
      );
  ...
  ts1 = log.timestampHR();
  log.error(
    "Failed to discover {} instantiated on S-Chain {}",
    strTokenSuffixUC,
    sc.chainName,
  );
  throw new Error(
    `Failed to discover ${strTokenSuffixUC} instantiated ` +
...
export async function waitForClonedTokenAppearErc20(
  sc: any,
  tokenERC20SC: state.TTokeInformation,
  ...
  if (
    "abi" in tokenERC20SC &&
    typeof tokenERC20SC.abi === "object" &&
    "address" in tokenERC20SC &&
    typeof tokenERC20SC.address === "string"
  ) {
    log.warning(
      "Skipping automatic ERC20 instantiation discovery, already done before",
    );
    return;
  }
  ...
  tokenERC20SC.abi = JSON.parse(
    JSON.stringify((tokensMN.joABI as any).ERC20_abi),
  );
...
export async function waitForClonedTokenAppearErc721(
  sc: any,
  tokenERC721SC: state.TTokeInformation,
  joAccountSC: state.TAccount,
  tokensMN: TTokesABIHolder,
  strMainnetName: string,
): Promise<void> {
  if (
    "abi" in tokenERC721SC &&
    typeof tokenERC721SC.abi === "object" &&
    "address" in tokenERC721SC &&
    typeof tokenERC721SC.address === "string"
  ) {
    log.warning(
      "Skipping automatic ERC721instantiation discovery, already done before",
    );
    return;
  }
  const addressCallFrom = joAccountSC.address();
  ...
  tokenERC721SC.abi = JSON.parse(
    JSON.stringify((tokensMN.joABI as any).ERC721_abi),
  );
...
export async function waitForClonedTokenAppearErc721WithMetadata(
  ...
  if (
    ...
    log.warning(
      "Skipping automatic ERC721_with_metadata instantiation discovery, " +
        "already done before",
    );
    return;
  }
  ...
  tokenERC721SC.abi = JSON.parse(
    JSON.stringify((tokensMN.joABI as any).ERC721_with_metadata_abi),
  );
  tokenERC721SC.address = addressOnSChain ? addressOnSChain.toString() : "";
}
...
export async function waitForClonedTokenAppearErc1155(
  sc: any,
  tokenERC1155SC: state.TTokeInformation,
  joAccountSC: state.TAccount,
  tokensMN: TTokesABIHolder,
  strMainnetName: string,
): Promise<void> {
  if (
    "abi" in tokenERC1155SC &&
    typeof tokenERC1155SC.abi === "object" &&
    "address" in tokenERC1155SC &&
    typeof tokenERC1155SC.address === "string"
  ) {
    log.warning(
      "Skipping automatic ERC1155 instantiation discovery, already done before",
    );
    return;
  }
  const addressCallFrom = joAccountSC.address();
  ...
  tokenERC1155SC.abi = JSON.parse(
    JSON.stringify((tokensMN.joABI as any).ERC1155_abi),
  );
...
export function hexToBytes(
  strHex?: any,
  isInversiveOrder?: boolean,
): Uint8Array {
  // convert a hex string to a byte array
  isInversiveOrder = !!(
    isInversiveOrder != null &&
    isInversiveOrder != undefined &&
    isInversiveOrder
  );
  strHex = strHex ?? "";
  ...
  if (isInversiveOrder) return arrBytes.reverse();
  return arrBytes;
}
...
export function bytesAlignLeftWithZeroes(
  arrBytes: Uint8Array,
  cntMin: number,
): Uint8Array {
  if (arrBytes.length >= cntMin) return arrBytes;
...
export function bytesAlignRightWithZeroes(
  arrBytes: Uint8Array,
  cntMin: number,
): Uint8Array {
  if (arrBytes.length >= cntMin) return arrBytes;
...
export function concatUint8Arrays(a: Uint8Array, b: Uint8Array): Uint8Array {
  ...
  if (typeof a === "string") a = hexToBytes(a);
...
export function checkKeyExistInABI(
  ...
  joABI: any,
  strKey: string,
  isExitOnError?: boolean,
): boolean {
  if (isExitOnError == null || isExitOnError == undefined) isExitOnError = true;
  try {
    if (strKey in joABI) return true;
  } catch (err) {
    if (isExitOnError) {
      log.fatal(
...
export function composeSChainNodeUrl(
  joNode: discoveryTools.TSChainNode,
): string {
...
export function composeImaAgentNodeUrl(
  ...
  let nPort = -1;
  if (
    "imaAgentRpcPort" in joNode &&
    joNode?.imaAgentRpcPort &&
